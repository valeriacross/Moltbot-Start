...
from C_shared100 import analyze_scene

# --- VERSIONE ---
# CHANGELOG 2.1.0 (17/07/2026): cambio di motore, non un patch — versione
# alzata di conseguenza su richiesta esplicita di Walter (da 2.0.2 a 2.1.0).
# Rimossi 6 import morti da shared (VALERIA_FACE, VALERIA_BODY_STRONG,
# VALERIA_BODY_SAFE, VALERIA_WATERMARK, VALERIA_NEGATIVE, VALERIA_DNA,
# build_valeria_identity — verificato via grep: zero usi fuori dalla
# dichiarazione, Filtro non inietta mai il DNA Valeria nei suoi prompt,
# confermato). Riscritti in positivo puro 3 negative prompt locali, non
# collegati al DNA: filtro underwater editorial ("NEGATIVE PROMPT — EXTRA:
# body hair..."), filtro LEGO ("NEGATIVE: real human, realistic
# proportions..."), wrapper generico prima di ogni filtro ("SUBJECT
# REFERENCE — LOCKED — DO NOT ALTER"). Causa: verificato che il modello
# dietro Flow non ha un campo negativePrompt indipendente — fix generale
# esteso anche a Filtro dopo l'audit di tutti i bot. Vedi HANDOFF sezione
# 2septendecies.
...
FILTERS = {
    ...
    "underwater_editorial": {
        ...
        "prompt": (
            ...
            "Skin stays completely smooth and hairless on the torso, arms and legs throughout."
        )
    },
    "3d_synthetic": {
        "label": "🪟 3D Synthetic",
...
def _run_lego_minifig(chat_id, uid, username, img_bytes):
    ...
    try:
        ...
        prompt = (
            ...
            f"The minifig has correct LEGO proportions throughout, with a glossy plastic texture and "
            f"bright, saturated catalog lighting."
        )
...
def _run_generation(chat_id, uid, username, data):
    ...
    try:
        full_prompt = (
            f"SUBJECT REFERENCE — HELD EXACTLY AS DESCRIBED:\n"
            f"The following description of the subject was extracted from the photo. "
            f"Reproduce the subject, outfit, accessories and setting precisely as described, "
            f"complete and faithful in every detail.\n\n"
            f"{outfit_desc}\n\n"
            f"{filter_prompt}"
        )
        last_prompt[uid] = full_prompt
