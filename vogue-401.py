...
# --- HEADER VERSIONE ---
VERSION = "4.0.1 (Persistent Prompt)"
...
def settings(m):
    ...
    bot.send_message(m.chat.id, f"<b>👠 VOGUE v{VERSION}</b>\nScegli il formato e la quantità:", reply_markup=markup)

@bot.callback_query_handler(func=lambda call: call.data in ["confirm_gen", "cancel_gen"])
def handle_confirmation(call):
    uid = call.from_user.id
    
    # MODIFICA: Rimuoviamo i bottoni ma lasciamo il testo del prompt intatto in chat
    ...
    bot.send_message(call.message.chat.id, f"🚀 <b>Generazione avviata...</b>")
    
    def run_task(idx):
        res, err = execute_generation(data['full_p'], data['img'])
        if res:
            bot.send_document(call.message.chat.id, io.BytesIO(res), visible_file_name=f"vogue_{idx+1}.jpg")
        else:
...
def ask_confirmation(m):
    ...
    preview_json = {
        ...
        "metadata": {"ar": user_ar[uid], "qty": user_qty[uid]}
    }
    ...
    bot.reply_to(m, 
        f"📝 <b>Anteprima Prompt:</b>\n<code>{safe_json}</code>\n\nProcedere?", reply_markup=markup)

# --- SERVER ---
app = flask.Flask(__name__)
@app.route('/')
def h(): return f"Vogue v{VERSION} Online"
