...
def generate_caption(scenario):
    try:
        prompt = (
            ...
            f"Rules: write exactly 1 complete sentence in ENGLISH, never cut it short, 4-6 emojis, poetic and fashion-forward tone. "
            f"The sentence must mention the location or the outfit specifically. End with proper punctuation. Return ONLY the caption, nothing else."
        )
        response = client.models.generate_content(
            model=MODEL_TEXT_ID,
            contents=[prompt],
...
def handle_callback(call):
    ...
    if call.data == "tira":
        ...
        caption_pre = generate_caption(scenario)
        if caption_pre:
            bot.send_message(cid, f"{caption_pre}\n#genderfluid #selfexpression #bodypositivity")
        full_p_pre, fmt_pre = build_prompt(scenario)
        ...
        if not scenario:
            bot.send_message(cid, "⚠️ Sessione scaduta. Tira di nuovo.", reply_markup=get_main_keyboard())
            return
        caption_retry = generate_caption(scenario)
        if caption_retry:
            bot.send_message(cid, f"{caption_retry}\n#genderfluid #selfexpression #bodypositivity")
        full_p_retry, fmt_retry = build_prompt(scenario)
