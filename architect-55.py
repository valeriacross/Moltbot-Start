...
# FIX: uid invece di cid — preferenze per utente, non per chat
user_engine = {}
user_last_input = {}
...
def execute_generation(cid, text, engine):
    ...
    bot.send_message(cid, "🔄 Vuoi rigenerare per un altro modello o iniziare da zero?", reply_markup=get_engine_kb(is_loop=True))
...
def build_prompt(m):
    ...
    wait = bot.send_message(cid, f"🚀 <b>Generazione Monolith per {user_engine[uid]}...</b>")
