...
def generate_caption(scenario):
    """Logica identica ad Architect: 3-5 emoji + frase max 10 parole."""
    try:
        caption_instr = (
        ...
        prompt = (
            f"Based on this fashion photo scenario, {caption_instr}\n\n"
            f"Location: {scenario['location']}. Outfit: {scenario['outfit']}. Mood: {scenario['mood']}."
        )
        response = client.models.generate_content(
            model=MODEL_TEXT_ID,
            contents=[prompt],
