...
def _draw_stud(draw, x, y, size, rgb):
    """Disegna un mattoncino 1×1 con stud circolare visibile."""
    from PIL import ImageDraw
    # Sfondo mattoncino
    draw.rectangle([x, y, x+size-1, y+size-1], fill=rgb)
    # Bordo ombra leggera
    shadow = tuple(max(0, c-30) for c in rgb)
    draw.rectangle([x, y, x+size-1, y+size-1], outline=shadow)
    # Stud: cerchio centrato, ~55% della dimensione del mattoncino
    pad = int(size * 0.22)
    draw.ellipse([x+pad, y+pad, x+size-pad, y+size-pad], fill=rgb, outline=shadow)
...
def _run_lego_mosaic(chat_id, uid, username, img_bytes):
    ...
    try:
        ...
        lego_waiting_list[uid] = {'counts': color_counts, 'cols': cols, 'rows': rows}
...
def _detect_bokeh_mode(src):
    """
    Analizza la foto e decide la modalità bokeh.
    Calcola il centro di massa dei pixel luminosi.
    Returns: 'radial' o 'vertical'
    """
    gray = src.convert("L")
    w, h = src.size
    ...
    center_y_ratio = bright_y_sum / bright_count / h

    if 0.30 <= center_y_ratio <= 0.65:
        return 'radial'
    else:
        return 'vertical'


def _apply_bokeh(src, mode):
    """
    Applica blur progressivo simulando bokeh F1.8.
    Zona centrale (soggetto) → nitida.
    Periferia → blur crescente.
    Returns: immagine con bokeh applicato (stessa dimensione)
    """
    ...
    # Genera maschera di distanza normalizzata (0=centro, 1=bordo)
    layers = []
    max_blur = 8   # raggi di blur massimi

    # Crea versioni con blur crescente
    blurred = [src.copy()]
    for radius in [2, 4, 6, max_blur]:
        blurred.append(src.filter(ImageFilter.GaussianBlur(radius=radius)))

    # Composita in base alla distanza dal soggetto
    ...
    for y in range(h):
        for x in range(w):
            if mode == 'radial':
                ...
                # Distanza verticale — soggetto occupa fascia centrale verticale
                dist = abs((y/h) - 0.45) * 2.2

            dist = min(dist, 1.0)

            # Scegli livello blur in base alla distanza
            if dist < 0.35:
                px = blurred[0].getpixel((x, y))
            elif dist < 0.55:
                px = blurred[1].getpixel((x, y))
            elif dist < 0.75:
                px = blurred[2].getpixel((x, y))
            elif dist < 0.90:
                px = blurred[3].getpixel((x, y))
            else:
                px = blurred[4].getpixel((x, y))
...
def _apply_bokeh_fast(src, mode):
    """
    Versione ottimizzata del bokeh — usa paste con maschera invece di pixel-by-pixel.
    Molto più veloce su immagini 37-52px.
    """
    from PIL import ImageFilter, Image as PILImage, ImageChops
    import math
...
def _is_subject_zone(x, y, cols, rows, mode):
    """True se la cella è nella zona soggetto (nitida)."""
    import math
    cx, cy = cols / 2, rows / 2
    if mode == 'radial':
        dist = math.sqrt(((x - cx) / max(cx, 1))**2 + ((y - cy) / max(cy, 1))**2)
...
# ─── LEGO GALAXY — RENDERER ISOMETRICO ────────────────────────────────────────

import math as _math

# Parametri prospettiva isometrica
_ISO_ANGLE   = 35        # gradi inclinazione faccia superiore
_ISO_TOP_H   = 0.45      # altezza relativa faccia superiore rispetto a STUD
_BRICK_H_MULT = 1.2      # moltiplicatore altezza brick rispetto a plate

def _iso_colors(rgb):
    """Genera colori per le 3 facce isometriche: top, front, shadow."""
    top    = tuple(min(255, int(c * 1.25)) for c in rgb)
    front  = rgb
    shadow = tuple(max(0,   int(c * 0.60)) for c in rgb)
    return top, front, shadow

def _draw_round_1x1(draw, x, y, s, rgb):
    """Cerchio pieno — sfondo bokeh, nessuna prospettiva."""
    pad = int(s * 0.08)
    shadow = tuple(max(0, c-35) for c in rgb)
    draw.ellipse([x+pad, y+pad, x+s-pad, y+s-pad], fill=rgb, outline=shadow)

def _draw_plate_iso(draw, x, y, ws, hs, s, rgb):
    """Plate flat con prospettiva isometrica leggera."""
    top, front, shadow = _iso_colors(rgb)
    pw = ws * s
    ph = hs * s
    oy = int(s * 0.18)   # spessore plate

    # Faccia frontale (sottile)
    draw.rectangle([x, y+ph-oy, x+pw, y+ph], fill=front)
    # Faccia superiore
    pts_top = [x, y, x+pw, y, x+pw, y+ph-oy, x, y+ph-oy]
    draw.polygon(pts_top, fill=top)
    # Bordo destro (ombra)
    draw.rectangle([x+pw-2, y, x+pw, y+ph], fill=shadow)

    # Studs sulla faccia superiore
    sp = int(s * 0.22)
    for sy in range(hs):
        for sx in range(ws):
            ox = x + sx*s + sp
            oy2 = y + sy*s + sp
            draw.ellipse([ox, oy2, ox+s-2*sp, oy2+s-2*sp-oy], fill=top, outline=shadow)

def _draw_brick_iso(draw, x, y, ws, hs, s, rgb, height_mult=1.0):
    """Brick standard con prospettiva isometrica 35° — effetto 3D."""
    top, front, shadow = _iso_colors(rgb)
    pw = ws * s
    ph = hs * s
    bh = int(s * _BRICK_H_MULT * height_mult)  # altezza visibile lato
    top_skew = int(bh * _math.sin(_math.radians(_ISO_ANGLE)))
    top_h_px = int(bh * _math.cos(_math.radians(_ISO_ANGLE)) * _ISO_TOP_H)

    # Faccia frontale
    front_y0 = y + top_h_px
    front_y1 = y + top_h_px + bh
    draw.rectangle([x, front_y0, x+pw-1, front_y1], fill=front)

    # Bordo destro ombra
    draw.polygon([x+pw-1, front_y0, x+pw+top_skew-1, y,
                  x+pw+top_skew-1, y+bh, x+pw-1, front_y1], fill=shadow)

    # Faccia superiore (parallelogramma)
    draw.polygon([x, front_y0,
                  x+top_skew, y,
                  x+pw+top_skew, y,
                  x+pw, front_y0], fill=top)

    # Highlight basso
    hl = tuple(min(255, c+30) for c in rgb)
    draw.line([x, front_y1, x+pw-1, front_y1], fill=hl, width=1)

    # Studs
    sp = int(s * 0.22)
    stud_top = top
    stud_out = shadow
    for sy_s in range(hs):
        for sx_s in range(ws):
            ox = x + sx_s*s + sp + int(top_skew * (1 - (sy_s+0.5)/hs))
            oy2 = y + sy_s * int(top_h_px/max(hs,1)) + sp//2
            ew = s - 2*sp
            eh = max(2, ew // 3)
            draw.ellipse([ox, oy2, ox+ew, oy2+eh], fill=stud_top, outline=stud_out)

def _draw_slope_iso(draw, x, y, s, rgb, direction='ne'):
    """
    Cuneo/slope isometrico — usato sui bordi del soggetto.
    direction: 'ne' = pendenza nord-est, 'nw' = nord-ovest
    """
    top, front, shadow = _iso_colors(rgb)
    bh = int(s * _BRICK_H_MULT)
    top_skew = int(bh * _math.sin(_math.radians(_ISO_ANGLE)))
    top_h_px = int(bh * _math.cos(_math.radians(_ISO_ANGLE)) * _ISO_TOP_H)

    if direction == 'ne':
        # Faccia frontale triangolare
        draw.polygon([x, y+top_h_px+bh, x+s, y+top_h_px, x+s, y+top_h_px+bh], fill=front)
        # Faccia superiore
        draw.polygon([x, y+top_h_px, x+top_skew, y, x+s+top_skew, y, x+s, y+top_h_px], fill=top)
    else:  # 'nw'
        draw.polygon([x, y+top_h_px, x+s, y+top_h_px+bh, x, y+top_h_px+bh], fill=front)
        draw.polygon([x, y+top_h_px, x+top_skew, y, x+s+top_skew, y, x+s, y+top_h_px], fill=top)

def _draw_cylinder_iso(draw, x, y, s, rgb):
    """Cilindro alto (round_1x1_tall) — per zone circolari del soggetto."""
    top, front, shadow = _iso_colors(rgb)
    bh = int(s * _BRICK_H_MULT * 1.5)
    top_h_px = int(s * 0.15)
    pad = int(s * 0.10)

    # Corpo cilindrico (rettangolo arrotondato)
    draw.rounded_rectangle([x+pad, y+top_h_px, x+s-pad, y+top_h_px+bh],
                           radius=pad*2, fill=front, outline=shadow)
    # Top ellisse
    draw.ellipse([x+pad, y, x+s-pad, y+top_h_px*2], fill=top, outline=shadow)

def _draw_jumper_iso(draw, x, y, s, rgb):
    """Jumper plate — stud singolo centrato, per transizioni."""
    _draw_plate_iso(draw, x, y, 1, 1, s, rgb)
    # Stud extra centrato e più grande
    cp = int(s * 0.30)
    shadow = tuple(max(0, c-35) for c in rgb)
    top, _, _ = _iso_colors(rgb)
    draw.ellipse([x+cp, y+cp, x+s-cp, y+s-cp//2], fill=top, outline=shadow)

def _color_variation(grid, row, col, rows, cols):
    """Calcola variazione colore media con i 4 vicini (0-255)."""
    ...
    for dr, dc in [(-1,0),(1,0),(0,-1),(0,1)]:
        ...
        if 0 <= nr < rows and 0 <= nc < cols:
            nr2, ng2, nb2 = lego_rgb.get(grid[nr][nc], (128,128,128))
            diffs.append(abs(cr-nr2)+abs(cg-ng2)+abs(cb-nb2))
    return sum(diffs)/len(diffs) if diffs else 0

def _edge_direction(src_orig_small, col, row, cols, rows):
    """Stima direzione del bordo locale — 'ne' o 'nw'."""
    if col+1 < cols and row+1 < rows:
        diag_ne = abs(sum(src_orig_small.getpixel((col+1,row))) -
                     sum(src_orig_small.getpixel((col,row+1))))
        diag_nw = abs(sum(src_orig_small.getpixel((col,row))) -
                     sum(src_orig_small.getpixel((col+1,row+1))))
        return 'ne' if diag_ne > diag_nw else 'nw'
    return 'ne'

def _run_lego_galaxy(chat_id, uid, username, img_bytes):
    """
    Filtro LEGO Galaxy con elementi 3D e prospettiva isometrica.
    Sfondo bokeh → round_1x1 piatti.
    Soggetto → brick_iso, slope, cylinder, jumper, plate in base alla zona.
    """
    try:
        from PIL import Image as PILImage, ImageDraw as PILDraw

        wait = bot.send_message(chat_id,
            "🌌 <b>Genero il LEGO Galaxy...</b>\n⏳ Rendering isometrico in corso.")

        STUD = 40
        A3_LONG  = 52
        A3_SHORT = 37
        # Canvas extra height per prospettiva isometrica (i brick escono verso l'alto)
        ISO_PAD = int(STUD * 1.5)
        ...
        # Canvas con padding superiore per prospettiva
        out_w = cols * STUD
        out_h = rows * STUD + ISO_PAD
        ...
        for row in range(rows):
            for col in range(cols):
                ...
                if not is_subj:
                    # Sfondo bokeh → cerchio piatto
                    _draw_round_1x1(draw, px, py, STUD, rgb)
                    continue

                # Soggetto → scegli elemento in base alla variazione colore
                var = _color_variation(grid, row, col, rows, cols)

                if var > 180:
                    # Alta variazione = bordo → slope
                    direction = _edge_direction(src_orig_small, col, row, cols, rows)
                    _draw_slope_iso(draw, px, py, STUD, rgb, direction)

                elif var > 110:
                    # Media-alta variazione = brick standard
                    _draw_brick_iso(draw, px, py, 1, 1, STUD, rgb, height_mult=1.0)

                elif var > 60:
                    # Media variazione = jumper (transizione)
                    _draw_jumper_iso(draw, px, py, STUD, rgb)

                elif var > 20:
                    # Bassa variazione = plate flat
                    _draw_plate_iso(draw, px, py, 1, 1, STUD, rgb)

                else:
                    # Zona omogenea = cilindro alto (effetto profondità)
                    _draw_cylinder_iso(draw, px, py, STUD, rgb)

        # Crop ISO_PAD eccedente in basso
        out = out.crop([0, 0, out_w, rows*STUD + ISO_PAD])
        ...
        lego_waiting_list[uid] = {'counts': color_counts, 'cols': cols, 'rows': rows}
        logger.info(f"✅ lego_galaxy inviato a {username} ({cols}×{rows}, {len(color_counts)} colori, bokeh={bokeh_mode})")
...
def cb_lego_list(call):
    ...
    lines = [f"🧱 <b>Lista mattoncini LEGO</b> — {cols}×{rows} studs\n"]
    for name, qty in sorted_colors:
        lines.append(f"• <b>{name}</b>: {qty} pz")
    lines.append(f"\n<b>Totale:</b> {total} mattoncini in {len(counts)} colori")

    msg = "\n".join(lines)
    chunks = [msg[i:i+4000] for i in range(0, len(msg), 4000)]
    for chunk in chunks:
        bot.send_message(call.message.chat.id, chunk, parse_mode="HTML")
    logger.info(f"📋 Lista LEGO inviata a {call.from_user.username} ({len(counts)} colori)")
