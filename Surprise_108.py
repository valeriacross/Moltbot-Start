...
def _mini_prompt_from_scenario(scenario, tipo):
    ...
    loc_short = _short(loc, 150)

    return (
        f"{tipo}\n"
        f"📍 <b>Location:</b> {html.escape(loc_short)}\n"
        f"🌤 <b>Sky:</b> {html.escape(_short(scenario.get('sky', ''), 100))}\n"
        f"👗 <b>Outfit:</b> {html.escape(_short(scenario.get('outfit', ''), 150))}\n"
        f"🎨 <b>Style:</b> {html.escape(_short(scenario.get('style', ''), 100))}\n"
        f"💃 <b>Pose:</b> {html.escape(_short(scenario.get('pose', ''), 100))}\n"
        f"✨ <b>Mood:</b> {html.escape(_short(scenario.get('mood', ''), 80))}\n"
        f"🏛 <b>Body:</b> Feminine hourglass, 180cm 85kg, D-cup bust, smooth skin"
    )
