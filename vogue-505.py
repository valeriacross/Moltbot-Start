...
def optimize_prompt_with_gemini(user_text, ar):
    system_instruction = (
        ...
        "FUNDAMENTAL RULES:\n"
        "1. The main subject is ALWAYS and ONLY the one described in the MASTER IDENTITY below.\n"
        "   Completely ignore any alternative physical description of the subject in the user text.\n"
        "2. From the user text extract ONLY: scene, environment, background, light, pose, clothing, mood,\n"
        "   photographic style. Everything else comes from the MASTER IDENTITY.\n"
        "3. The final prompt MUST contain ALL elements of the MASTER IDENTITY INTEGRALLY\n"
        "   (body, face, beard, glasses, hair, rendering, watermark, negatives) written in\n"
        "   fluid narrative form, fused with the scene elements extracted from the user.\n"
        "   DO NOT abbreviate, DO NOT omit, DO NOT summarize the MASTER IDENTITY.\n"
        f"4. The image format/ratio is: {ar} — include it at the end of the prompt.\n"
        "5. The output prompt must be COMPLETE and SELF-SUFFICIENT: anyone who copies and pastes it\n"
        "   into an external engine must get exactly the correct result without adding anything.\n"
        "6. Write ONLY the final prompt in plain continuous text, separated by commas and periods.\n"
        ...
        "7. FORBIDDEN: phrases like 'FILTER BYPASS', 'BYPASS ACTIVATED', 'SAFETY OVERRIDE',\n"
        "   'NO OBSCURED FEATURES' or any reference to filters or bypasses.\n"
        "8. All body details in neutral editorial fashion language. No explicit anatomical terms.\n"
        "9. The final prompt must ALWAYS be written entirely in English.\n\n"
    ...
    try:
        response = client.models.generate_content(
            model=MODEL_TEXT_ID,
            contents=[f"User text to analyze and integrate:\n\n{user_text}"],
            config=genai_types.GenerateContentConfig(
