...
from C_shared100 import VALERIA_DNA, EDITORIAL_WRAPPER, build_valeria_identity, generate_caption, review_and_fix, body_art_clause, multi_subject_clause, has_multiple_figures
...
# CHANGELOG 2.5.4 (26/07/2026): il fix 2.5.3 aveva tolto solo "extreme
# close-up on face" dalla lista "Framing options" (mosaico 4 scatti) — non
# sufficiente. Walter ha mostrato 5 mosaici generati con la 2.5.3 attiva,
# in OGNI mosaico almeno uno scatto (in due casi due su quattro) resta un
# primo piano stretto sul volto, nonostante l'opzione esplicita fosse
# stata rimossa. Causa: "close-up bust" era rimasta nella stessa lista —
# framing quasi indistinguibile da quello tolto, e con IDENTITY LOCK che
# dedica più testo a volto/barba/occhiali di qualsiasi altro elemento del
# prompt, il modello sembra sovra-pescarla rispetto alle altre opzioni
# (dovrebbe uscire ~1/4 per media statistica, usciva in quasi tutti e 5 i
# mosaici mostrati). Rimossa anche questa. Non verificato se altre opzioni
# rimaste (es. "medium shot from the waist up") producano lo stesso
# effetto — nessuna segnalata finora, non toccate preventivamente.
...
# esplicitamente dice "same density and placement as described... full
# ornamental complexity reproduced completely". Causa reale trovata a
# monte, in shared 2.4.2: il campo BACKGROUND di _ANALYZE_PROMPT era
# l'unico senza richiesta di enumerazione oggetto-per-oggetto, quindi
# l'analisi di una foto con sfondo denso (decine di orologi/lanterne/
# porcellane sospese a più profondità) veniva compressa in poche frasi di
# categoria — nessun lock qui può recuperare un dettaglio mai catturato
# nel testo a monte. Aggiunta comunque la stessa clausola di densità già
# usata in OUTFIT DETAIL LOCK, per coerenza e per non perdere ulteriore
...
# foto di riferimento reale che Walter allega al prompt in Flow mostra
# occhiali rotondi/spessi/scuri, non ottagonali/sottili/tartarugato. "As
# described above" faceva riferimento a VALERIA_FACE (shared), già
# corretta — ma qui il testo "tortoiseshell" era hardcoded di nuovo,
# localmente, indipendente da shared, quindi il conflitto restava anche
# dopo il fix di shared. Sostituito nei 3 punti con lo stesso rimando
# esplicito alla foto usato in shared: "eyeglasses always present, exactly
# as shown in the attached reference photo". Non toccato il resto del
# blocco IDENTITY LOCK (età, barba lunghezza — nessuna cifra hardcoded lì,
# solo "full silver-grey beard", nessuna contraddizione accertata) né HAIR
# LOCK (blocco separato, non in discussione in questa sessione). Vedi
# HANDOFF, TODO segnato il 21/07 dopo il fix di shared 2.4.1.
...
# CHANGELOG 2.6.0 (01/08/2026): supporto foto di riferimento con 2+ soggetti
...
# ancora testato in produzione. Bump di MINOR (2.5.4 → 2.6.0), non patch:
# nuova capacità aggiunta al bot, non un fix — file rinominato di
# conseguenza in Atelier_260.py (Walter ha corretto un errore di Claude che
# aveva lasciato lo stesso nome file con solo il changelog interno
# aggiornato, in violazione della regola "mai stesso nome qualsiasi sia la
# modifica").
# CHANGELOG 2.6.1 (01/08/2026): Walter ha testato in produzione il
# multi-soggetto di 2.6.0: modalità singola ok (paragonabile a Vogue), il
# MOSAICO no — figura secondaria con volto reale invece di Valeria.
# Corretta la riga "Subject identity (face, hair, beard, glasses — ZERO
# drift)" nel checklist "What stays identical across all 4 shots" (SOLO
# mosaico — la riga equivalente in modalità singola non è stata toccata,
# già confermata funzionante) per menzionare esplicitamente che l'identità
# vale per OGNI figura quando ce ne sono più di una, non solo una. Fix di
# parte 1 in shared (2.4.10, vedi changelog lì): VALERIA_MULTI_SUBJECT_LOCK
# riscritto per essere esplicito quanto gli altri LOCK. Bump di PATCH
# (2.6.0 → 2.6.1), non minor: fix mirato su un problema osservato, non una
# nuova capacità — file rinominato in Atelier_261.py. Non ancora testato in
# produzione.
# CHANGELOG 2.6.2 (03/08/2026): Walter ha notato che la riga "Subject
# identity" del checklist mosaico (2.6.1) era statica — restava con la
# parentesi "if multiple figures are present..." anche quando FIGURES
# rileva una sola persona, testo morto in quel caso. Resa condizionale
# tramite has_multiple_figures() (shared 2.4.11) — con una sola figura
# torna alla riga originale senza la parentesi. Bump di PATCH (2.6.1 →
# 2.6.2), non minor: fix mirato, non una nuova capacità — file rinominato
# in Atelier_262.py. Non ancora testato in produzione.
# CHANGELOG 2.7.0 (03/08/2026): mostrata nell'header del prompt shooting
# editorial l'indicazione del fallback MODEL_LITE, se scattato durante
# analisi o review_and_fix — 503 ("riprova tra poco, potrebbe tornare su
# flash") vs 429 ("resta così fino alle 08:00", quota esaurita per la
# giornata). Usa GeminiClient.last_fallback_code/reset_fallback() (shared
# 2.4.12) — reset() chiamato a inizio richiesta, prima di
# describe_outfit_from_image(), così l'informazione riflette sia
# l'eventuale fallback in analisi sia quello in review_and_fix. Bump di
# MINOR (2.6.2 → 2.7.0), non patch: nuova funzionalità visibile
# all'utente, non un fix — file rinominato in Atelier_270.py. Non ancora
...
def build_shooting_prompt(outfit_desc, mode="mosaic"):
    ...
    if mode == "single":
        prompt = (
            ...
            f"precisely, as the setting of the whole scene — same quantity, density and placement of every "
            f"object, prop and architectural element listed, not simplified or reduced in number.\n\n"
            f"**Technical:** 8K, cinematic, photorealistic.\n"
        )
    else:
        _subject_identity_line = (
            "- Subject identity (face, hair, beard, glasses — ZERO drift; if multiple figures are present, "
            "EVERY figure shares this identical identity, not only one)\n"
            if has_multiple_figures(outfit_desc) else
        ...
        prompt = (
            ...
            f"- Framing options: full body, three-quarter, wide angle, over-the-shoulder, "
            f"low-angle full body, high-angle full body, medium shot from the waist up, tight crop on "
            f"hands and detail\n"
            ...
            f"**⚠️ CRITICAL — BACKGROUND LOCK:** The exact location and background described above appears "
            f"precisely, identically in ALL 4 shots, as the setting of the whole scene — same quantity, "
            f"density and placement of every object, prop and architectural element listed, not simplified "
            f"or reduced in number.\n\n"
            f"**Technical:** 8K, cinematic, photorealistic.\n"
        )
    return prompt
    ...
    def _process():
      try:
        ...
        if flt.get("shooting"):
            ...
            # --- Nota fallback MODEL_LITE, se scattato durante analisi o review_and_fix ---
            # FIX 2.4.12: 503 è transitorio (il modello può tornare disponibile a breve,
            # vale la pena riprovare tra poco); 429 è quota esaurita per la giornata
            # (resta su lite fino al reset delle 08:00, riprovare prima non cambia nulla).
            _fb = gemini.last_fallback_code
            if _fb == "503":
                fallback_note = " · ⚠️ 503 overload → lite (riprova tra poco, potrebbe tornare su flash)"
            elif _fb == "429":
                fallback_note = " · ⚠️ 429 quota esaurita → lite (resta così fino alle 08:00)"
            elif _fb == "transient":
                fallback_note = " · ⚠️ errore transitorio → lite"
            else:
                fallback_note = ""

            # --- Invia prompt ---
            header = f"🎬 <b>Shooting Editorial</b> | {mode_label}{fallback_note}\n\n"
            CHUNK = 3800
            if len(full_p) <= CHUNK:
                bot.send_message(cid,
