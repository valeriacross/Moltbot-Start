...
**Generato:** 15/07/2026, fine sessione (aggiornato da versione 14/07)
**Da:** Walter Caponi
...
## 2. STATO FILE ATTUALE (15/07/2026)

| File | Versione | Koyeb service | Run command | Chiavi API |
|------|---------|---------------|--------------|-----------|
| `C_shared100.py` | **2.3.18** | (comune a tutti) | — | — |
| `Vogue_202.py` | **2.0.2** | colossal-giselle/vogue | `python Vogue_202.py` | 2 |
| `Architect_302.py` | **3.0.2** | homely-annabelle/thearchitect | `python Architect_302.py` | 1 |
| `Atelier_209.py` | **2.0.9** | flexible-denna/atelier | `python Atelier_209.py` | 5 |
| `Filtro_201.py` | **2.0.1** | screeching-jobina/filtro | `python Filtro_201.py` | 1 |
| `Surprise_202.py` | **2.0.2** | surprise1/sorpresa | `python Surprise_202.py` | 1 |
...
## 2sedecies. SESSIONE 13/07/2026 — ALLINEAMENTO /info AL MOTORE REALE (4 BOT)
...
## 5. VOGUE — STORICO E STATO (v2.0.2)
...
## 7. ATELIER — STORICO E STATO (v2.0.9)

- **v2.0.9 (15/07) — fix generale, non un altro patch sui capelli.** Walter ha mostrato un mosaico con 2 scatti su 4 ancora calvi nonostante v2.0.8. Prima di scrivere un terzo patch mirato, verificata la causa root: documentazione ufficiale Google Cloud (prompting guide Nano Banana) conferma che il modello dietro Flow **non ha un campo `negativePrompt` indipendente** — architettura multimodale end-to-end (comprensione semantica del prompt intero → token immagine), non un diffusion model con sottrazione vettoriale come Stable Diffusion/Midjourney/Imagen (che infatti restituisce un errore 400 se si prova a passare `negativePrompt` via API su Nano Banana Pro). La guida ufficiale raccomanda esplicitamente *"use positive framing: describe what you want, not what you don't want"* come principio di prodotto per questo modello specifico — non un'opinione di stile. Questo spiega meglio delle due sessioni di patch precedenti (v2.0.7, v2.0.8) perché la calvizie continuava a ripresentarsi: si stava rinforzando un meccanismo che su questo modello non ha mai avuto l'enforcement che il nome "NEGATIVE PROMPT" suggerisce.
  **Modifiche:** rimossi TUTTI i blocchi `**NEGATIVE PROMPT:**`/`**⚠️ NEGATIVE PROMPT...**` in Atelier — nelle 3 funzioni principali (`build_full_prompt`, `build_shooting_prompt` single e mosaic) E nei 14 preset del dizionario `FILTERS` (body hair, indoor/outdoor setting, daylight, ecc. — verificato che ognuno dei 14 era ridondante con la Scene/Lighting/Framing positiva già sopra, rimossi senza sostituzione). Riscritti in positivo puro: COLOR LOCK, OUTFIT DETAIL LOCK, HAIR LOCK (tolta la coda "Do NOT generate..."), BACKGROUND LOCK, la clausola outfit-assente ("do NOT invent garments" → descrizione diretta di cosa succede invece). Aggiunto un nuovo blocco locale "⚠️ IDENTITY LOCK" (barba/occhiali/corpo, positivo) in tutte e 3 le funzioni — prima quella parte viveva SOLO nella lista negative locale, senza rinforzo positivo locale equivalente a HAIR LOCK.
  **Non toccato, deliberatamente:** `VALERIA_FACE` nello shared (righe 198-241) ha ancora frasi negative tipo "DO NOT shave it" — stesso principio si applicherebbe, ma è shared, usato anche da Vogue, fuori scope per questa sessione (Walter ha confermato scope su Atelier quando gli ho posto la domanda esplicitamente). Non elimina la varianza — Flow resta non deterministico, nessun seed disponibile — ma allinea l'approccio a quanto Google stessa dice funzionare meglio su questo modello. Non ancora testato in produzione, vedi TODO #24.
- **v2.0.8 (14/07):** fix di follow-up a v2.0.7 — protezione HAIR LOCK riscritta perché incompleta. Walter ha testato la 207 e mostrato output Flow reali (mosaico 4 scatti) con capelli lunghi mossi oltre le spalle, in tutti e 4 gli scatti — coerente, non casuale: la 207 proteggeva solo un lato (negative prompt anti-calvizie), lasciando l'altro lato (lunghezza eccessiva) completamente scoperto, quindi Flow è scivolato verso quell'estremo. Riscritto in tutte e 3 le funzioni: descrizione HAIR LOCK resa quantitativa ("short on the sides and back, nape and ears clearly visible, ending well above the shoulders" invece del generico "short... Italian cut") + negative prompt esteso su entrambi i lati insieme (`bald, shaved head, buzzcut, receding hairline` E `long hair, shoulder-length hair, hair past the shoulders, flowing hair, wavy long hair, hair extensions`). Stesso scope della 207, solo Atelier. **Esito reale (15/07): la "conferma funzionante" del 14/07 non ha retto** — un mosaico successivo ha mostrato di nuovo calvizie in 2 scatti su 4. Causa root-cause diversa, vedi v2.0.9 sopra.
