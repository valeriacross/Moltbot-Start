    ...
    try:
        ...
        if engine == "Gemini":
            instr = (
                ...
                f"PHASE 1 — SCENE EXTRACTION (do this first, before writing any prompt):\n"
                f"Look carefully at the image and extract EVERY visual detail:\n"
                f"- Environment/location: where is this? Indoor/outdoor, specific setting, architecture, props\n"
                f"- Outfit: every garment, fabric type, color (use HEX), cut, fit, embellishments, shoes\n"
                f"- Lighting: natural/artificial, direction, color temperature, shadows, mood\n"
                f"- Pose: body position, camera angle, framing, depth of field\n"
                f"- Background: textures, colors, elements, depth\n"
                f"Do NOT skip or generalize ANY detail. Do NOT replace scene elements with 'studio' or 'neutral backdrop'.\n\n"
                f"PHASE 2 — PROMPT GENERATION:\n"
                f"Using ONLY the scene details extracted in Phase 1, write a MASTER PROMPT for Gemini image generation.\n"
                f"Replace the original subject with the DNA below. Keep the scene 100% faithful to the reference.\n\n"
                f"ENGINE STYLE RULES (mandatory):\n{engine_style}\n\n"
                f"{vs_context}"
                f"STEPS:\n"
                f"1. Open with the mandatory SYNTHETIC FACE IDENTITY LOCK header as specified.\n"
...
def build_prompt_from_image(m):
    ...
    wait = bot.send_message(cid,
        ...
        f"⏳ Step 1: VisionStruct scan (max 25s) → Step 2: Prompt generation.\n"
        f"<i>Attendi ~40-60 secondi totali.</i>")
    def task():
