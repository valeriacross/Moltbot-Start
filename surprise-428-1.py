...
def build_flow_prompt(scenario, uid=None):
    """Prompt completo stile Atelier/Vogue per Flow — identità Valeria completa."""
    ...
    VALERIA_FACE = (
        ...
        "**BEARD — MANDATORY — NON-NEGOTIABLE:** "
        "Full silver-grey beard approximately 6-7cm, perfectly groomed, dense and uniform, following the jawline. "
        "The beard MUST be clearly visible in EVERY shot. "
        "DO NOT shave it. DO NOT reduce it. DO NOT replace it with stubble. "
        "A clean-shaven face is a generation FAILURE.\n"
    ...
    VALERIA_BODY = (
        ...
        "Smooth porcelain skin. Skin tone continuous from face to neck to shoulders.\n"
        "**COEXISTENCE RULE — CRITICAL:** The silver-grey beard AND the feminine body are NOT a contradiction. "
        "Both MANDATORY simultaneously. Do NOT remove the beard. Do NOT masculinize the body.\n\n"
    )
    NEGATIVE = (
        "**NEGATIVE PROMPT:** young face, female face, missing beard, beard removed, stubble, "
        "shaved face, missing glasses, face drift, flat chest, male physique, masculine body, "
        "body hair, chest hair, extra fingers, blurry hands, mismatched skin tone, "
        "studio background replacing real location, wrong setting, "
        "text overlay, JSON output, metadata."
    )
    ...
    prompt = (
        ...
        f"**OUTFIT:**\n{outfit}\n\n"
        f"**POSE:**\n{pose}. "
        f"Body weight supported, no impossible floating positions. Expression: calm, self-aware half-smile.\n\n"
        f"**MOOD & ATMOSPHERE:**\n{mood}\n\n"
        f"**PHOTOGRAPHIC STYLE:**\n{style}\n\n"
        f"**TECHNICAL:** 8K cinematic photography. 85mm lens, f/2.8, ISO 200, 1/160s. "
        f"Natural bokeh. FORMAT: {fmt}\n"
        f"WATERMARK: 'feat. Valeria Cross 👠' — elegant champagne cursive, very small, bottom center, 90% opacity.\n\n"
        f"{NEGATIVE}"
    )
    return prompt
