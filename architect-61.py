...
def handle_callbacks(c):
    ...
    if uid in user_last_input:
        ...
        execute_generation(cid, saved['text'], engine_choice, img_bytes=saved['img'])
    else:
...
def clean_output(text):
    """Rimuove frasi introduttive tipiche di Gemini che non servono nel prompt finale."""
    lines = text.splitlines()
    skip_patterns = [
    ...
    # Rimuovi le prime righe che iniziano con frasi introduttive
    while lines and any(lines[0].strip().lower().startswith(p) for p in skip_patterns):
        lines.pop(0)
    # Rimuovi eventuali righe vuote iniziali rimaste
    ...
    adapted = adapt_to_engine(clean_output(raw), engine)
