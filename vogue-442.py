...
# --- MODELLI DISPONIBILI ---
MODELS = {
    "🍌 NanoBanana": "nano-banana-pro-preview",
    "⚡ Flash Exp":  "gemini-2.0-flash-exp-image-generation",
    "🔮 Gemini 3":   "gemini-3-pro-image-preview",
    "🖼️ Imagen 4":   "imagen-4.0-generate-001",
    "💎 Imagen Ultra": "imagen-4.0-ultra-generate-001",
}
DEFAULT_MODEL = "nano-banana-pro-preview"

# Modelli Imagen hanno API diversa
...
pending_prompts = {}
user_last_prompt = {}  # Salva l'ultimo prompt inviato all'API per debug
...
# --- GENERAZIONE GEMINI ---
def execute_generation_gemini(full_prompt, model_id, img_rif_bytes=None):
    try:
        ...
        logger.error(f"❌ Eccezione generazione Gemini: {e}", exc_info=True)
        return None, f"❌ Errore interno:\n<code>{html.escape(str(e))}</code>"

# --- GENERAZIONE IMAGEN ---
def execute_generation_imagen(full_prompt, model_id, ar):
    try:
        # Imagen usa aspect_ratio invece di testo nel prompt
        ar_map = {
            "1:1": "1:1", "2:3": "2:3", "3:4": "3:4",
            "4:3": "4:3", "3:2": "3:2", "16:9": "16:9", "9:16": "9:16"
        }
        aspect_ratio = ar_map.get(ar, "2:3")
        ...
        if response.generated_images:
            ...
            return img.image.image_bytes, None

        logger.warning("⚠️ Imagen: nessuna immagine generata.")
        return None, "❌ Imagen non ha restituito immagini. Riprova."

    except Exception as e:
        logger.error(f"❌ Eccezione generazione Imagen: {e}", exc_info=True)
        return None, f"❌ Errore Imagen:\n<code>{html.escape(str(e))}</code>"

# --- DISPATCHER GENERAZIONE ---
def execute_generation(full_prompt, model_id, ar, img_rif_bytes=None):
    logger.info(f"🎨 Generazione con modello: {model_id}")
    if model_id in IMAGEN_MODELS:
        return execute_generation_imagen(full_prompt, model_id, ar)
    else:
        return execute_generation_gemini(full_prompt, model_id, img_rif_bytes)

# --- BOT TELEGRAM ---
bot = telebot.TeleBot(TOKEN, parse_mode="HTML")

# --- KEYBOARD MODELLO ---
def get_model_keyboard(uid):
    current = user_model[uid]
    markup = types.InlineKeyboardMarkup()
    row = []
    for label, model_id in MODELS.items():
        tick = "✅ " if current == model_id else ""
        row.append(types.InlineKeyboardButton(f"{tick}{label}", callback_data=f"mdl_{model_id}"))
    # 2 per riga
    for i in range(0, len(row), 2):
        markup.row(*row[i:i+2])
    return markup
...
def settings(m):
    ...
    bot.send_message(
        ...
        f"Formato: <b>{user_ar[uid]}</b> | Quantità: <b>{user_qty[uid]}</b> | Modello: <b>{user_model[uid]}</b>\n\n"
...
def cmd_modello(m):
    uid = m.from_user.id
    username = m.from_user.username or m.from_user.first_name
    logger.info(f"🔧 /modello da {username} (id={uid})")
    bot.send_message(
        m.chat.id,
        f"<b>🔧 Seleziona il modello di generazione</b>\n"
        f"Attuale: <code>{user_model[uid]}</code>",
...
def help_cmd(m):
    ...
    bot.send_message(m.chat.id,
        ...
        f"/modello — cambia modello di generazione\n"
...
def info_cmd(m):
    ...
    bot.send_message(m.chat.id,
        ...
        f"Modello attivo: <code>{user_model[uid]}</code>\n"
...
def lastprompt_cmd(m):
    ...
    if not prompt:
        ...
        return
    # Invia in chunks da 4000 chars se troppo lungo
    max_chunk = 3800
...
# --- CALLBACK: MODELLO ---
@bot.callback_query_handler(func=lambda call: call.data.startswith("mdl_"))
def handle_model_callback(call):
    uid = call.from_user.id
    username = call.from_user.username or call.from_user.first_name
    new_model = call.data.replace("mdl_", "")
    user_model[uid] = new_model
    label = next((l for l, m in MODELS.items() if m == new_model), new_model)
    logger.info(f"🔧 {username} (id={uid}) → modello: {new_model}")
    bot.answer_callback_query(call.id, f"✅ Modello: {label}")
    try:
        bot.edit_message_text(
            f"<b>🔧 Seleziona il modello di generazione</b>\n"
            f"Attuale: <code>{new_model}</code>",
        ...
        logger.warning(f"⚠️ Impossibile aggiornare markup modello: {e}")
...
def handle_confirmation(call):
    ...
    model_id = data['model']
    ar = data['ar']
    label = next((l for l, m in MODELS.items() if m == model_id), model_id)
    user_last_prompt[uid] = data['full_p']  # salva per /lastprompt
    logger.info(f"🚀 {username} (id={uid}) → generazione | qty={qty} | ar={ar} | modello={model_id}")

    bot.send_message(
        call.message.chat.id,
        f"🚀 <b>Generazione avviata!</b>\n"
        f"📸 Sto creando <b>{qty}</b> foto con <b>{label}</b>...\n"
        f"⏳ Tempo stimato: ~{qty * 20}–{qty * 40} secondi. Attendi."
    )

    def run_task(idx):
        t_start = time.time()
        logger.info(f"   🎨 Scatto {idx+1}/{qty} per {username} con {model_id}...")
        res, err = execute_generation(data['full_p'], model_id, ar, data['img'])
        ...
        if res:
            try:
                bot.send_document(
                    ...
                    caption=f"✅ Scatto {idx+1}/{qty} — {elapsed}s — {label}"
                )
...
def ask_confirmation(m):
    ...
    label = next((l for l, m in MODELS.items() if m == user_model[uid]), user_model[uid])
    markup = types.InlineKeyboardMarkup()
    ...
    bot.reply_to(
        m,
        f"📝 <b>Prompt ottimizzato</b> ({ar_snapshot} | {user_qty[uid]} foto | {label}):\n\n"
