...
def build_full_prompt(filter_key, outfit_description, scene_override=None, mode="single"):
    ...
    return (
        ...
        f"**⚠️ HAIR LOCK — ABSOLUTE PRIORITY:** The subject has short silver-grey hair (Italian cut, "
        f"slightly voluminous top, nape exposed). Hair must remain fully present and visible, unchanged "
        f"in length, color and style. Do NOT generate a bald head, shaved head, buzzcut or receding "
        f"hairline. A missing or altered hairline is a generation FAILURE.\n\n"
...
def build_shooting_prompt(outfit_desc, mode="mosaic"):
    ...
    if mode == "single":
        ...
        prompt = (
            ...
            f"{body_art_clause(outfit_desc)}"
            f"**⚠️ HAIR LOCK — ABSOLUTE PRIORITY:** The subject has short silver-grey hair (Italian cut, "
            f"slightly voluminous top, nape exposed) in EVERY shot. Hair must remain fully present and "
            f"visible, unchanged in length, color and style, identically across all 4 shots. Do NOT "
            f"generate a bald head, shaved head, buzzcut or receding hairline in any shot. A missing or "
            f"altered hairline in even one of the 4 shots is a generation FAILURE.\n\n"
            f"**⚠️ CRITICAL — BACKGROUND LOCK:** The exact location and background described above MUST appear "
            ...
            f"hair loss, female face, face drift, desaturated colors, color shift, "
            f"clothing added where none exists, garments invented from color palette, "
            f"simplified outfit, missing embellishments, missing embroidery, missing gemstones, plain fabric "
            f"replacing detailed pattern, flattened texture, generic garment reinterpretation.\n"
        )
