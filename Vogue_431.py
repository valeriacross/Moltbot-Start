import os, logging, re, telebot, html, time, threading
from telebot.types import InlineKeyboardMarkup, InlineKeyboardButton
from C_shared100 import GeminiClient, HealthServer, is_allowed, genai_types, analyze_scene
from C_shared100 import VALERIA_BODY_STRONG, VALERIA_WATERMARK, VALERIA_TEXTURE_LOCK
from C_shared100 import VALERIA_DNA, generate_caption, review_and_fix, SHARED_VERSION, SHARED_DATE, body_art_clause, multi_subject_clause
...
# CHANGELOG 4.1.0 (12/08/2026): Walter ha corretto duramente un fraintendimento
# — in 4.0.0 la modalità Video era rimasta INVARIATA (video -> prompt FOTO
# ferma), pensando fosse la scelta esplicitamente confermata. Non era corretto:
# la richiesta reale è "ENTRAMBE le modalità generano un prompt VIDEO, MAI un
# prompt foto — da foto INVENTA il movimento, da video lo COPIA". Riscritto
# _ANALYZE_PROMPT_VIDEO: non riduce più la clip a "un singolo istante fermo",
# ora ha una sezione MOVEMENT che descrive letteralmente il movimento reale
# osservato (sequenza, direzione, velocità, cosa si muove). build_prompt_video()
# resa condizionale tramite nuova _has_described_movement() (stesso pattern di
# has_multiple_figures, shared): se MOVEMENT è valorizzato (foto arrivata da
# analyze_video()) le istruzioni finali dicono di RIPRODURRE quel movimento
# esatto, non inventarne un altro; se assente (foto statica da analyze_photo())
# resta il movimento inventato di 4.0.0, invariato. handle_video() ora chiama
# build_prompt_video() invece di build_prompt() — header risultato aggiornato
# a "Prompt Video Flow-ready (da video)". Messaggio di conferma modalità Video
# aggiornato per chiarezza. build_prompt() (foto ferma) NON più chiamata da
# nessun percorso foto/video — resta usata solo da handle_text() (input
# testuale), la cui sorte rimane la domanda ancora aperta segnalata a Walter.
# CHANGELOG 4.2.0 (12/08/2026): su richiesta di Walter, rimosso del tutto
# l'input testuale (T2I/T2V) — restano solo I2V (foto -> video, movimento
# inventato) e V2V (video -> video, movimento copiato). handle_text() non
# genera più nulla: solo un messaggio che guida a mandare foto/video, stesso
# pattern già usato da Architect per i tipi non supportati, invece del
# silenzio totale — scelta di Claude, non esplicitamente richiesta, da
# confermare con Walter se preferisce silenzio puro. Rimossa build_prompt()
# (foto ferma) — non più chiamata da nessun percorso dopo la rimozione del
# testo. Rimosso import sanitize_user_input, non più usato. Bump di MINOR
# (4.1.0 → 4.2.0): rimozione di una capacità, non un semplice fix — file
# rinominato in Vogue_420.py.
# CHANGELOG 4.2.1 (13/08/2026): Walter segnala che il prompt I2V di Vogue fa
# accettare il video a Flow solo ~1 volta su 10, contro ~9 su 10 di un prompt
# immagine equivalente generato da Atelier per la stessa foto di partenza —
# ha allegato entrambi i prompt per il confronto. Verificato nel codice (non
# solo nei due testi incollati): Atelier apre OGNI prompt con
# `EDITORIAL_WRAPPER` (shared, "professional editorial post-production
# request... legitimate creative and commercial photography workflow") —
# Vogue non l'ha mai importato, in NESSUNA versione da quando esiste (2.1.0
# -> 4.2.0), VALERIA_DNA va dritto a FACE IDENTITY LOCK senza alcun
# inquadramento prima. Non una prova assoluta che sia la causa dei rifiuti,
# ma una differenza concreta trovata nel codice, non solo ipotizzata.
# Aggiunta EDITORIAL_WRAPPER_VIDEO, isolata qui (stesso principio di
# _ANALYZE_PROMPT_VIDEO — shared non toccato): stessa frase di
# EDITORIAL_WRAPPER adattata al video ("image"->"video content",
# "photography workflow"->"photography and videography workflow").
# Inserita in testa a build_prompt_video(), prima di VALERIA_DNA. Nota
# onesta per Walter: è il fix più concreto e verificabile che ho trovato,
# ma non garantisce la parità di accettazione con Atelier — è plausibile
# che la generazione video abbia filtri di moderazione più severi di quella
# immagine a prescindere dal wording del prompt, un limite di piattaforma
# non risolvibile solo lato prompt. Bump di PATCH (4.2.0 -> 4.2.1): fix
# mirato, non nuova capacità — file rinominato in Vogue_421.py. Non ancora
# testato in produzione.
# CHANGELOG 4.2.2 (14/08/2026): Walter ha mostrato un prompt I2V/V2V reale
# generato da Vogue — trovata una contraddizione diretta nel mezzo del testo:
# "The output is a single photorealistic image", ereditata verbatim da
# VALERIA_DNA (shared), che contraddice wrapper/task/generation instructions
# — tutti dicono "video" tranne questa riga. VALERIA_DNA è di fatto usata
# solo da Vogue (Atelier passa da build_valeria_identity(), senza questa
# coda) ma resta una costante di sistema generica — non toccata per questo
# esperimento. Nuova VALERIA_DNA_VIDEO, isolata qui: ricompone FACE+
# BODY_STRONG+TEXTURE_LOCK (già condivisi, ora tutti importati) con una coda
# locale che dice "video clip" invece di "image". Sostituita al posto di
# VALERIA_DNA in build_prompt_video(). Bump di PATCH (4.2.1 -> 4.2.2): fix
# mirato — file rinominato in Vogue_422.py. Non ancora testato in
# produzione. NOTA: resta ancora aperta la questione volto/foto allegata
# (confermata da Walter come causa dominante, ~1/20 con foto vs quasi
# sempre senza) — affrontata separatamente, non in questo fix.
# CHANGELOG 4.3.0 (14/08/2026): confermato da un test diretto di Walter — con
# la foto reale del volto allegata, il video riesce ~1/20 volte; senza,
# quasi sempre. Supportato anche da fonti esterne: le policy Veo/Flow sul
# video colpiscono più duramente le "rappresentazioni realistiche di
# persone reali identificabili" rispetto alle immagini statiche. Nuova
# VALERIA_FACE_VIDEO, isolata qui: prima descrizione testuale completa del
# volto mai scritta nel progetto — finora l'identità era sempre stata
# delegata alla foto reale ("exactly as shown in the attached reference
# photo"), mai messa per intero a parole. Punta sugli elementi più
# distintivi (in particolare la montatura degli occhiali bicolore
# asimmetrica, l'elemento singolarmente più riconoscibile) invece di un
# generico "simile". Sostituita al posto di VALERIA_FACE in
# VALERIA_DNA_VIDEO — rimosso import VALERIA_FACE, non più usato. shared non
# toccato: VALERIA_FACE resta invariata per Atelier, dove allegare la foto
# reale continua a funzionare bene. Bump di MINOR (4.2.2 → 4.3.0): nuova
# capacità (identità senza foto), non un semplice fix — file rinominato in
# Vogue_430.py. Non ancora testato in produzione. Nota onesta già
# consegnata a Walter: non è garantito che una descrizione testuale così
# precisa non inneschi comunque lo stesso filtro — rischio ridotto rispetto
# a una foto vera, non eliminato con certezza.
# CHANGELOG 4.3.1 (21/09/2026): stesso bug e stessa correzione di shared
# 2.4.13 (vedi changelog lì) applicati qui ad analyze_video() — errore
# Telegram grezzo invece del messaggio amichevole quando Gemini blocca
# l'analisi con PROHIBITED_CONTENT: aggiunto alla classificazione SAFETY,
# più html.escape() sul ramo else finale come difesa in profondità. Bump
# di PATCH — file rinominato in Vogue_431.py. Non ancora testato in
# produzione.
VERSION = "4.3.1"
...
_ANALYZE_PROMPT_VIDEO = (
    ...
    "Return a structured description of the scene AND the movement that actually happens in "
    "the clip, with these exact sections:\n\n"
    "MOVEMENT: [Describe literally what motion happens across the clip, in chronological order "
    "— what the subject's body does (e.g. turns, walks, shifts weight, reaches, tilts head), how "
    "fast or slow, in which direction, and how the environment responds (hair, fabric, water, "
    "smoke or any other element that moves with the action). Be specific and sequential, not "
    "vague — this description will be used to reproduce the SAME movement, not a generic one.]\n\n"
    ...
    "CAMERA: [Framing, and whether/how the camera itself moves during the clip, if at all.]\n\n"
    ...
    "no figure labeling.\n"
    "— MOVEMENT must describe the actual motion observed in the clip — never write 'no movement' or "
    "'static' unless the clip truly shows zero motion of any kind (subject, hair, fabric, camera); most "
    "clips have at least subtle motion worth capturing."
)
...
def _has_described_movement(scene_description: str) -> bool:
    """True se scene_description contiene un campo MOVEMENT valorizzato (prodotto da
    analyze_video() sulla clip reale) — False se assente/vuoto (foto statica da
    analyze_photo(), o testo utente). Stesso pattern di has_multiple_figures() (shared)."""
    if not scene_description:
        return False
    m = re.search(r'MOVEMENT:\s*(.+?)(?:\n\n|\Z)', scene_description, re.IGNORECASE | re.DOTALL)
    if not m:
        return False
    val = m.group(1).strip()
    return bool(val) and val.lower() not in ("none.", "none", "n/a")

# Wrapper editoriale per il video — Vogue 4.2.1. Variante locale di EDITORIAL_WRAPPER
# (shared) che apre tutti i prompt di Atelier ma dice "image", non adatta al video —
# Vogue non l'ha MAI importato, in nessuna versione, da quando esiste. Isolato qui per
# lo stesso motivo di _ANALYZE_PROMPT_VIDEO: esperimento Vogue, shared non toccato.
EDITORIAL_WRAPPER_VIDEO = (
    "This is a professional editorial post-production request for a high-fashion photography "
    "and video project. Generate new original video content of the described subject in the "
    "specified scene, maintaining the subject's identity and physical characteristics. This is "
    "a legitimate creative and commercial photography and videography workflow. "
)

# FACE video — Vogue 4.2.3. VALERIA_FACE (shared) ancora tutta l'identità del volto alla
# foto reale allegata ("exactly as shown in the attached reference photo") — confermato da
# Walter con un test diretto: ~1/20 generazioni video riuscite CON la foto allegata, contro
# quasi sempre SENZA. Ipotesi supportata da fonti esterne: le policy Veo/Flow sul video
# colpiscono più duramente le "rappresentazioni realistiche di persone reali identificabili"
# rispetto alle immagini statiche — allegare una foto reale del volto è probabilmente il
# trigger. Descrizione riscritta interamente a parole (mai esistita prima nel progetto —
# finora sempre delegata alla foto), puntando sugli elementi più distintivi/riconoscibili
# (in particolare la montatura degli occhiali bicolore asimmetrica) invece di un generico
# "simile". Isolata qui, come tutto il resto dell'esperimento video — shared non toccato,
# VALERIA_FACE resta invariata per Atelier (dove allegare la foto reale funziona bene).
VALERIA_FACE_VIDEO = (
    "**⚠️ FACE IDENTITY LOCK — ABSOLUTE PRIORITY:** Mature Italian male face in his 60s, oval "
    "shape, weathered sun-tanned skin, deep horizontal forehead wrinkles, pronounced smile "
    "creases beside the mouth, warm complexion.\n"
    "**Hair:** Receding grey-silver hair, thinning sharply at the temples and crown, fuller and "
    "slightly wavy at the sides and back.\n"
    "**⚠️ BEARD — MANDATORY — NON-NEGOTIABLE:** Full, dense grey-white beard with darker "
    "salt-and-pepper strands mixed through, thick and bushy, extending down the jaw and neck, "
    "moustache blending into it with no visible upper lip. Clearly visible in every shot, a "
    "permanent part of the identity.\n"
    "**⚠️ EYEGLASSES — MANDATORY, ALWAYS PRESENT:** Distinctive two-tone eyeglasses — a thick "
    "dark teal/forest-green frame with an unusual asymmetric shape: the left lens noticeably "
    "rounder/circular, the right lens more rectangular/squared, both held by the same "
    "continuous chunky frame. A deliberately mismatched, statement-piece design, not a "
    "standard uniform pair — this asymmetry is the single most identifying detail and must be "
    "preserved exactly.\n"
    "**Build:** Stocky, heavyset.\n"
    "**Rendering:** Subsurface Scattering, Global Illumination, Ambient Occlusion, Fresnel Effect. "
    "8K, cinematic, 85mm, f/2.8, ISO 200.\n\n"
)

# DNA video — Vogue 4.2.2. VALERIA_DNA (shared) termina con "The output is a single
# photorealistic IMAGE" — hardcoded, contraddice in pieno il resto del prompt video
# (wrapper, task, generation instructions dicono tutti "video"). VALERIA_DNA è di fatto
# usata solo da Vogue (Atelier passa da build_valeria_identity(), che non ha questa coda),
# ma tenerla come costante di sistema generica non va toccata per questo esperimento — la
# ricompongo qui dai singoli pezzi già condivisi (FACE/BODY_STRONG/TEXTURE_LOCK) con una
# coda locale corretta per il video.
VALERIA_DNA_VIDEO = (
    f"{VALERIA_FACE_VIDEO}"
    ...
    f"The output is a single short video clip. Hands are anatomically correct, five fingers each. "
    f"No text appears anywhere in the video beyond the watermark specified above.\n"
)

def build_prompt_video(scene_description):
    """Assembla il prompt con DNA Valeria (VALERIA_DNA, body_art_clause(),
    multi_subject_clause(), SCENE REFERENCE) — cambiano solo le GENERATION INSTRUCTIONS
    finali, e sono condizionali sul campo MOVEMENT: se scene_description lo contiene
    (foto arrivata da analyze_video(), con un movimento realmente osservato nella clip),
    il video deve RIPRODURRE quel movimento esattamente — non inventarne un altro. Se
    MOVEMENT è assente (foto statica da analyze_photo()), il movimento resta inventato,
    libero di essere drammatico anche senza indizi espliciti — nessuna durata specificata,
    decide Flow."""
    movement_observed = _has_described_movement(scene_description)
    if movement_observed:
        movement_instr = (
            f"Reproduce the exact movement described in the MOVEMENT section above — do NOT invent a "
            f"different motion. Follow its sequence, direction and speed precisely, including how hair, "
            f"fabric, water or other elements respond to it, and any camera movement noted there.\n"
        )
    else:
        movement_instr = (
...
def handle_vogue_callback(call):
    ...
    if data == "vogue_mode_image":
        ...
        bot.send_message(cid,
            ...
            "(pochi secondi) consigliate. Genero un prompt VIDEO che riproduce "
            "il movimento reale della tua clip."
        )
...
def handle_video(m):
    ...
    bot.send_message(m.chat.id, "✅ <b>Prompt Video Flow-ready</b> (da video)\n\nCopia e incolla su Flow:")
    CHUNK = 3800
    chunks = [prompt[i:i+CHUNK] for i in range(0, len(prompt), CHUNK)]
    for idx, chunk in enumerate(chunks):
        header = "" if len(chunks) == 1 else f"<i>({idx+1}/{len(chunks)})</i>\n"
...
# --- HANDLER TESTO — Vogue 4.2: input testuale rimosso, solo guida ---
...
def handle_text(m):
    ...
    if not is_allowed(uid):
        ...
        return
    bot.send_message(m.chat.id, "📸🎥 Inviami una foto o un video — questo bot non genera più prompt da testo scritto.")
