...
def analyze_photo_location(photo_bytes, mime_type="image/jpeg"):
    return "photo reference provided by user", None
...
photo_part     = {}   # uid → bytes della foto originale
...
def handle_callback(call):
    ...
    if data in ("mode_auto", "mode_auto_photo"):
        ...
        if has_photo:
            header += " <i>(allega la foto su Flow/Gemini)</i>"
...
# --- HANDLER FOTO LOCATION (5.0 — senza API) ---
...
def handle_photo_location(message):
    ...
    try:
        ...
        photo_part[uid] = file_bytes
        bot.send_message(cid,
            "📷 <b>Foto ricevuta!</b>\n\n"
            "Descrivi la location in un messaggio di testo.\n"
            "<i>Es: spiaggia rocciosa atlantica, onde forti, cielo grigio</i>",
            parse_mode="HTML")
        awaiting_photo[uid] = 'text'
        logger.info(f"📷 Foto ricevuta da {username} (id={uid}), attendo descrizione")
    except Exception as e:
        logger.error(f"❌ handle_photo_location: {e}", exc_info=True)
        bot.send_message(cid, "❌ Errore ricezione foto. Riprova.")

# --- HANDLER TESTO DESCRIZIONE LOCATION (5.0) ---
@bot.message_handler(func=lambda m: awaiting_photo.get(m.from_user.id) == 'text', content_types=['text'])
def handle_photo_description(message):
    uid = message.from_user.id
    cid = message.chat.id
    username = message.from_user.username or message.from_user.first_name
    ...
    bot.send_message(cid,
        f"✅ <b>Location:</b> <i>{html.escape(desc)}</i>\n\nCome vuoi procedere?",
        parse_mode="HTML",
        reply_markup=get_mode_with_photo_keyboard())
    logger.info(f"📍 DESC {username} (id={uid}): {desc}")
