...
**Generato:** 10/08/2026, fine sessione (aggiornato da versione 05/08)
**Da:** Walter Caponi
...
## 2. STATO FILE ATTUALE (10/08/2026)

| File | Versione | Koyeb service | Run command | Chiavi API |
|------|---------|---------------|--------------|-----------|
| `C_shared100.py` | **2.4.12** | (comune a tutti) | — | — |
| `Vogue_310.py` | **3.1.0** | colossal-giselle/vogue | `python Vogue_310.py` | 2 |
...
19. **`SHARED_DATE` (e qualunque data di bump/changelog) deve sempre essere la data odierna REALE nel momento in cui la modifica viene effettivamente applicata — mai una data dedotta dal contenuto della richiesta di Walter (timestamp di un log allegato, nome file di una foto, data citata in un messaggio).** Errore ripetuto più volte in questa sessione: i bump shared 2.4.4-2.4.8 e la rigenerazione HANDOFF sono stati datati usando le date che comparivano nei materiali allegati da Walter (log, nomi file) invece della data reale del giorno in cui Claude stava scrivendo la modifica. **Prima di scrivere qualunque data in `SHARED_DATE`, in un changelog o nel campo "Generato" dell'HANDOFF, usare la data corrente reale — se in dubbio, chiederla esplicitamente a Walter piuttosto che dedurla dal contenuto allegato.**
...
22. **Il comando "Update" deve aggiornare TUTTO il README, non solo la sezione narrativa "Fix robustezza"/TODO — in particolare l'intestazione "Ultimo aggiornamento" e la tabella "Bot attivi" (nomi file, versioni) in cima al file, facili da dimenticare perché non fanno parte del changelog.** Scoperto 05/08: per tre Update consecutivi (01/08, 02/08, 05/08) Claude ha aggiornato correttamente la sezione "Fix robustezza" e i TODO, ma ha lasciato intestazione e tabella Bot attivi ferme al 25/07 — Walter se n'è accorto da uno screenshot di GitHub (che rifletteva esattamente il file consegnato, non una mancata sincronizzazione da parte sua). **Ad ogni Update, oltre a "Fix robustezza"/TODO, controllare e aggiornare sempre anche: intestazione "Ultimo aggiornamento", tabella "Bot attivi" (file/versione per ogni bot), riga "Shared" (versione shared).**
...
40. **TODO — decisione ancora da confermare con Walter.** `/caption` e l'input testuale di Vogue restano indipendenti dalla scelta menu Immagine/Video (decisione presa da Claude senza conferma esplicita) — chiedere a Walter se va bene così o se vuole che l'input testuale sia vincolato a una delle due modalità. Vedi sezione 2quattuorvicies.
