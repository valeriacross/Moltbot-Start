...
VARIABILI = {
    ...
    "outfit_top": [
        ...
        "leather biker jacket", "elegant off-shoulder gown top",
        "sequined crop top", "oversized cashmere turtleneck",
        "tailored white shirt open at collar", "velvet bustier corset",
        "embroidered kimono jacket", "flowing chiffon wrap top",
        "sculptural architectural blazer", "backless halter neck top",
        "classic trench coat", "printed silk shirt",
        "draped one-shoulder top"
    ],
    "outfit_bottom": [
        "wide-leg palazzo trousers in ivory", "midi skirt in plaid",
        "floor-length bias-cut satin skirt", "mini skirt in faux leather",
        "structured pencil skirt in black", "flowy bohemian maxi skirt",
        "sequined evening skirt", "asymmetric wrap skirt",
        "knit midi skirt in caramel", "dramatic ball gown skirt",
        "wide culottes in linen", "pleated midi skirt in silk",
        "high-waisted a-line skirt", "sheer layered tulle skirt",
        "fitted wrap skirt with side slit"
    ],
    "scarpe": [
    ...
    "filtro_fx": [
        ...
        "⬆️⬇️ Cinematic High-Angle — extreme high angle top shot transitioning to dramatic low angle, dual perspective split-frame, architectural foreshortening",
        "⬆️ Dramatic Low-Angle — extreme low angle heroic shot, dynamic upward perspective, powerful foreshortening, monumental presence",
        "🌟 Glossy Opal — hyper-glossy opalescent skin, liquid chrome reflections, pearl iridescence, studio specular highlights, glass-like surface sheen",
        "🌈 Iridescent — iridescent holographic color shifts, rainbow light diffraction, prismatic skin tones, spectral color bleeding",
        "🌈🌈 Rainbow Neon — electric neon rainbow lighting, cyberpunk color saturation, vivid magenta cyan yellow overlays, blacklight glow",
        "🌌 Galaxy Couture — cosmic galaxy backdrop, nebula colors, star field, deep space atmosphere, Milky Way swirls, cosmic dust",
        "💛 Neon HDR — ultra HDR neon palette, extreme color contrast, electric glow, hyper-saturated shadows and highlights",
        "👻 Ghost Temporal — motion blur ghost trails, temporal echo doubles, multiple exposure time-lapse, spectral movement traces",
        "💧 Dissolvence — portrait dissolving into fluid motion trails, liquid light dissolve, Sony Venice motion smear, paint-like liquid blur",
        ...
        "🪟 3D Synthetic — photorealistic 3D CGI render, Unreal Engine 5 hyperrealism, subsurface scattering skin, ray-traced reflections",
        "🧯 Graffiti Artist — urban graffiti mural style, spray paint texture, street art bold outlines, concrete wall backdrop, tag lettering",
        ...
        def run_retry():
            ...
            if img:
                try:
                    bot.send_document(cid, io.BytesIO(img),
                        visible_file_name="sorpresa.jpg",
                        caption=f"✅ Retry — {elapsed}s",
                        reply_markup=get_retry_keyboard())
                    generic = build_generic_prompt(combo)
                    bot.send_message(cid,
                        f"📋 <b>Prompt generico:</b>\n\n<code>{html.escape(generic)}</code>")
                    logger.info(f"✅ {username} (id={uid}) — retry in {elapsed}s")
                except Exception as e:
