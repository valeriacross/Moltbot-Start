...
def _send_dual_output(cid, uid, scenario, caption_text):
    """
    Invia output duplice in ordine B:
    1. Caption (una sola)
    2. [Mini prompt Single · Prompt Flow Single]
    3. [Mini prompt Mosaic · Prompt Flow Mosaic]
    4. Pulsanti
    """
    # 1. Caption
    if caption_text:
    ...
    mini_single, _ = generate_mini_prompt(p_single)
    if mini_single:
        bot.send_message(cid,
            f"📷 <b>Scatto singolo</b>\n{mini_single}",
            parse_mode="HTML")
    CHUNK = 3800
    ...
    mini_mosaic, _ = generate_mini_prompt(p_mosaic)
    if mini_mosaic:
        bot.send_message(cid,
            f"🗂️ <b>Mosaico 4 foto</b>\n{mini_mosaic}",
            parse_mode="HTML")
    chunks_m = [p_mosaic[i:i+CHUNK] for i in range(0, len(p_mosaic), CHUNK)]
    for idx, chunk in enumerate(chunks_m):
        header = "" if len(chunks_m) == 1 else f"<i>({idx+1}/{len(chunks_m)})</i>\n"
        bot.send_message(cid,
            f"{header}<code>{html.escape(chunk)}</code>",
            parse_mode="HTML")
    ...
    # Salva entrambi i prompt
    last_flow_prompt[uid] = p_single  # compat per /conferma esistente
...
def handle_callback(call):
    ...
    if data in ("shooting_single", "shooting_mosaic"):
        pass  # non più usati — output sempre duplice
        try:
    ...
    if data == "mode_auto":
        ...
        def pick_auto():
            try:
                ...
                logger.error(f"❌ pick_auto: {err_text}")
