...
VALERIA_FACE = (
    "**FACE IDENTITY LOCK — ABSOLUTE PRIORITY — DO NOT OVERRIDE:**\n"
...
def generate_caption_from_image(img_bytes):
    ...
    Copia esatta da VogueBot 6.7.1."""
    try:
        img_part = genai_types.Part.from_bytes(data=img_bytes, mime_type="image/jpeg")
        instr = (
...
# --- CAPTION DA PROMPT ---
def generate_caption_from_prompt(outfit_desc, filter_key=None, settings=None):
    """Genera caption social dalla descrizione outfit/scena — NON dal prompt completo.
    Usa solo location, outfit, colori e mood — zero riferimenti alla persona."""
    try:
        # Estrae solo le sezioni utili per la caption dall'outfit_desc
        context = outfit_desc[:1500] if outfit_desc else "high-fashion editorial scene"
        instr = (
            f"Based on this fashion editorial scene description, write a social media caption.\n\n"
            f"SCENE DESCRIPTION:\n{context}\n\n"
            "Rules:\n"
            "- Start with 4 or 5 emoji that match the style, setting, mood, colors and fashion\n"
            "- Follow with a short phrase of maximum 10 words\n"
            "- Focus ONLY on: location, outfit, colors, atmosphere, fashion, mood\n"
            "- Do NOT mention any person, nationality, ethnicity, age, gender or physical traits\n"
            "- Do NOT use flags, country adjectives or cultural references to people\n"
            "- No hashtags, no punctuation at the end\n"
            "- Return ONLY the caption on a single line, nothing else\n"
            "Example: '🌊✨👙🌅 golden hour on the mediterranean coast'\n"
            "Example: '🖤🌹💃🎭 dark glamour at the theatre'\n"
            "Example: '🌸🎋🍵✨ silk and stillness in the bamboo grove'"
        )
        response = client.models.generate_content(
            model="gemini-3-flash-preview",
            contents=[instr]
        ...
        logger.error(f"❌ Errore caption da prompt: {e}", exc_info=True)
        return None, f"❌ Errore: {html.escape(str(e))}"
...
def build_shooting_prompt(outfit_desc, mode="mosaic"):
    ...
    if mode == "single":
        prompt = (
            ...
            f"**NEGATIVE PROMPT:** studio background replacing original location, body hair, chest hair, missing glasses, missing beard, face drift, desaturated colors, color shift.\n"
        )
    else:
        prompt = (
            f"{EDITORIAL_WRAPPER}\n"
            ...
            f"wrong setting, body hair, chest hair, missing glasses, missing beard, face drift, desaturated colors, color shift.\n"
        )
    return prompt
...
def cmd_start(m):
    ...
    bot.send_message(m.chat.id, "<b>Atelier pronto.</b>")
    bot.send_message(m.chat.id, f"<b>ATELIER v{VERSION}</b>\nInvia in atelier. Scegli il filtro.", reply_markup=get_filter_keyboard())
        ...
        def retry_task():
            ...
            if img:
                sent = bot.send_document(cid, io.BytesIO(img),
                    visible_file_name="atelier_retry.jpg",
...
def _run_standard(cid, uid, username, pdata):
    ...
    bot.send_message(cid,
        f"✦ <b>ATELIER — generazione in corso...</b>\n"
    ...
    def run_task(idx):
        ...
        with lock:
            ...
            if completed[0] == count:
                ...
                flt_label = FILTERS[flt]["label"] if flt else "?"
                bot.send_message(cid,
                    f"✅ <b>Sessione completata</b> · <b>{flt_label}</b>\n\nCosa vuoi fare?",
                    reply_markup=get_loop_keyboard())
    ...
    def run_variant_tracked(v):
        ...
        with lock_dual:
            ...
            if completed_dual[0] == total_variants:
                ...
                flt_label = FILTERS[flt]["label"] if flt else "?"
                bot.send_message(cid,
                    f"✅ <b>Doppia variante completata</b> · <b>{flt_label}</b>\n\nCosa vuoi fare?",
                    reply_markup=get_loop_keyboard())
...
def handle_photo(m):
    ...
    if flt.get("shooting"):
        ...
        caption_text, cap_err = generate_caption_from_prompt(outfit_desc)
...
@app.route('/')
def health():
    return f"ATELIER v{VERSION} Online"
