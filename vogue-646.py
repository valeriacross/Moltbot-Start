...
def _extract_scene_for_caption(prompt_text):
    """Estrae la parte descrittiva del prompt saltando i blocchi tecnici."""
    import re
    keywords = [
        "MASTER PROMPT", "NEGATIVE PROMPT", "Rendering physics",
        "Watermark", "SynthID", "SYNTHETIC FACE IDENTITY LOCK",
        "Facial identity", "MANDATORY BODY", "Hair:", "TASK:",
        "STEPS:", "DNA:", "ENGINE STYLE"
    ]
    ...
    return text[:600] if text else prompt_text[1000:1600]

def generate_caption_from_image(img_bytes):
    """Genera caption social guardando l'immagine — focalizzata su outfit e location, non sul soggetto."""
    try:
        img_part = genai_types.Part.from_bytes(data=img_bytes, mime_type="image/jpeg")
        response = client.models.generate_content(
            model=MODEL_TEXT_ID,
            contents=[
                img_part,
                "Look at this fashion photo. Write a social media caption. "
                "RULES: "
                "- Describe ONLY the specific outfit items you see (fabric, color, style, details) and/or the location/setting. "
                "- Do NOT use generic words like 'dreamscape', 'elegance', 'style', 'fashion' alone. "
                "- Do NOT mention the person, their age, gender, face or body. "
                "- Include 4-6 emojis that match the actual colors and mood of the outfit. "
                "- 1 complete evocative sentence. Be specific: name the actual garments you see. "
...
def generate_caption_from_text(prompt_text):
    """Genera caption social dal testo del prompt, estraendo solo la parte descrittiva."""
    try:
        scene_extract = _extract_scene_for_caption(prompt_text)
        response = client.models.generate_content(
            model=MODEL_TEXT_ID,
            contents=[
                f"Based on this fashion photo description, write a social media caption. "
                f"1 complete sentence in English, 4-6 emojis, evocative fashion-forward tone, "
                f"mention the outfit or location specifically. "
...
def generate_caption(img_bytes):
    ...
    try:
        ...
        instr = (
            ...
            "Format: 3-5 contextual emoji + one short phrase of maximum 10 words. "
...
def handle_confirmation(call):
    ...
    # Caption generata subito dal prompt testuale — disponibile anche se la generazione fallisce
    caption_pre = generate_caption_from_text(data['full_p'])
    if caption_pre:
        bot.send_message(call.message.chat.id, f"{caption_pre}\n#genderfluid #selfexpression #bodypositivity")
...
def ask_confirmation(m):
    ...
    if m.content_type == 'photo':
        try:
            ...
            # CAPTION: generata subito dall'immagine originale, prima di qualsiasi altra elaborazione
            caption_immediata = generate_caption_from_image(img_data)
            if caption_immediata:
                bot.send_message(m.chat.id, f"{caption_immediata}\n#genderfluid #selfexpression #bodypositivity")
        except Exception as e:
        ...
        if not user_text or len(user_text.strip()) < 30:
            ...
            user_last_prompt[uid] = {'full_p': faceswap_prompt, 'img': None}
            # Caption dal prompt testuale — non dall'immagine
            markup = types.InlineKeyboardMarkup()
...
def handle_choice(call):
    ...
    if call.data == "choice_faceswap":
        ...
        def do_faceswap():
            ...
            pending_prompts[uid] = {'full_p': faceswap_prompt, 'qty': qty, 'img': None}
            user_last_prompt[uid] = {'full_p': faceswap_prompt, 'img': None}
