...
def ask_confirmation(m):
    ...
    if m.content_type == 'photo':
        ...
        # Se la caption è assente o molto corta → FACESWAP EDITORIALE
        if not user_text or len(user_text.strip()) < 30:
            logger.info(f"🔄 Faceswap editoriale per {username} (id={uid})")
            wait_msg = bot.reply_to(m, "🔄 <b>Faceswap editoriale in corso...</b>")
            faceswap_prompt = (
                ...
                "**TASK: EDITORIAL FACESWAP**\n"
                "Analyze the reference image carefully. Extract and preserve EXACTLY:\n"
                "- The full scene and background environment\n"
                "- The complete outfit, garments, accessories and styling\n"
                "- The lighting setup, color grade and mood\n"
                "- The general framing and composition\n\n"
                "Replace the original subject ENTIRELY with the Valeria Cross master identity "
                "(face, hair, beard, glasses from the provided reference face image).\n\n"
                "**POSE & EXPRESSION — EDITORIAL REINTERPRETATION:**\n"
                "Do NOT copy the pose mechanically. Reinterpret it editorially:\n"
                "- Keep the general body orientation and energy of the original\n"
                "- Introduce a subtle variation: shift of weight, slight turn of head, "
                "repositioned hand, adjusted angle\n"
                "- Expression: calm, self-aware, characteristic Valeria Cross half-smile — "
                "never blank, never forced\n"
                "The result should feel like a natural editorial shot, not a copy-paste.\n\n"
                "**TECHNICAL:** 8K, cinematic, photorealistic, unified lighting, "
                "perfect skin continuity face→neck→body. "
                "Watermark: 'feat. Valeria Cross 👠' — champagne cursive, bottom center, very small, 90% opacity.\n\n"
                "**NEGATIVE PROMPT:** face drift, wrong identity, female face, young face, missing beard, "
                "missing glasses, dark hair, stiff pose, mechanical copy, faceswap artifacts, "
                "mismatched skin tone, disconnected neck, body hair, chest hair."
            )
            settings = user_qty.get(uid, 1)
            qty = settings if isinstance(settings, int) else 1
            ...
            markup = types.InlineKeyboardMarkup()
            markup.row(
                types.InlineKeyboardButton("✅ Genera", callback_data="confirm_yes"),
                types.InlineKeyboardButton("❌ Annulla", callback_data="confirm_no")
            )
            bot.send_message(m.chat.id,
                f"🔄 <b>Faceswap editoriale</b>\n"
                f"📐 Formato: <b>{ar}</b> | 📸 Foto: <b>{qty}</b>\n\n"
                f"Scena, outfit e luce dall'immagine — posa e espressione reinterpretate.\n\n"
                f"Procedere?",
                reply_markup=markup)
            return
