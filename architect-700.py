...
def generate_monolith_prompt(user_input, engine):
    try:
        ...
        response = client.models.generate_content(model="gemini-3.1-flash-latest", contents=[instr])
        if response.text:
...
def review_and_fix(prompt, engine):
    ...
    try:
        ...
        response = client.models.generate_content(model="gemini-3.1-flash-latest", contents=[review_instr])
        if response.text:
