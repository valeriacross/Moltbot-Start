...
def _notify_key_rotation(new_key_num: int):
    if _active_cid:
        try:
            bot.send_message(_active_cid, f"🔑 <b>API Key {new_key_num}</b>", parse_mode="HTML")
        except Exception:
            pass

gemini.on_key_rotation(_notify_key_rotation)
...
# --- STATO UTENTE ---
user_settings = defaultdict(lambda: {'ratio': '2:3', 'count': 1, 'mode': 'single'})
...
def build_full_prompt(filter_key, outfit_description, ratio, scene_override=None, mode="single"):
...
def handle_callbacks(call):
    ...
    # --- LOOP (deprecato — flusso reale usa cancel_gen/go_home) ---
        ...
        user_filter.pop(uid, None)
        pending_prompts.pop(uid, None)
        bot.send_message(cid, "Foto singola o mosaico di 4?", reply_markup=get_mode_keyboard())
    ...
    def _process():
        ...
        if is_dual:
            ...
            for v in flt["variants"]:
                full_p = review_and_fix(build_full_prompt(filter_key, outfit_desc, settings['ratio'], scene_override=v["scene"], mode=mode), gemini)
            ...
            full_p = review_and_fix(build_full_prompt(filter_key, outfit_desc, settings['ratio'], mode=mode), gemini)
            pending_prompts[uid] = {
                'is_dual': False,
                'full_p': full_p,
