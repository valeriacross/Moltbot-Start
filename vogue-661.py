...
def caption_cmd(m):
    ...
    # Se c'è un'immagine già disponibile, usala direttamente
    last_img = user_last_image.get(uid)
    if last_img:
        bot.send_message(m.chat.id, "📝 <b>Genero caption dall'ultima immagine...</b>\n⏳ Attendi qualche secondo.")
        def run_caption_last():
            cap, err = generate_caption_from_image(last_img)
            if cap:
                bot.send_message(m.chat.id, f"📝 <b>Caption:</b>\n\n<code>{html.escape(cap)}</code>")
                logger.info(f"✅ Caption da last_image inviata a {username}")
            else:
                bot.send_message(m.chat.id, f"❌ Caption fallita: {err}")
        executor.submit(run_caption_last)
        return

    # Nessuna immagine disponibile — chiede nuova foto
...
def handle_caption_only(call):
    ...
    def run_caption():
        cap, err = generate_caption_from_image(data['img'])
        if cap:
            bot.send_message(call.message.chat.id,
                f"📝 <b>Caption:</b>\n\n<code>{html.escape(cap)}</code>")
...
def ask_confirmation(m):
    ...
    if m.content_type == 'photo':
        ...
        if not user_text or len(user_text.strip()) < 30:
            ...
            user_last_prompt[uid] = {'full_p': faceswap_prompt, 'img': img_data}
            markup = types.InlineKeyboardMarkup()
...
def handle_choice(call):
    ...
    if call.data == "choice_faceswap":
        ...
        def do_faceswap():
            ...
            pending_prompts[uid] = {'full_p': faceswap_prompt, 'qty': qty, 'img': img_data}
            user_last_prompt[uid] = {'full_p': faceswap_prompt, 'img': img_data}
