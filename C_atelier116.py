...
def describe_outfit_from_image(img_bytes):
    ...
    Passa i bytes originali senza conversione — identico a Vogue e Architect.
    """
    try:
...
def handle_photo(m):
    ...
    if pending_caption.pop(uid, False):
        ...
        if not caption_text:
            bot.send_message(cid, err or "❌ Caption fallita. Riprova.")
        else:
    ...
    def _process():
        ...
        if not outfit_desc:
            logger.error(f"❌ Analisi outfit fallita dopo 5 tentativi per {username} — nessun prompt generato")
            ...
            bot.send_message(cid,
                "❌ <b>Impossibile analizzare l'immagine dopo 5 tentativi.</b>\n\n"
                "Nessun prompt generato. Prova con un'altra foto.",
