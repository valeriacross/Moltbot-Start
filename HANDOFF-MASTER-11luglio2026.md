...
**Generato:** 11/07/2026, fine sessione (aggiornato da versione 10/07)
**Da:** Walter Caponi
...
## 2. STATO FILE ATTUALE (11/07/2026)
...
| `Architect_301.py` | **3.0.1** | homely-annabelle/thearchitect | `python Architect_301.py` | 1 |
...
**Nota versioning (corretta il 10/07):** il numero nel nome file **corrisponde sempre esattamente** alla VERSION interna, senza punti (es. `2.0.4` → `Atelier_204.py`, `3.0.1` → `Architect_301.py`) — non è un contatore progressivo indipendente. Su un salto di versione non incrementale (es. `2.0.6` → `3.0.0`), il nome file deve riflettere il nuovo numero di versione per intero, non "il prossimo della sequenza". Vedi sezione 2duodecies per il dettaglio dell'errore fatto e corretto in questa data.

**Nota chiavi (25/06):** una chiave spostata da Architect (2→1) ad Atelier (4→5). `C_shared100.py` ora legge fino a `GOOGLE_API_KEY_5`.

---

## 2terdecies. SESSIONE 11/07/2026 — ARCHITECT 3.0.1, DA JSON A PROMPT TESTUALE
...
## 6. ARCHITECT — STORICO E STATO (v3.0.1)

- **v3.0.1 (11/07):** Walter ha giudicato l'output JSON della 3.0.0 "ingestibile" e "non pubblicabile". Cambiato l'output da JSON a testo in formato prompt — sezioni etichettate in chiaro (`SUBJECT:`, `OUTFIT:`, `ACCESSORIES:`, `BODY ART:`, `PROPS & ACTIONS:`, `BACKGROUND:`, `LIGHTING:`, `CAMERA:`, `COLOR PALETTE:`, `MOOD:`), file `.txt` invece di `.json`. Rimossa tutta la logica di parsing/validazione JSON (non più necessaria per testo libero). `/lastjson` → di nuovo `/lastprompt`. Vedi sezione 2terdecies. **Non ancora testato in produzione.**
...
**Flusso attuale (v3.0.1):** utente invia una foto → Architect analizza con `analyze_image_full()` → restituisce un file `.txt` scaricabile, in formato prompt a sezioni etichettate, con la scena completa (soggetto reale, outfit, accessori, body art, sfondo, luce, palette, mood) — nessun DNA, nessuna sostituzione identitaria, pronto da copiare e usare. Nessuna modalità da scegliere, nessun testo da inviare. `/lastprompt` reinvia l'ultimo file senza rianalizzare.

**Comandi (v3.0.1):** `/start` · `/help` · `/info` · `/lastprompt` · `/shared`
...
12. **TODO aperto (10/07) — SUPERATO l'11/07 dal cambio di formato:** il punto sul JSON di Architect 3.0.0 non è più rilevante — l'output non è più JSON. Vedi punto 13.
13. **TODO aperto (11/07) — priorità alta, da fare prima di ogni altra cosa su Architect:** `Architect_301.py` (formato prompt testuale, sezione 2terdecies) non è mai stato testato in produzione. Verificare su Koyeb: (a) foto normale → file `.txt` con le 10 sezioni etichettate, testo scorrevole e incollabile in Flow; (b) foto di Valeria → conferma che SUBJECT descriva correttamente barba/occhiali/corpo senza intervento di DNA; (c) `/lastprompt` dopo una foto → reinvia lo stesso file; (d) verificare a occhio che Gemini rispetti il formato richiesto — senza più un parser JSON che lo validi, una deviazione dal formato (es. markdown residuo, sezioni mancanti) passerebbe inosservata finché qualcuno non legge il file.
