...
def build_flow_prompt(scenario, uid=None):
    ...
    import re as _re
    loc_raw = (photo_location.get(uid) or scenario.get('location', '')) if uid else scenario.get('location', '')
    if '{' in loc_raw and '"subject"' in loc_raw:
        values = _re.findall(r'"(?:subject|ground|sky|colors|lighting|depth|mood)"\s*:\s*"([^"]+)"', loc_raw)
        loc = ", ".join(v.strip() for v in values if v.strip()) if values else loc_raw[:120]
    else:
        loc = loc_raw
