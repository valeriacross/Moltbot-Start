...
from C_shared100 import VALERIA_DNA, EDITORIAL_WRAPPER, build_valeria_identity, generate_caption, generate_mini_caption, generate_mini_prompt, review_and_fix
...
def get_confirm_keyboard():
    """Keyboard post-prompt: navigazione + mini caption + mini prompt."""
    markup = InlineKeyboardMarkup()
    markup.row(
    ...
    markup.row(
        InlineKeyboardButton("📝 Mini caption", callback_data="atelier_minicaption"),
        InlineKeyboardButton("📋 Mini prompt",  callback_data="atelier_miniprompt"),
    )
    return markup
    ...
    def _process():
        ...
        if is_dual:
            ...
            try:
                ...
                logger.warning(f"⚠️ caption post-prompt: {_cap_err}")
...
def _get_atelier_prompt_text(uid):
    """Estrae il testo del prompt da last_prompt[uid] — gestisce sia full_p che variants."""
    prompt_data = last_prompt.get(uid)
    if not prompt_data:
        return None
    if isinstance(prompt_data, dict):
        if prompt_data.get('full_p'):
            return prompt_data['full_p']
        if prompt_data.get('variants'):
            # Mosaico: concatena le varianti
            return "\n\n---\n\n".join(prompt_data['variants'])
    return str(prompt_data)


@bot.callback_query_handler(func=lambda c: c.data == "atelier_minicaption")
def cb_atelier_minicaption(call):
    uid = call.from_user.id
    if not is_allowed(uid): return
    try: bot.answer_callback_query(call.id)
    except Exception: pass
    prompt_text = _get_atelier_prompt_text(uid)
    if not prompt_text:
        bot.send_message(call.message.chat.id, "⚠️ Nessun prompt disponibile.")
        return
    wait = bot.send_message(call.message.chat.id, "📝 <b>Genero la mini caption...</b>")
    cap, err = generate_mini_caption(prompt_text, gemini)
    try: bot.delete_message(call.message.chat.id, wait.message_id)
    except Exception: pass
    bot.send_message(call.message.chat.id, cap if cap else err or "❌ Errore.", parse_mode="HTML")


@bot.callback_query_handler(func=lambda c: c.data == "atelier_miniprompt")
def cb_atelier_miniprompt(call):
    uid = call.from_user.id
    if not is_allowed(uid): return
    try: bot.answer_callback_query(call.id)
    except Exception: pass
    ...
    if not prompt_text:
        bot.send_message(call.message.chat.id, "⚠️ Nessun prompt disponibile.")
        return
    wait = bot.send_message(call.message.chat.id, "📋 <b>Genero il mini prompt...</b>")
    mini, err = generate_mini_prompt(prompt_text, gemini)
    try: bot.delete_message(call.message.chat.id, wait.message_id)
    except Exception: pass
    bot.send_message(call.message.chat.id, mini if mini else err or "❌ Errore.", parse_mode="HTML")
