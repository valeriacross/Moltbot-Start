...
# CHANGELOG 2.0.9 (15/07/2026): fix generale, non un altro patch puntuale.
...
# mosaic). Riscritti in positivo puro: COLOR LOCK, OUTFIT DETAIL LOCK,
# HAIR LOCK (tolta la coda "Do NOT generate..."), BACKGROUND LOCK, la
# clausola outfit-assente ("do NOT invent garments" → descrizione diretta
# di cosa succede invece). Aggiunto un nuovo blocco locale "⚠️ IDENTITY
# LOCK" (barba/occhiali/corpo, positivo) in tutte e 3 le funzioni — prima
# quella parte viveva SOLO nella lista negative locale, senza rinforzo
# positivo locale equivalente a HAIR LOCK. Non toccato: VALERIA_FACE nello
# shared (righe 198-241) ha ancora frasi negative tipo "DO NOT shave it" —
# stesso principio si applicherebbe, ma è shared, usato anche da Vogue,
# fuori scope per questa sessione. Non elimina la varianza — Flow resta
# non deterministico — ma allinea l'approccio a quanto Google stessa dice
# funzionare meglio su questo modello specifico.
VERSION = "2.0.9"
