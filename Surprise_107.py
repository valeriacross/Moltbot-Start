...
def _send_dual_output(cid, uid, scenario, caption_text):
    ...
    1. Caption (una sola)
    2. Mini prompt Scatto singolo (dal dict scenario — valori reali)
       [Prompt Flow Scatto singolo]
    3. Mini prompt Mosaico 4 foto (dal dict scenario — valori reali)
       [Prompt Flow Mosaico] + 🎲 Nuova scena
    """
    # 1. Caption
    if caption_text:
    ...
    # 2. Scatto singolo
    mini_single = _mini_prompt_from_scenario(scenario, "📷 <b>Scatto singolo</b>")
    bot.send_message(cid, mini_single, parse_mode="HTML")
    ...
    # 3. Mosaico 4 foto
    mini_mosaic = _mini_prompt_from_scenario(scenario, "🗂️ <b>Mosaico 4 foto</b>")
    bot.send_message(cid, mini_mosaic, parse_mode="HTML")
...
def _mini_prompt_from_scenario(scenario, tipo):
    ...
    loc_short = loc.split(',')[0].strip()[:80] if len(loc) > 80 else loc

    return (
        f"{tipo}\n"
        f"📍 <b>Location:</b> {html.escape(loc_short)}\n"
        f"🌤 <b>Sky:</b> {html.escape(_short(scenario.get('sky', ''), 70))}\n"
        f"👗 <b>Outfit:</b> {html.escape(_short(scenario.get('outfit', ''), 90))}\n"
        f"🎨 <b>Style:</b> {html.escape(_short(scenario.get('style', ''), 70))}\n"
        f"💃 <b>Pose:</b> {html.escape(_short(scenario.get('pose', ''), 70))}\n"
        f"✨ <b>Mood:</b> {html.escape(_short(scenario.get('mood', ''), 60))}\n"
        f"🏛 <b>Body:</b> Feminine hourglass, 180cm 85kg, D-cup bust, smooth skin"
    )
