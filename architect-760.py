...
BODY: Soft feminine proportions, hourglass silhouette, full D-cup bust (MANDATORY — always visible regardless of pose), 180cm, 85kg. Completely hairless body (NO body hair, NO chest hair, NO arm hair, NO leg hair, NO peli — zero body hair on ALL surfaces).
...
ENGINE_PROFILES = {
    ...
    "Gemini": {
        ...
        "replacements": {
            ...
            "Cup D": "full D-cup bust clearly visible", "hairless": "completely hairless body (NO body hair, NO chest hair, NO arm hair, NO leg hair — zero body hair on ALL surfaces)",
    ...
    try:
        ...
        if engine == "Gemini":
            instr = (
                ...
                f"Then write a professional MASTER PROMPT optimized for Gemini image generation, replacing the subject with Valeria Cross DNA.\n\n"
                f"ENGINE STYLE RULES (mandatory):\n{engine_style}\n\n"
                f"{vs_context}"
                f"STEPS:\n"
                f"1. Open with the mandatory SYNTHETIC FACE IDENTITY LOCK header as specified.\n"
                f"2. Write a structured prompt using labeled sections: Scene, Subject body, Outfit, "
                f"Facial identity, Hair, Lighting and camera, Rendering physics.\n"
                f"3. Extract ALL visual elements from the image (scene, outfit, light, pose, environment). "
                f"Replace the original subject entirely with Valeria Cross DNA expanded inline. "
                f"Do NOT use the name 'Valeria Cross' anywhere in the output.\n"
...
def review_and_fix(prompt, engine):
    ...
    try:
        review_instr = (
            ...
            f"   - 'full D-cup bust clearly visible'\n"
            ...
            f"'body hair, chest hair, arm hair, leg hair, hairy torso, hairy arms, flat chest, masculine torso'.\n\n"
            f"PROMPT TO REVIEW:\n\n{prompt}"
        )
