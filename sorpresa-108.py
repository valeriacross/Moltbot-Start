...
def estrai_combinazione():
    ...
    stile/stile_artistico e filtro_fx sono mutualmente esclusivi a cascata."""
    combo = {k: random.choice(v) for k, v in VARIABILI.items()}
    # Se esce filtro FX attivo → azzera stile_artistico
    if combo['filtro_fx'] != 'none — pure photorealistic render':
        combo['stile_artistico'] = 'none — photorealistic'
    # Se esce stile_artistico attivo → azzera stile fashion
