...
def _extract_scene_for_caption(prompt_text):
    """Estrae la parte descrittiva del prompt (scena, outfit, mood) saltando i blocchi tecnici."""
    import re
    # Rimuove blocchi tecnici comuni
    blocchi_da_rimuovere = [
        r'MASTER PROMPT.*?(?=
...
|\Z)',
        r'SYNTHETIC FACE IDENTITY LOCK.*?(?=
...
|\Z)',
    ...
    text = prompt_text
    for pattern in blocchi_da_rimuovere:
        text = re.sub(pattern, '', text, flags=re.DOTALL | re.IGNORECASE)
    # Prende le prime 600 chars della parte rimasta
    text = re.sub(r'\n{3,}', '\n\n', text).strip()
    return text[:600] if text else prompt_text[:600]
...
def ask_confirmation(m):
    ...
    if m.content_type == 'photo':
        ...
        if not user_text or len(user_text.strip()) < 30:
            ...
            def _finalize_batch(uid=uid, cid=m.chat.id, username=username):
                ...
                for pos, (i, full_p) in enumerate(ok):
                    caption_pre = generate_caption_from_text(full_p)
                    if caption_pre:
        ...
        if not user_text or len(user_text.strip()) < 30:
            ...
            caption_fs = generate_caption_from_text(faceswap_prompt)
            markup = types.InlineKeyboardMarkup()
