import os, html, logging, threading
import telebot
...
# CHANGELOG 3.1.0 (01/08/2026): supporto foto di riferimento con 2+ soggetti
# distinti (m+f, m+m, f+f...) su richiesta di Walter, stesso giorno del fix
# analogo su Vogue/Atelier (shared 2.4.9) — ma qui senza nessun DNA da
# iniettare, quindi nessun equivalente di MULTI-SUBJECT LOCK: SUBJECT ora
# descrive ogni persona reale separatamente (label breve consistente,
# riutilizzata in OUTFIT/ACCESSORIES/BODY ART) invece di assumere una sola
# persona in frame — coerente con lo scopo di Architect (descrizione fedele
# del soggetto reale, nessuna sostituzione identitaria). Non ancora testato
# in produzione. Bump di MINOR (3.0.2 → 3.1.0), non patch: nuova capacità,
# non un fix — file chiamato Architect_310.py fin da subito.
VERSION = "3.1.0"
...
last_prompt = {}   # uid → testo — ultimo prompt generato, per /lastprompt

# --- PROMPT DI ANALISI — TESTO A SEZIONI, NESSUN DNA ---
# CHANGELOG 3.0.2 (12/07/2026): dato che il prompt testuale è sufficientemente
# breve (a differenza del JSON strutturato della 3.0.0, molto più verboso),
# Walter ha chiesto di consegnarlo come testo diretto in chat invece che come
# file — coerente con Vogue/Atelier/Filtro/Surprise, che fanno tutti così.
# Aggiunta send_prompt_text(): stesso pattern di chunking a 3800 caratteri già
# usato in Vogue, nessun bottone post-prompt (non pertinente qui). Rimossi
# bot.send_document(), io.BytesIO, il nome file generato per ogni foto, e
# l'import `io` (non più usato). last_prompt ora memorizza il testo puro
# invece della tupla (testo, filename) — semplificato dato che non serve più
# un nome file. /lastprompt ora reinvia il testo in chat, non un file.
# CHANGELOG 3.0.1 (12/07/2026): Walter ha segnalato che l'output JSON della
...
# che qui non c'è nessun DNA da iniettare al posto suo). Consegnato ALLORA
# come file .txt (poi cambiato in 3.0.2, vedi sopra — a quel punto era ancora
# file invece di .json). Rimossa tutta la logica di parsing/validazione
...
ANALYSIS_PROMPT = (
    ...
    "omitting the label:\n\n"
    "SUBJECT: [How many distinct people are in the frame. If exactly one: full literal "
    "description — apparent gender presentation, apparent age range, face shape, skin, "
    "facial hair, eyes, eyebrows, expression, hair color/length/style/texture, glasses or "
    "eyewear, body build and proportions, pose, how the subject is framed by the camera. "
    "If two or more people: give each one a short consistent label (e.g. 'left person', "
    "'right person', 'kneeling person') and describe each one's full literal appearance "
    "separately using the same categories, plus each one's position in the frame and how "
    "they physically interact with each other — literal and specific. Reuse the same "
    "labels consistently in OUTFIT/ACCESSORIES/BODY ART below. No identity substitution, "
    "no fictional character overlay, no DNA injection of any kind for anyone in frame — "
    "describe every person exactly as literally seen.]\n\n"
    ...
    "If SUBJECT describes two or more people: within OUTFIT, ACCESSORIES and BODY ART, "
    "describe each person's garments/accessories/markings SEPARATELY, labeled with the "
    "same short label used in SUBJECT (e.g. 'Left person: ... Right person: ...') — never "
    "merge different people's garments into one generic description. If SUBJECT describes "
    "only one person, describe these sections exactly as before, with no labeling.\n\n"
...
def start(m):
    ...
    bot.send_message(m.chat.id,
        ...
        f"Inviami una foto: ti restituisco il prompt completo della scena in chat "
...
def cmd_info(m):
    bot.send_message(m.chat.id,
        ...
        f"Output: testo in chat, pronto all'uso — nessun file da scaricare.\n"
    )

def send_prompt_text(chat_id: int, text: str):
    """Invia il prompt come testo in chat, a pezzi da 3800 caratteri se necessario —
    stesso pattern usato da Vogue/Atelier/Filtro/Surprise. Nessun file allegato."""
    CHUNK = 3800
...
def handle_photo(m):
    ...
    def task():
        try:
            ...
            bot.send_message(m.chat.id,
                "✅ <b>Prompt completo</b> — nessun DNA Valeria, descrizione fedele dell'immagine:")
            send_prompt_text(m.chat.id, text)
