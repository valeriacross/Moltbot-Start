...
**Generato:** 13/07/2026, fine sessione (aggiornato da versione 12/07)
**Da:** Walter Caponi
...
## 2. STATO FILE ATTUALE (13/07/2026)
...
| `Atelier_206.py` | **2.0.6** | flexible-denna/atelier | `python Atelier_206.py` | 5 |
...
## 7. ATELIER — STORICO E STATO (v2.0.6)
...
7. **TODO aperto (04/07), ridotto il 10/07 — RISOLTO il 13/07 (sezione 2sedecies).** Vogue/Atelier/Surprise allineati a `gemini-3.5-flash`. **Correzione all'assunzione originale:** Filtro non aveva la stringa da correggere — `MODEL_ID`/`MODEL_TEXT_ID` erano dead code, mai mostrate in `/info`; rimosse invece di modificate. Non ancora testato su Koyeb — vedi punto 17.
...
11. **TODO aperto (08/07) — test da fare per Vogue e Atelier, NON per Architect (superato):** nessuno dei 2 file toccati per la clausola BODY ART (ora `Vogue_202.py`, `Atelier_206.py` — stesso codice della clausola, solo rinominati dai bump successivi del 13/07) è stato provato su Koyeb. Da verificare: (a) foto senza tatuaggi → prompt più corto, clausola assente; (b) foto con tatuaggi (es. quella del 07/07) → clausola presente, fedeltà come già vista su Atelier 203.
...
16. **TODO aperto (12/07):** analisi location a sezioni (ex 50 parole), ora in `Surprise_202.py` (stesso codice, rinominato dal bump del 13/07), non ancora testata in produzione. Verificare su Koyeb: (a) foto location → le 4 sezioni (BACKGROUND/LIGHTING/CAMERA/MOOD) arrivano complete in chat, non più il paragrafo generico; (b) messaggio di conferma senza troncamento si comporta bene con testi lunghi; (c) foto PNG/WebP (non solo JPEG) → confermare che il fix `detect_mime_type()` funzioni. Vedi sezione 2quindecies.
17. **TODO aperto (13/07) — priorità media:** allineamento `/info` al modello reale su Vogue/Atelier/Surprise (`Vogue_202.py`, `Atelier_206.py`, `Surprise_202.py`) e rimozione dead code in Filtro (`Filtro_201.py`) non ancora testati in produzione. Verificare su Koyeb: (a) `/info` sui 4 bot mostra `gemini-3.5-flash`; (b) `/info` di Surprise mostra "254 location" invece di "200"; (c) nessuna regressione — sono modifiche a sole stringhe/costanti morte, nessuna logica toccata, ma non verificato a runtime. Vedi sezione 2sedecies.
19. **TODO aperto (13/07):** ampliamento liste pose/framing/expression/angle nel mosaico Atelier (`Atelier_206.py`) non ancora testato in produzione. Walter deve verificare su Koyeb con generazioni reali su Flow che i 4 scatti risultino effettivamente più differenziati tra loro rispetto a prima (era il problema segnalato). Vedi sezione 7 per il dettaglio della modifica e dell'errore di lettura corretto in questa sessione.
18. **TODO aperto (13/07) — decisione da prendere, non urgente ma non ignorabile:** 5 voci di `LOCATION_POOL` in `Surprise_202.py` referenziano IP Disney esplicitamente (Frozen ×2, Bella e la Bestia, Fantasia, Re Leone). Rischio: blocco chiave Google sui termini d'uso, o claim di copyright se le immagini generate vengono pubblicate. Walter deve decidere: (a) rimuovere le 5 voci dal pool, (b) riscriverle senza nomi di proprietà Disney (stessa estetica, IP genericizzata), (c) lasciarle, rischio accettato consapevolmente. Vedi sezione 2sedecies per il dettaglio.
