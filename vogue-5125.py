...
def execute_generation(full_prompt, img_rif_bytes=None):
    try:
        ...
        if len(full_prompt) > 1800:
            truncated = full_prompt[:1800]
            last_nl = truncated.rfind('\n')
            full_prompt = truncated[:last_nl] if last_nl > 1260 else truncated
            logger.info(f"   ✂️ Prompt troncato a {len(full_prompt)} chars per Gemini image")
