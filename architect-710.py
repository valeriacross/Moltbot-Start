...
def execute_generation(cid, text, engine, img_bytes=None):
    ...
    if img_bytes:
        vs_json = analyze_image_visionstruct(img_bytes)
        if vs_json:
            logger.info("   ✅ VisionStruct JSON disponibile — inject nel prompt")
        else:
            logger.info("   ⚠️ VisionStruct non disponibile — generazione standard")
        raw = generate_from_image(img_bytes, text, engine, vs_json=vs_json)
    else:
...
def build_prompt_from_image(m):
    ...
    wait = bot.send_message(cid,
        ...
        f"⏳ Step 1: VisionStruct scan → Step 2: Prompt generation.")
    def task():
