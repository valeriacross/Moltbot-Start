...
VARIABILI = {
    ...
    "outfit_intero": [
        ...
        "polka dot brown one-piece swimsuit, bandeau with gold ring detail",
...
STILE_ARTISTICO_OVERRIDES = {
    ...
    "🏛️ De Chirico — metaphysical painting, long dramatic shadows, empty piazzas, classical architecture, eerie stillness": {
        ...
        "punto_di_ripresa_ok": [
            "wide angle 24mm full body", "full body wide shot",
            "85mm portrait, eye level", "low angle looking up",
            "mid-shot waist up", "environmental portrait, subject in context",
        ],
    },
    "🔷 Mondrian — primary color geometric grid, De Stijl abstraction, flat bold rectangles": {
        "colore_ok": [
    ...
    "🖌️ Banksy — urban stencil street art, black and white spray paint, sharp political irony, graffiti aesthetic": {
        "colore_ok": [
            ...
            "bold red and gold",  # rosso come unico accento tipico di Banksy
        ],
        "pattern": "none — solid color",
        "sfondo_ok": [
...
def estrai_combinazione():
    ...
    # 12. Outfit intero estratto → azzera top, bottom e pattern
    if not combo['outfit_intero'].startswith('none'):
        combo['outfit_top'] = 'none'
        combo['outfit_bottom'] = 'none'
        combo['pattern'] = 'none — solid color'
...
def build_prompt(combo):
    ...
    has_intero = not combo.get('outfit_intero', 'none').startswith('none')
    outfit_str = (
        f"**Outfit:** {combo['outfit_intero']}, {combo['scarpe']}.\n"
        if has_intero else
        f"**Outfit:** {combo['outfit_top']} with {combo['outfit_bottom']}, {combo['scarpe']}.\n"
    )
    scene = (
        f"**BODY (MANDATORY):** Feminine hourglass proportions, 180cm, 85kg. FULL D-CUP BUST — prominent, natural, clearly visible. Wide rounded hips, defined waist, full thighs. THIS BODY IS FEMININE. DO NOT GENERATE A MALE PHYSIQUE. Completely hairless body. Smooth porcelain skin.\n"
...
def build_threads_caption(combo):
    ...
    # Fallback: colore → prima parola; posa → prima parola; outfit
    has_intero = not combo.get('outfit_intero', 'none').startswith('none')
    outfit_word = combo['outfit_intero'].split()[0].lower() if has_intero else combo['outfit_top'].split()[0].lower()
    extras = [
...
def format_combinazione(combo):
    ...
    for k, label in VARIABILI_LABELS.items():
        if has_intero and k in ('outfit_top', 'outfit_bottom'):
            continue  # nasconde top/bottom quando c'è intero
        if not has_intero and k == 'outfit_intero':
            continue  # nasconde intero quando non usato
        lines.append(f"{label}: <b>{combo[k]}</b>")
    fmt = combo.get('_formato', '2:3')
    lines.append(f"📐 Formato: <b>{fmt}</b>")
    lines.append(f"🏛 Body: <b>Feminine hourglass, 180cm 85kg, D-cup bust, smooth skin</b>")
    return "\n".join(lines)
...
def handle_callback(call):
    ...
    if call.data == "tira":
        ...
        if not combo:
            ...
            return
        combo_text = format_combinazione(combo)
        # Caption e prompt generico generati subito — disponibili anche se la generazione fallisce
        full_p_pre = build_prompt(combo)
        fmt_pre = combo.get('_formato', '2:3')
        last_prompt[uid] = full_p_pre
        generic_pre = build_generic_prompt(combo)
        threads_pre = build_threads_caption(combo)
        bot.send_message(cid, f"🧵 <b>Caption Threads:</b>\n<code>{html.escape(threads_pre)}</code>")
        bot.send_message(cid, f"📋 <b>Prompt generico:</b>\n\n<code>{html.escape(generic_pre)}</code>")
            ...
            if img:
                try:
                    bot.send_document(cid, io.BytesIO(img),
                        ...
                        caption=f"✅ {elapsed}s | {fmt_pre}",
                        reply_markup=get_retry_keyboard())
                    logger.info(f"✅ {username} (id={uid}) — generata in {elapsed}s | formato={fmt_pre}")
                except Exception as e:
        ...
        if not combo:
            bot.send_message(cid, "⚠️ Sessione scaduta. Tira di nuovo.", reply_markup=get_main_keyboard())
            return
        full_p_retry = build_prompt(combo)
        fmt_retry = combo.get('_formato', '2:3')
        last_prompt[uid] = full_p_retry
        generic_retry = build_generic_prompt(combo)
        threads_retry = build_threads_caption(combo)
        bot.send_message(cid, f"🧵 <b>Caption Threads:</b>\n<code>{html.escape(threads_retry)}</code>")
        bot.send_message(cid, f"📋 <b>Prompt generico:</b>\n\n<code>{html.escape(generic_retry)}</code>")
            ...
            if img:
                try:
                    bot.send_document(cid, io.BytesIO(img),
                        ...
                        reply_markup=get_retry_keyboard())
                    logger.info(f"✅ {username} (id={uid}) — retry in {elapsed}s | formato={fmt_retry}")
                except Exception as e:
