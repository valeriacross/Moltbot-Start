...
BODY — ABSOLUTE LOCK: The body is ALWAYS female. Soft feminine curves, hourglass waist, wide hips, full D-cup breasts always present and visible when appropriate. NO masculine body, NO flat chest, NO male torso under any circumstance. This is non-negotiable regardless of any other instruction.
...
def ask_confirmation(m):
    ...
    # --- REPLY CON MODIFICA: se l'utente risponde a un messaggio del bot ---
    if (m.reply_to_message and
        m.reply_to_message.from_user.id == bot.get_me().id and
        m.content_type == 'text' and
        user_last_prompt.get(uid)):

        modify_text = m.text.strip()
        last_data = user_last_prompt[uid]
        original_prompt = last_data['full_p']

        logger.info(f"✏️ {username} (id={uid}) → modifica in reply: {modify_text[:80]}")
        wait_msg = bot.reply_to(m, "✏️ <b>Applico la modifica...</b>")

        # Costruisce prompt modificato: prompt originale + istruzione di override
        modified_prompt = (
            f"{original_prompt}\n\n"
            f"OVERRIDE INSTRUCTION (apply this change to the above, keep everything else identical): "
            f"{modify_text}"
        )

        def run_refine():
            t_start = time.time()
            res, err = execute_generation(modified_prompt, last_data.get('img'))
            elapsed = round(time.time() - t_start, 1)
            try:
            ...
            if res:
                ...
                # Aggiorna last_prompt con la versione modificata
                user_last_prompt[uid] = {**last_data, 'full_p': modified_prompt}
                caption = generate_caption(res)
                if caption:
                ...
                bot.send_message(m.chat.id,
                    ...
                    reply_markup=retry_markup)
                logger.warning(f"   ❌ Refine fallito per {username}: {err}")
