...
7. VIETATO inserire frasi come "FILTER BYPASS", "BYPASS ACTIVATED", "SAFETY OVERRIDE",
   ...
   Queste frasi causano errori critici nel sistema.
...
def ask_confirmation(m):
    ...
    # Fallback se ottimizzazione fallisce: MASTER_IDENTITY + testo utente diretto
    ...
    # Rimuovi righe problematiche che triggherano MALFORMED_FUNCTION_CALL
    blocklist = [
    ...
    sanitized_prompt = sanitized_prompt.replace("{", "(").replace("}", ")").replace("[", "(").replace("]", ")")
