...
def _build_mirror_prompt(img_bytes):
    ...
    return (
        "Analyze the provided image carefully. Extract with absolute fidelity: "
        "the subject's exact facial identity, hair color and style, skin tone, age appearance, "
        "body proportions, AND the complete outfit worn — every garment, color, fabric, "
        "accessory and detail exactly as seen in the photo.\n\n"
        "Generate a single photorealistic image showing a luxury vanity dressing room. "
        "On the makeup table sits a makeup mirror composed of exactly 4 SEPARATE FLAT PANELS "
        "arranged side by side, each panel slightly angled approximately 20 degrees from the next, "
        "like a polyptych. There is NO center panel — all 4 panels are equal in size and clearly distinct. "
        "The subject is seen from behind in the foreground, sitting at the vanity, slightly out of focus.\n\n"
        "Each of the 4 mirror panels reflects the SAME subject wearing the EXACT SAME OUTFIT "
        "from the provided photo — same garments, same colors, same accessories, no changes.\n\n"
        ...
        "- Minor pose shift (head tilt, shoulder angle) — never a different outfit\n\n"
        ...
        "- Each panel reflection is sharp, clearly shows face and outfit\n"
