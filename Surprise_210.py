...
# {VALERIA_NEGATIVE}" (una dentro _build_base_prompt(), di fatto già dead
# code: il suo valore di ritorno stringa non era mai usato da nessuno dei
# chiamanti, verificato — le altre 2 nei return diretti di
# build_flow_prompt_single/mosaic, quelle sì usate), 2 blocchi "**NEGATIVE
# PROMPT:**" (single e mosaic), il blocco "--- NEGATIVE ---" della funzione
# gruppo/pride (Walter, Carlotta, Fufos, Fritz — incluse le frasi negative
# in WALTER_DNA/CARLOTTA_DNA, "NO long hair"/"never dark brown or blonde"),
# il "NEGATIVE:" della funzione di tiling/mosaico testo, e il blocco "BODY
# — DO NOT OVERRIDE" nella funzione Pride a 6 pannelli. Tolto anche
# l'import morto di VALERIA_NEGATIVE (eliminata da shared 2.4.0) e la riga
# `base, *_ = _build_base_prompt(scenario)` in build_flow_prompt_single()
# — dead code, il valore non veniva mai letto. Causa: verificato che il
# modello dietro Flow non ha un campo negativePrompt indipendente — fix
# generale esteso a tutto l'impianto DNA condiviso. Vedi HANDOFF sezione
# 2septendecies.
# CHANGELOG 2.0.2 (13/07/2026): due fix a /info (handle_info). 1) Stringa
# modello allineata al motore reale — "gemini-3-flash-preview" →
# "gemini-3.5-flash" (era rimasta disallineata dal passaggio fatto in
# shared 2.3.16, 04/07). 2) Conteggio location corretto da "200" (valore
# statico mai aggiornato) al conteggio reale di LOCATION_POOL, contato
# via len() a import-time invece di scritto a mano — evita che il numero
# torni a disallinearsi silenziosamente se il pool cambia in futuro
# (LOCATION_POOL conta 254 elementi a oggi, non 200; gli altri pool
# mostrati in /info — outfit/stili/pose/sky/mood — erano già corretti,
# verificato con len() su ciascuno prima di toccare la riga). Nessun'altra
# modifica.
# CHANGELOG 2.0.1 (12/07/2026): analisi location da foto (handle_location_photo)
# era limitata a "un paragrafo conciso di massimo 50 parole" — troppo corta e
# poco dettagliata secondo Walter, che vuole lo stesso livello di analisi di
# Architect_302.py. Riscritto location_prompt in sezioni etichettate
# (BACKGROUND/LIGHTING/CAMERA/MOOD, stessa struttura di ANALYSIS_PROMPT in
# Architect ma senza SUBJECT/OUTFIT/ACCESSORIES/BODY ART — esclusi di
# proposito, coerente con lo scopo di questa funzione: qui serve solo la
# location, l'outfit lo genera Surprise stesso altrove). Aumentato max_tokens
# a 2048 per lasciare spazio alla risposta più lunga. Anche il messaggio di
# conferma troncava a 200 caratteri (location_text[:200]) — ora mostra il
# testo completo, a pezzi da 3800 caratteri se necessario, stesso pattern di
# Vogue/Architect. In più, mime_type era hardcoded a "image/jpeg" — sostituito
# con detect_mime_type() per supportare correttamente anche PNG/WebP, coerente
# con gli altri bot.
...
def build_flow_prompt_single(scenario):
    ...
    shooting_block = (
        ...
        "**⚠️ CRITICAL — BACKGROUND LOCK:** The exact location described below appears precisely, "
        "as the setting of the whole scene.\n\n"
    )
...
def build_flow_prompt_mosaic(scenario):
    ...
    shooting_block = (
        ...
        "Generate exactly 4 editorial photographs of the same subject in the same scene. Each of the "
        "4 shots is a genuinely distinct combination of pose, framing, expression and angle — no two "
        "shots repeat the same combination:\n"
        "- Pose options: standing, seated, reclining, dynamic movement, leaning, walking\n"
        "- Framing options: full body, three-quarter, close-up bust, wide angle\n"
        "- Expression options: confident, dreamy, fierce, playful\n"
        "- Angle options: frontal, profile, three-quarter, back view\n\n"
        "What stays identical across all 4 shots:\n"
        "- Subject identity (face, hair, beard, glasses — ZERO drift)\n"
        "- Location and background setting\n"
        "- Lighting mood and color palette\n"
        "- Outfit (same garments, same colors, same details)\n\n"
        "**⚠️ CRITICAL — BACKGROUND LOCK:** The exact location described below appears precisely, "
        "identically in ALL 4 shots, as the setting of the whole scene.\n\n"
    )
    (_, loc, outfit, sky, pose, mood, style) = _build_base_prompt(scenario)
    return (
        f"{VALERIA_FACE}{VALERIA_BODY_STRONG}"
...
def handle_location_photo(m):
    ...
    # Prompt dedicato: estrae solo location/ambiente, non outfit — stesso
    # livello di dettaglio e stessa struttura a sezioni di ANALYSIS_PROMPT
    # in Architect_302.py, ma limitato a scena/ambiente (SUBJECT/OUTFIT/
    # ACCESSORIES/BODY ART esclusi di proposito, coerente con lo scopo di
    # questa funzione: qui serve solo la location, l'outfit lo genera
    # Surprise stesso in modo casuale altrove).
    try:
        from C_shared100 import genai_types, detect_mime_type
        mime = detect_mime_type(img_bytes)
        img_part = genai_types.Part.from_bytes(data=img_bytes, mime_type=mime)
        location_prompt = (
            "Describe in complete, exhaustive detail ONLY the location, setting and "
            "environment visible in this image, from every point of view. Ignore "
            "completely any person, clothing, outfit or accessories — describe only "
            "the physical environment. Organize the description into the labeled "
            "sections below, each as a detailed paragraph, precise enough that "
            "someone could recreate a very similar setting from this text alone.\n\n"
            "BACKGROUND: [Exact location, architecture, surfaces, props, environment "
            "— be specific.]\n\n"
            "LIGHTING: [Light source, direction, quality, color temperature.]\n\n"
            "CAMERA: [Framing, angle, depth of field, point of view.]\n\n"
            "MOOD: [Overall atmosphere, time of day, color grade, cinematic style.]\n\n"
            "Output ONLY the labeled sections above, in that order, as plain text — "
            "no markdown formatting, no commentary before or after."
        )
        location_text = gemini.generate(location_prompt, contents=[img_part], max_tokens=2048)
    except Exception as e:
        location_text = None
    ...
    bot.send_message(cid, "✅ <b>Location acquisita:</b>")
    CHUNK = 3800
    chunks = [location_text[i:i+CHUNK] for i in range(0, len(location_text), CHUNK)]
    for idx, chunk in enumerate(chunks):
...
WALTER_DNA = (
    ...
    "thin octagonal Vogue Havana dark tortoiseshell frame eyeglasses, always present, "
    "hourglass feminine body silhouette, smooth hairless skin, soft curves. "
    "Minimal makeup, limited to subtle Pride-themed styling, with the described short silver-grey hair throughout."
)
...
CARLOTTA_DNA = (
    ...
    "Hair color stays exactly rich auburn-red with golden tones throughout."
)
...
def _build_pride_prompt(fixed_location=None):
    ...
    prompt = (
        ...
        f"**⚠️ IDENTITY LOCK:** Carlotta's hair colour and Walter's hair colour, glasses and beard stay "
        f"exactly as described above for each subject. Both dog breeds stay exactly as described. All "
        f"four subjects appear sharp, clearly recognizable and fully present in the frame, in the exact "
        f"location described above as the setting."
    )
...
def _build_flag_prompt() -> str:
    ...
    prompt = (
        ...
        "**BODY — HELD EXACTLY AS DESCRIBED:**\n"
        "Feminine hourglass. Height 180 cm, D-cup bust, defined waist, full hips. "
        "Completely smooth, hairless skin throughout. Long slim legs.\n\n"
        "---\n\n"
        "**PANEL TEMPLATE:**\n"
        ...
        "identical panel-to-panel, with continuous edges. Flat solid background per panel — smooth, "
        ...
        "**TECHNICAL:** Format 3:2.8 landscape. 8K sharp fashion editorial, bright and vivid throughout, "
        "editorial clothing as described, completely smooth hairless skin.\n\n"
