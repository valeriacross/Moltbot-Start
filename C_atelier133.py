...
def _atelier_send_extras(cid, uid, prompt_text, img_data):
    """Invia automaticamente caption e mini prompt dopo il prompt Flow."""
    # 1. Caption
    try:
    ...
    # 2. Mini prompt
    try:
        mini, _ = generate_mini_prompt(prompt_text)
        if mini:
            bot.send_message(cid, mini, parse_mode="HTML")
    except Exception as e:
        logger.warning(f"⚠️ auto mini prompt: {e}")
...
def handle_photo(m):
    ...
    def _process():
        ...
        if flt.get("shooting"):
            ...
            _atelier_send_extras(cid, uid, full_p, img_data)
        ...
        if is_dual:
            ...
            for v in variant_list:
                ...
                if len(v['full_p']) <= CHUNK:
                    ...
                    for idx, chunk in enumerate(chunks[1:], 2):
                        bot.send_message(cid, f"<i>({idx}/{len(chunks)})</i>\n<code>{html.escape(chunk)}</code>", parse_mode="HTML")
            _atelier_send_extras(cid, uid, variant_list[0]['full_p'], img_data)
