...
def _run_lego_mosaic(chat_id, uid, username, img_bytes):
    ...
    try:
        ...
        STUD = 20          # px per stud nell'immagine output
        ...
        bot.send_photo(chat_id, buf,
            ...
            parse_mode="HTML", reply_markup=list_markup)
...
def _detect_bokeh_mode(src):
    ...
    # Centro di massa dei pixel con luminosità > 128
    bright_y = []
    for idx, v in enumerate(arr):
        if v > 128:
            bright_y.append(idx // w)

    if not bright_y:
        return 'radial'

    center_y_ratio = sum(bright_y) / len(bright_y) / h

    # Se il centro di massa è nella fascia centrale → radiale (ritratto)
    # Se è nella fascia bassa → verticale (figura intera)
...
def _draw_brick(draw, x, y, w_studs, h_studs, stud_size, rgb):
    """
    Disegna un mattoncino LEGO di dimensioni w_studs × h_studs studs.
    Effetto rilievo con ombra e highlight.
    """
    pw = w_studs * stud_size
    ph = h_studs * stud_size
    shadow = tuple(max(0, c - 40) for c in rgb)
    highlight = tuple(min(255, c + 40) for c in rgb)

    # Corpo mattoncino
    draw.rectangle([x, y, x + pw - 1, y + ph - 1], fill=rgb)
    # Bordo ombra basso-destra
    draw.line([x, y + ph - 1, x + pw - 1, y + ph - 1], fill=shadow, width=1)
    draw.line([x + pw - 1, y, x + pw - 1, y + ph - 1], fill=shadow, width=1)
    # Bordo highlight alto-sinistra
    draw.line([x, y, x + pw - 1, y], fill=highlight, width=1)
    draw.line([x, y, x, y + ph - 1], fill=highlight, width=1)

    # Studs: uno per cella
    for sy in range(h_studs):
        for sx in range(w_studs):
            pad = int(stud_size * 0.22)
            ox = x + sx * stud_size
            oy = y + sy * stud_size
            draw.ellipse(
                [ox + pad, oy + pad, ox + stud_size - pad, oy + stud_size - pad],
                fill=rgb, outline=shadow
            )


def _run_lego_galaxy(chat_id, uid, username, img_bytes):
    """
    Filtro LEGO Galaxy: mattoncini misti 1×1, 1×2, 2×2.
    Zone soggetto → 1×1 e 1×2 per dettaglio.
    Zone sfondo (bokeh) → 2×2 per aree omogenee.
    Bokeh mode rilevato automaticamente dall'immagine.
    """
    try:
        from PIL import Image as PILImage, ImageDraw as PILDraw
        import math
        ...
        # Rileva modalità bokeh automaticamente sull'immagine originale
        ...
        # Applica bokeh e ridimensiona
        src_bokeh = _apply_bokeh_fast(src, bokeh_mode)
        src_small = src_bokeh.resize((cols, rows), PILImage.LANCZOS)
        src_orig_small = src.resize((cols, rows), PILImage.LANCZOS)  # originale per colori soggetto

        lego_rgb_map = {name: (r, g, b) for name, r, g, b in LEGO_PALETTE}
        color_counts = {}

        # Griglia colori — usa originale per soggetto, bokeh per sfondo
        grid = []
        for row in range(rows):
            line = []
            for col in range(cols):
        ...
        out_w = cols * STUD
        out_h = rows * STUD
        out = PILImage.new("RGB", (out_w, out_h), (30, 30, 30))
        draw = PILDraw.Draw(out)

        # Rendering dual-pass con maschera booleana "già disegnato"
        ...
        for row in range(rows):
            for col in range(cols):
                ...
                cname = grid[row][col]
                rgb = lego_rgb_map[cname]
                is_subject = _is_subject_zone(col, row, cols, rows, bokeh_mode)

                # Tenta 2×2 solo in zona sfondo con colore omogeneo
                if (not is_subject
                        and col + 1 < cols and row + 1 < rows
                        and not placed[row][col+1]
                        and not placed[row+1][col]
                        and not placed[row+1][col+1]):

                    # Verifica omogenità: tutti e 4 i pixel hanno stesso colore LEGO
                    neighbors = [
                        grid[row][col+1],
                        grid[row+1][col],
                        grid[row+1][col+1],
                    ]
                    if all(n == cname for n in neighbors):
                        _draw_brick(draw, col*STUD, row*STUD, 2, 2, STUD, rgb)
                        placed[row][col] = placed[row][col+1] = True
                        placed[row+1][col] = placed[row+1][col+1] = True
                        color_counts[cname] = color_counts.get(cname, 0) + 1
                        continue

                # Tenta 1×2 in zona soggetto — segue il gradiente
                if (is_subject and col + 1 < cols
                        and not placed[row][col+1]
                        and grid[row][col+1] == cname):

                    # Orientamento: guarda il gradiente verticale vs orizzontale
                    if row + 1 < rows:
                        vert_diff = abs(sum(src_orig_small.getpixel((col, row+1))) -
                                       sum(src_orig_small.getpixel((col, row))))
                    else:
                        vert_diff = 255

                    if col + 1 < cols:
                        horiz_diff = abs(sum(src_orig_small.getpixel((col+1, row))) -
                                        sum(src_orig_small.getpixel((col, row))))
                    else:
                        horiz_diff = 255

                    # Se gradiente orizzontale debole → mattone orizzontale 1×2
                    if horiz_diff < vert_diff:
                        _draw_brick(draw, col*STUD, row*STUD, 2, 1, STUD, rgb)
                        placed[row][col] = placed[row][col+1] = True
                        color_counts[cname] = color_counts.get(cname, 0) + 1
                        continue

                # Fallback: 1×1
                _draw_brick(draw, col*STUD, row*STUD, 1, 1, STUD, rgb)
                placed[row][col] = True
                color_counts[cname] = color_counts.get(cname, 0) + 1
        ...
        bot.send_photo(chat_id, buf,
            caption=(f"🌌 <b>LEGO Galaxy</b> — {cols}×{rows} studs | "
                     f"{total} mattoncini | bokeh: {bokeh_mode}"),
            parse_mode="HTML", reply_markup=list_markup)
