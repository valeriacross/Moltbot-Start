...
**Generato:** 29/08/2026, fine sessione (aggiornato da versione 10/08)
**Da:** Walter Caponi
...
8. Ad ogni update richiesto: HANDOFF + README insieme (VERSIONI_BOT Excel escluso su richiesta esplicita di Walter, 28/07 — se ne occupa lui)
...
## 2. STATO FILE ATTUALE (29/08/2026)

| File | Versione | Koyeb service | Run command | Chiavi API |
|------|---------|---------------|--------------|-----------|
| `C_shared100.py` | **2.4.12** | (comune a tutti) | — | — |
| `Vogue_430.py` | **4.3.0** | colossal-giselle/vogue | `python Vogue_430.py` | 2 |
| `Architect_310.py` | **3.1.0** | homely-annabelle/thearchitect | `python Architect_310.py` | 1 |
| `Atelier_270.py` | **2.7.0** | flexible-denna/atelier | `python Atelier_270.py` | 5 |
...
## 2quinvicies. SESSIONE 11-14/08/2026 — VOGUE 4.0→4.3: SEMPRE VIDEO, MAI FOTO

**Richiesta di Walter:** Vogue video (V2V) funziona benissimo per il suo uso; Vogue foto (I2V) è troppo simile ad Atelier — voleva che da un'immagine Vogue generasse un prompt VIDEO invece che foto, inventando un movimento coerente (es. "donna con cane" → "donna che cammina col cane").

**Vogue 4.0.0:** nuova `build_prompt_video()` — copia di `build_prompt()` con solo le GENERATION INSTRUCTIONS finali cambiate (video a scatto unico, movimento inventato anche drammatico senza indizi espliciti, nessuna durata specificata). `handle_photo()` passa a chiamarla. `handle_video()` lasciato invariato (video → prompt foto ferma) — **decisione rivelatasi sbagliata**, vedi sotto.

**Correzione 4.1.0, dopo una reazione dura di Walter:** il video "invariato" produceva un prompt FOTO, non un prompt video — non era quello che intendeva. Richiesta reale: **entrambe le modalità generano SEMPRE un prompt video** — da foto lo inventa, da video lo COPIA (riproduce il movimento realmente osservato). Riscritto `_ANALYZE_PROMPT_VIDEO`: non riduce più la clip a un istante fermo, nuovo campo `MOVEMENT` che descrive sequenza/direzione/velocità osservate. `build_prompt_video()` resa condizionale tramite nuova `_has_described_movement()` (stesso pattern di `has_multiple_figures`, shared): se MOVEMENT è valorizzato, riproduce quel movimento esatto; se assente (foto statica), lo inventa. `handle_video()` ora chiama anch'essa `build_prompt_video()`.

**Vogue 4.2.0:** su richiesta di Walter, rimosso del tutto l'input testuale (T2I/T2V) — restano solo I2V e V2V. `handle_text()` non genera più nulla, solo un messaggio guida (scelta di Claude, non richiesta esplicitamente). Rimossa `build_prompt()` (foto ferma, non più chiamata da nessuno) e l'import `sanitize_user_input`.

**Il problema vero — tasso di accettazione Flow:** Walter segnala che il prompt I2V di Vogue fa accettare il video a Flow solo ~1/10 volte, contro ~9/10 di un prompt immagine equivalente di Atelier per la stessa foto di partenza — allegati entrambi i prompt per il confronto diretto.

- **Fix 4.2.1:** verificato nel codice (non solo nei due prompt incollati) che Atelier apre OGNI prompt con `EDITORIAL_WRAPPER` (shared) — la frase "professional editorial post-production request... legitimate creative and commercial photography workflow". Vogue non l'ha MAI importata, in nessuna versione da quando esiste. Aggiunta `EDITORIAL_WRAPPER_VIDEO`, isolata in Vogue (shared non toccato), wording adattato al video.
- **Walter ha testato: stesso risultato, nessun miglioramento.**
- **Fix 4.2.2:** guardando un prompt reale generato da Vogue, trovata una contraddizione diretta nel mezzo del testo — `VALERIA_DNA` (shared, di fatto usata solo da Vogue) termina con *"The output is a single photorealistic **image**"*, mentre il resto del prompt dice "video" tre volte. Nuova `VALERIA_DNA_VIDEO`, isolata in Vogue: ricompone FACE+BODY_STRONG+TEXTURE_LOCK con una coda locale che dice "video clip" invece di "image".
- **Diagnosi risolutiva, test diretto di Walter:** senza allegare la foto reale del volto, la generazione video riesce quasi sempre; con la foto allegata, ~1/20. Causa isolata con un test pulito (stessa scena, unica variabile cambiata: presenza/assenza della foto). Supportato anche da ricerca esterna: le policy Veo/Flow sul video restringono più duramente delle immagini statiche le "rappresentazioni realistiche di persone reali identificabili" — allegare una foto vera del volto è probabilmente il trigger diretto per il video, mentre su Atelier (immagini) non lo è.
- **Fix 4.3.0:** nuova `VALERIA_FACE_VIDEO`, isolata in Vogue — prima descrizione testuale completa del volto di Valeria mai scritta nel progetto (l'identità era sempre stata delegata alla foto reale allegata). Walter ha fornito una foto di riferimento per farla scrivere puntando sugli elementi più distintivi/riconoscibili (in particolare la montatura degli occhiali bicolore asimmetrica — l'elemento singolarmente più identificativo). Sostituisce ogni riferimento a "the attached reference photo" nel percorso video. shared non toccato: `VALERIA_FACE` resta invariata per Atelier, dove allegare la foto reale continua a funzionare. **Non ancora testato in produzione con questa nuova descrizione.**

**Nota permanente onesta, ripetuta più volte a Walter durante questa sessione:** nessuno di questi fix garantisce la parità di accettazione con Atelier — è plausibile che la generazione video abbia filtri di moderazione strutturalmente più severi di quella immagine, un limite di piattaforma non risolvibile solo lato prompt.

---

## 2quattuorvicies. SESSIONE 05-10/08/2026 — VOGUE 3.0/3.1: SUPPORTO VIDEO

**Punto di partenza:** discussione preliminare su cosa succede se si manda un video invece di una foto ai bot — verificato che nessuno dei tre principali (Vogue/Atelier/Architect) ha un handler `content_types=['video']`: un video viene ignorato silenziosamente, nessuna risposta, nessun log. Approfondito se avrebbe senso aggiungerlo: confermato che Gemini analizza video nativamente (senza bisogno di estrarre frame a mano) per file sotto i 20MB o clip più corte di ~1 minuto, tramite lo stesso `Part.from_bytes()` già usato per le foto — e che il vincolo pratico più stretto non è Gemini ma **Telegram**: l'API cloud standard dei bot permette download fino a 20MB via `getFile`, tetto fisso indipendente dalla durata. Con i video reali di Walter (4-10 secondi con Flow) il vincolo di fatto sparisce — file da pochi MB, qualche migliaio di token per Gemini, nessun impatto pratico sulla quota giornaliera (stessa `analyze_scene()`, stessa quota delle foto).

**Decisione di architettura — Vogue 3.0 come fork indipendente, non un patch su Vogue 2.2:** su richiesta esplicita di Walter, `Vogue_300.py` è partito come copia esatta di `Vogue_220.py` — se il video non avesse funzionato, rollback = ridistribuire `Vogue_220.py` senza aver toccato nient'altro. Tre scelte di disegno concordate prima di scrivere codice: (1) il nuovo prompt di analisi video vive isolato dentro Vogue, **non** in `C_shared100.py` — zero rischio per Atelier/Architect/Vogue 2.2; (2) menu esplicito su `/start` ("📷 Immagine" / "🎥 Video") invece di rilevamento automatico dal tipo di file — se mandi il tipo sbagliato rispetto alla scelta, messaggio di mismatch invece di processare comunque; (3) copia esatta come base, nessun altro ripensamento aggiunto insieme al video.

**Implementazione:** nuovo `_ANALYZE_PROMPT_VIDEO` — stesse identiche sezioni di `_ANALYZE_PROMPT` (shared) così `body_art_clause()`/`multi_subject_clause()` e `build_prompt()` restano invariati — cambia solo l'istruzione iniziale (considerare l'intera clip, scegliere il momento più rappresentativo per una foto ferma) e una regola sul movimento. Nuova `analyze_video()`, stessa struttura/classificazione errori di `analyze_scene()` ma locale a Vogue. Controllo `m.video.file_size` **prima** del download (niente chiamate sprecate se supera i 20MB). `/caption` e l'input testuale restano indipendenti dalla scelta modalità — decisione presa da Claude senza conferma esplicita di Walter, da tenere d'occhio.

**Test reale di Walter:** ha mandato un video (mermaid tail garment su scoglio), il prompt generato ha catturato bene il momento dinamico (`PROPS & ACTIONS`: *"water droplets splash in mid-air around the flaring tail fin"*) invece di una descrizione statica — coerente con l'istruzione di scegliere il momento più rappresentativo. Nessun problema nella generazione; un artefatto isolato segnalato da Walter (*"tral blue star-shaped gems"*) si è rivelato un errore di copia-incolla nel messaggio a Claude, non un difetto del prompt. **Confermato "funziona egregiamente per essere una beta".**

**Fix Vogue 3.1.0 — uscita dalla beta:** su decisione di Walter, Vogue 2.2 è considerata archiviata — Vogue 3.x diventa la versione di riferimento. Tolto "(beta)" da pulsante e messaggio di conferma modalità video. Aggiunto un terzo pulsante "🎥 Video" alla tastiera post-prompt (riusa il callback `vogue_mode_video` già gestito, nessuna nuova logica) accanto a "📸 Nuova foto"/"🏠 Home", invariati.

---

## 2trevicies. SESSIONE 03-05/08/2026 — RIFINITURE MULTI-SOGGETTO, VISIBILITÀ FALLBACK, DEPLOY

**Conferma multi-soggetto oltre i 2:** Walter ha testato con 4 figure contemporaneamente (non solo 2) — risultato soddisfacente, ogni figura con l'identità di Valeria e outfit separato. Il fix di 2.4.9/2.4.10 regge anche scalando oltre il caso a due persone.

**Decisione esplicita di Walter: nessun fix sul mosaico multi-soggetto.** Con 3+ figure il mosaico a 4 scatti a volte smista le figure in modo strano tra i pannelli (una persona per pannello invece di tutte insieme, o conteggio variabile pannello per pannello) — Walter ha deciso di NON investirci, anzi trova i risultati imprevedibili "simpatici" quando capitano. Nessuna modifica al codice per questo.

**Bug scoperto da Walter — checklist mosaico con testo morto:** la riga "Subject identity" del checklist "What stays identical across all 4 shots" (corretta in 2.6.1 per menzionare ogni figura) era scritta come testo STATICO — restava con la frase "if multiple figures are present..." anche quando FIGURES rileva una sola persona, dove non ha senso. A differenza di `multi_subject_clause()`, correttamente condizionale. **Fix shared 2.4.11 + Atelier 2.6.2:** estratta la logica di rilevamento in una nuova funzione `has_multiple_figures()` (usata internamente anche da `multi_subject_clause()`, stessa regex non più duplicata) — la riga del checklist ora è condizionale, torna al testo originale con una sola figura.

**Nuova richiesta di Walter — visibilità del fallback nel prompt:** quando scatta il fallback su `MODEL_LITE` (503 o 429), Walter vuole vedere il codice errore nell'header del prompt, per sapere se conviene riprovare a breve (503, transitorio) o se resterà su lite fino al reset delle 08:00 (429, quota esaurita per la giornata). **Fix shared 2.4.12 + Atelier 2.7.0 (minor, nuova funzionalità visibile):** nuovo `GeminiClient.last_fallback_code` ("503"/"429"/"transient"/None), impostato dentro `generate()` nello stesso punto in cui scatta il fallback — non si azzera da solo dentro `generate()` perché una richiesta utente fa più chiamate (analisi + review_and_fix) e l'informazione deve sopravvivere a entrambe. Nuovo metodo `reset_fallback()`, stesso pattern di `reset_counters()` — Atelier lo chiama a inizio richiesta. Header ora mostra ad es. `· ⚠️ 429 quota esaurita → lite (resta così fino alle 08:00)`.

**Conferma in produzione (05/08, log reale):** un 503 UNAVAILABLE su `review_and_fix` è stato recuperato in ~2.4 secondi passando a `gemini-3.1-flash-lite` sulla stessa chiave — nessun errore visibile all'utente. Prima verifica reale del fallback reattivo su 503 (introdotto in 2.4.7/2.4.8), non solo sulla carta.

**Incidente di deploy (05/08), non un bug di codice:** dopo aver caricato `Atelier_270.py` su Koyeb, il servizio `flexible-denna/atelier` ha iniziato a fallire il deploy. Walter ha mandato uno screenshot dell'errore generico di Koyeb, poi il log completo — che mostra la causa reale: `python: can't open file '/workspace/Atelier_261.py': [Errno 2] No such file or directory`. Il **Run command** del servizio Koyeb era rimasto a `Atelier_261.py` (due rename fa), mai aggiornato dopo i rename successivi a 262 e poi 270. Verificato prima il codice consegnato (sintassi pulita, import corretti, nessun problema) prima di ipotizzare la causa esterna — coerente con la regola già scritta di controllare il codice prima di attribuire un fallimento a cause esterne. Vedi lezione #21 in sezione 11.

---

## 2duovicies. SESSIONE 01-02/08/2026 — SUPPORTO MULTI-SOGGETTO (2+ PERSONE NELLA FOTO)

**Punto di partenza:** Walter ha chiesto di gestire foto di riferimento con 2 o più soggetti distinti (m+f, m+m, f+f...): il prompt generato deve tenere conto di tutti, presentando una figura multipla sempre con lo stesso DNA/volto di Valeria — rilevamento automatico, non un comando esplicito. In un secondo momento, senza bisogno di spiegazioni aggiuntive, Walter ha chiesto anche outfit separato per figura (non condiviso).

**Verificato prima di scrivere codice:** Architect non inietta alcun DNA — non gli serve nessun "lock" di identità, gli serve solo descrivere fedelmente ciascuna persona reale separatamente.

**Fix shared 2.4.9:** nuovo campo `FIGURES:` in `_ANALYZE_PROMPT` (conta le persone, posa/posizione/interazione fisica di ciascuna, cieco sull'identità — stesso approccio di OUTFIT/PROPS & ACTIONS). OUTFIT/ACCESSORIES/BODY ART resi strutturabili per figura (stessa label breve usata in FIGURES) quando FIGURES rileva 2+ persone. Nuova costante `VALERIA_MULTI_SUBJECT_LOCK` + funzione `multi_subject_clause()` — stesso identico pattern di `body_art_clause()`: nessun impatto sul caso comune (una sola figura).

**Fix Atelier 2.6.0 e Vogue 2.2.0:** inserito `multi_subject_clause()` subito dopo `body_art_clause()` in tutti i punti di assemblaggio del prompt finale (3 in Atelier: `build_full_prompt`, `build_shooting_prompt` mode single e mosaico; 1 in Vogue: `build_prompt`). Bump di **MINOR**, non patch: nuova capacità, non un fix.

**Fix Architect 3.1.0:** `SUBJECT` reso multi-figura consapevole (label breve consistente, riusata in OUTFIT/ACCESSORIES/BODY ART) invece di assumere una sola persona in frame. Nessun `MULTI-SUBJECT LOCK` qui — Architect non inietta DNA, descrive fedelmente le persone reali.

**Errore di processo di Claude, corretto da Walter:** la prima consegna di Atelier/Vogue aveva mantenuto lo stesso nome file (`Atelier_254.py`/`Vogue_210.py`) aggiornando solo il changelog interno — in violazione della regola "mai stesso nome file qualsiasi sia la modifica". Corretto su richiesta di Walter, rinominando ad `Atelier_260.py`/`Vogue_220.py` (e `Architect_310.py` fin da subito, stesso errore evitato). Vedi lezione #20 in sezione 11.

**Test in produzione (Walter, foto reale con 2 donne):**
- **Architect** → aderenza totale all'originale (nessuna sostituzione, come da progetto).
- **Vogue** → ottimo, doppia figura con volto di Valeria su entrambe.
- **Atelier modalità singola** → ottimo, paragonabile a Vogue (stessa struttura: un solo scatto, ogni LOCK una volta sola).
- **Atelier MOSAICO** → fallito: la figura secondaria ha mantenuto il volto reale della foto originale invece di quello di Valeria.

**Diagnosi del fallimento sul mosaico:** due cause concorrenti individuate. (1) `VALERIA_MULTI_SUBJECT_LOCK` era un rimando indiretto ("sharing the identical face... described above"), debole a confronto con FACE IDENTITY LOCK/IDENTITY LOCK/HAIR LOCK/BEARD MANDATORY che nel resto del prompt ribadiscono l'identità della figura principale più volte. (2) Solo nel mosaico esiste una sezione "What stays identical across all 4 shots" con la riga `Subject identity (face, hair, beard, glasses — ZERO drift)` al singolare — rinforzava proprio l'idea di un solo soggetto.

**Fix shared 2.4.10:** `VALERIA_MULTI_SUBJECT_LOCK` riscritto per essere esplicito quanto gli altri LOCK — dice direttamente che la sostituzione è totale e vale per OGNI figura, non solo la principale, nessun volto originale sopravvive per nessuna figura.

**Fix Atelier 2.6.1 (patch, non minor — fix mirato):** corretta SOLO la riga "Subject identity" del checklist mosaico per menzionare esplicitamente ogni figura — la riga equivalente in modalità singola non è stata toccata, già confermata funzionante.

**Confermato in produzione il 02/08:** su 12 immagini generate con il fix attivo, il mosaico (il caso peggiore osservato) tiene il volto di Valeria su entrambe le figure — accettabile.

---

## 2unvicies. SESSIONE 30/07/2026 — FALLBACK GEMINI 503/429 SU MODEL_LITE

**Punto di partenza:** Walter segnala errori "Servizio Gemini non disponibile — Sovraccarico temporaneo" ricorrenti da due settimane, tipicamente dalle 12/13 di Lisbona fino a sera tardi, su tutti i bot.

**Diagnosi iniziale (ricerca web):** i 503 "model overloaded" su Gemini sono un fenomeno noto/diffuso nel 2026, correlato al carico globale — coerente con l'orario segnalato (Lisbona mezzogiorno = fine della finestra a basso traffico USA, inizio del carico che sale con l'orario lavorativo USA). Verificato nel codice: il retry esistente in `generate()` ruota le chiavi ma non aiuta contro un vero overload lato server, condiviso da tutte le chiavi sullo stesso modello.

**Fix shared 2.4.6 (superato lo stesso giorno da 2.4.7):** prima versione — soglia proattiva `LITE_FALLBACK_AT = 75`: oltre 75 call totali/giorno (75% di una quota stimata di 100), le chiamate sul modello di default passavano a `MODEL_LITE`. Su richiesta esplicita di Walter, cambiato approccio: rimossa la soglia, reso reattivo.

**Fix shared 2.4.7:** fallback reattivo — su un errore 503/overload, ritenta SUBITO con `gemini-3.1-flash-lite` sulla stessa chiave, prima di ruotare. Per il 429 restava il comportamento precedente (sola rotazione chiavi), perché lì la rotazione aiuta davvero (quota separata per chiave).

**Svolta con log reale:** Walter ha allegato un log di produzione (`flexible-denna-atelier`). Non era un 503 — era un **429 RESOURCE_EXHAUSTED**: quota free tier di **20 richieste/giorno per chiave per modello** su `gemini-3.5-flash` (già documentata in sezione 4, mai collegata prima a questo sintomo), esaurita su tutte e 5 le chiavi nello stesso giro (rotazione 2→3→4→5→1, tutte in 429). Questo spiega anche meglio il pattern orario originale: probabilmente non solo overload, ma consumo delle 100 chiamate/giorno totali (5 chiavi × 20) che si esaurisce a metà giornata con l'uso normale.

**Fix shared 2.4.8:** il trigger del fallback reattivo esteso a QUALSIASI errore transitorio (503/overload, 429/quota, timeout, connessione) — non solo 503. `MODEL_LITE` ha una quota separata da `gemini-3.5-flash`, non toccata dall'esaurimento sul modello principale.

**Non toccato:** `Atelier_254.py` — nessuna modifica di codice, usa `generate()` senza toccare la logica interna.

**Non ancora testato in produzione.**

**Errore di processo di Claude in questa sessione, per trasparenza:** durante questo lavoro Claude ha erroneamente dichiarato due volte di non avere più i file di lavoro (`C_shared100.py`/`Atelier_254.py`/HANDOFF/README) già presenti nell'area di lavoro della stessa chat, e in un caso ha attribuito una discrepanza già presente nel file (il fix 2.4.5 e poi l'estensione al 429 già scritti) a un presunto editing "in parallelo" da parte di Walter. Walter ha chiarito di usare solo questa chat — non c'era nessun editing esterno, era una gestione del contesto interna a Claude su una sessione molto lunga. Vedi lezione #18 in sezione 11.
...
## 2duodevicies. SESSIONE 25/07/2026 — FOTO DI RIFERIMENTO AUTOREVOLE (VOLTO), DENSITÀ SFONDO, FRAMING MOSAICO
...
**Osservazione parziale su produzione (stesso giorno, un mosaico mostrato da Walter dopo il fix 2.4.1/2.5.1):** identità stabile sui 4 scatti — stessi occhiali rotondi spessi scuri, stessa barba, nessuna deriva verso forme ottagonali/sottili. Segnale positivo ma su un solo campione, non una validazione esaustiva su un modello non deterministico (vedi lezione #16). Trovato, non legato ai nostri fix: refuso di rendering testo nel watermark di uno dei 4 pannelli ("feat. Valeria Crosa" invece di "Cross") — limite noto di Flow/Nano Banana sul rendering di testo piccolo su mosaici multi-pannello, non un problema di prompt engineering, non toccato.

**Causa diversa, stesso giorno — densità dello sfondo insufficiente rispetto alla foto originale:** Walter ha allegato foto originale + generazione Flow + prompt completo per un secondo caso (tea-party surrealista pieno di orologi/lanterne/porcellane sospese). Sfondo generato molto più povero di dettaglio e con palette diversa (verde salvia dominante vs. nero/oro/rosa dell'originale) — soggetto, pose e mosaico corretti, nessun problema lì. Diagnosi: `_ANALYZE_PROMPT` (shared) chiede per OUTFIT/ACCESSORIES/BODY ART/PROPS un'enumerazione esaustiva oggetto-per-oggetto con HEX — per BACKGROUND chiedeva solo "be specific", una riga generica, zero richiesta di elenco o densità. La compressione avveniva già in fase di analisi della foto, prima che Flow disegnasse qualunque cosa — nessun lock in generazione può recuperare un dettaglio mai catturato nel testo.

**Fix shared 2.4.2:** campo BACKGROUND di `_ANALYZE_PROMPT` riscritto con lo stesso trattamento esaustivo di OUTFIT — enumerazione per oggetto, conteggio/densità approssimativa per elementi ripetuti, HEX dove rilevante, profondità spaziale.

**Fix Atelier 2.5.2:** `⚠️ CRITICAL — BACKGROUND LOCK` (entrambi i rami, single e mosaico) non aveva istruzione di densità rispetto alla scena originale, a differenza di `OUTFIT DETAIL LOCK` — aggiunta la stessa clausola ("same quantity, density and placement... not simplified or reduced in number").

**Domanda di Walter, sessione stessa:** i fix generalizzano a qualsiasi scena originale, o funzionano solo per stanze piene di oggetti come il caso di test (es. una giungla con fiori e animali)? Risposta: il meccanismo è generico (nessun riferimento hardcoded al caso specifico), ma l'unico esempio illustrativo nel campo BACKGROUND era sbilanciato verso scene "da collezione di oggetti" (l'esempio degli orologi), rischio di bias sottile su scene organiche.

**Fix shared 2.4.3:** aggiunto un secondo esempio parallelo per contenuto naturale/organico (fogliame, fauna) accanto a quello man-made, così l'istruzione BACKGROUND resta domain-agnostic.
...
## 4. C_SHARED100.PY — STORICO COMPLETO FIX (v2.3.0 → v2.4.12)

- **v2.4.12 (03/08/2026):** nuovo `GeminiClient.last_fallback_code`/`reset_fallback()` — visibilità nell'header del prompt su quale errore (503/429/transient) ha fatto scattare il fallback su `MODEL_LITE`. Vedi sezione 2trevicies.
- **v2.4.11 (03/08/2026):** estratta `has_multiple_figures()` da `multi_subject_clause()` — riusata da Atelier per rendere condizionale la riga "Subject identity" del checklist mosaico (prima era testo statico anche con una sola figura). Vedi sezione 2trevicies.
- **v2.4.10 (01/08/2026):** `VALERIA_MULTI_SUBJECT_LOCK` riscritto per essere esplicito quanto gli altri LOCK dopo che il mosaico Atelier ha fallito il test in produzione (figura secondaria con volto reale) — dice direttamente che la sostituzione vale per OGNI figura, non solo la principale. Vedi sezione 2duovicies.
- **v2.4.9 (01/08/2026):** nuovo campo `FIGURES` in `_ANALYZE_PROMPT`, OUTFIT/ACCESSORIES/BODY ART resi strutturabili per figura, nuova `VALERIA_MULTI_SUBJECT_LOCK`/`multi_subject_clause()` — supporto foto con 2+ soggetti distinti, tutti sostituiti dalla stessa identità Valeria. Vedi sezione 2duovicies.
- **v2.4.8 (30/07/2026):** trigger del fallback reattivo su `MODEL_LITE` esteso a QUALSIASI errore transitorio (503/overload, 429/quota, timeout, connessione), non solo 503 — scoperto con un log reale che il sintomo era in realtà 429/quota (20 richieste/giorno/chiave/modello esaurite su tutte le chiavi), non overload. Vedi sezione 2unvicies.
- **v2.4.7 (30/07/2026):** fallback su `MODEL_LITE` reso reattivo (su 503/overload, ritenta subito sulla stessa chiave prima di ruotare) invece che su soglia di consumo — rimossa `LITE_FALLBACK_AT` introdotta in 2.4.6. Vedi sezione 2unvicies.
- **v2.4.6 (30/07/2026, superata lo stesso giorno):** prima versione del fallback su `MODEL_LITE` — soglia proattiva `LITE_FALLBACK_AT = 75` call/giorno. Vedi sezione 2unvicies.
- **v2.4.5 (28/07/2026):** nuova riga incondizionata dopo COEXISTENCE in `VALERIA_BODY_STRONG` ("BUST VOLUME — ALWAYS VISIBLE") e `VALERIA_BODY_SAFE` ("SILHOUETTE — ALWAYS VISIBLE") — torso reso maschile su scene a pelle scoperta nonostante FULL D-CUP BUST/COEXISTENCE già presenti. Vedi sezione 2vicies.
- **v2.4.4 (27/07/2026):** nuova costante `VALERIA_TEXTURE_LOCK`, generica e in positivo, per la fedeltà di scala/densità di qualunque pattern/texture descritta nella scena. Inserita sia in `VALERIA_DNA` sia in `build_valeria_identity()`. Vedi sezione 2undevicies.
- **v2.4.3 (25/07/2026):** campo BACKGROUND di `_ANALYZE_PROMPT` (fix 2.4.2, sotto) aveva un solo esempio illustrativo orientato a scene "da collezione di oggetti" (orologi). Su domanda esplicita di Walter se il fix generalizzasse a scene organiche (es. una giungla), aggiunto un secondo esempio parallelo per contenuto naturale/organico, per restare domain-agnostic. Vedi sezione 2duodevicies.
- **v2.4.2 (25/07/2026):** campo BACKGROUND di `_ANALYZE_PROMPT` era l'unico senza richiesta di enumerazione oggetto-per-oggetto (a differenza di OUTFIT/ACCESSORIES/BODY ART/PROPS, tutti "every X as a standalone object" con HEX) — causa di sfondi generati molto più poveri di dettaglio/densità rispetto alla foto originale. Riscritto con lo stesso trattamento esaustivo di OUTFIT. Vedi sezione 2duodevicies e Atelier 2.5.2 per la metà lato generazione.

- **v2.4.1 (25/07/2026):** `VALERIA_FACE`, `review_and_fix()` (regole 2 e 4) e `sanitize_user_input()` non specificano più forma/lunghezza di occhiali e barba (era: "thin octagonal Vogue Havana dark tortoiseshell", "beard 6-7cm") — sostituito con rimando esplicito alla foto di riferimento reale allegata da Walter in Flow, confermata da lui come sempre autorevole su questi due punti. I tre punti reiniettavano la vecchia specifica indipendentemente l'uno dall'altro; corretti insieme nella stessa sessione, altrimenti il fix su uno solo veniva vanificato dagli altri due. Corretto anche un disallineamento trovato per caso: `VERSION` era rimasta a `2.3.18`, `SHARED_VERSION` (quella esportata) era già a `2.4.0` dal bump del 17/07 — allineate entrambe. Vedi sezione 2duodevicies.
...
**Limite Google free tier (confermato da log reali):** 20 richieste/giorno per chiave per modello (`gemini-3-flash`). Quota condivisa per progetto Google Cloud — ogni chiave è su un progetto diverso (verificato via screenshot Google AI Studio: 10 progetti distinti, tutti free tier). **Ricollegato il 30/07 al sintomo "Servizio Gemini non disponibile":** un log reale ha mostrato questo limite esaurito su tutte e 5 le chiavi di Atelier nello stesso giro — vedi sezione 2unvicies.
...
17. **Un testo DNA "corretto" in un solo punto non è un fix finché non si verifica che non sia duplicato altrove — cercare SEMPRE tutte le occorrenze prima di dichiarare un fix completo.** Scoperto 25/07: la specifica sbagliata di occhiali/barba ("tortoiseshell", "6-7cm") era hardcoded in `VALERIA_FACE` (shared) MA ANCHE, indipendentemente, in `review_and_fix()` (2 regole distinte, stesso file) e `sanitize_user_input()` (stesso file) — che reiniettavano la vecchia specifica come ultimo step prima di Flow, vanificando il fix su `VALERIA_FACE` da solo — E nei 3 blocchi locali `IDENTITY LOCK` di Atelier, completamente indipendenti da shared. Un `grep` mirato sulla stringa esatta ("tortoiseshell", "octagonal", "6-7cm") su TUTTI i file del progetto, non solo sul file appena modificato, ha rivelato la duplicazione — senza quel grep il fix sarebbe stato dichiarato "fatto" mentre in produzione il comportamento sarebbe rimasto identico per Vogue e Atelier (gli unici due bot che chiamano `review_and_fix()`). **Procedura da ripetere per qualunque futuro fix su testo DNA:** dopo aver corretto la fonte più ovvia, `grep -rn` della stringa esatta rimossa su tutti i file `.py` del progetto prima di considerare il fix completo.
18. **In una sessione lunga con molti turni, verificare SEMPRE lo stato reale dei file nell'area di lavoro (`ls`/`grep`/`view`) prima di dire "non ho X" o di ipotizzare cause esterne (editing in parallelo, altre sessioni) per una discrepanza inattesa.** Scoperto 30-31/07: Claude ha detto due volte a Walter di non avere più `C_shared100.py`/`Atelier_254.py` o l'HANDOFF/README, quando erano ancora presenti nell'area di lavoro della stessa chat da turni precedenti; in un caso ha anche attribuito un fix già scritto nel file (2.4.5, poi l'estensione al 429) a un fantomatico editing esterno "in parallelo" da parte di Walter — che invece usa solo questa chat. La causa reale era la gestione del proprio contesto su una conversazione molto lunga (dettagli di tool call precedenti compressi/non più visibili), non un fattore esterno. **Prima di affermare l'assenza di un file o ipotizzare un editor esterno, controllare sempre con `ls -la`/`grep` lo stato reale dell'area di lavoro.**
19. **`SHARED_DATE` (e qualunque data di bump/changelog) deve sempre essere la data odierna REALE nel momento in cui la modifica viene effettivamente applicata — mai una data dedotta dal contenuto della richiesta di Walter (timestamp di un log allegato, nome file di una foto, data citata in un messaggio) NÉ dedotta incrementando la data dell'ultimo file su cui si stava già lavorando.** Errore ripetuto più volte: i bump shared 2.4.4-2.4.8 sono stati datati usando le date nei materiali allegati da Walter. **Recidiva il 29/08:** un Update di HANDOFF/README è stato datato "14/08" — la data dell'ultimo file da cui si stava continuando — invece di controllare la data reale odierna, nonostante questa stessa lezione fosse già scritta. **Prima di scrivere qualunque data — in `SHARED_DATE`, in un changelog, nel campo "Generato"/"Ultimo aggiornamento" di HANDOFF/README — controllare sempre la data corrente reale del sistema, mai dedurla dal contesto o dall'ultimo file toccato.**
20. **Il nome file di un bot deve SEMPRE cambiare quando il file viene modificato, qualunque sia la modifica — mai consegnare un file con lo stesso nome aggiornando solo il changelog/VERSION interni.** Scoperto 01/08: Claude ha consegnato nuove versioni di `Atelier_254.py` e `Vogue_210.py` (supporto multi-soggetto) mantenendo lo stesso nome file, aggiornando solo `VERSION` e il changelog interno — in violazione diretta della regola già scritta in sezione 2 ("il numero nel nome file corrisponde sempre esattamente alla VERSION interna"). Corretto da Walter, che ha anche indicato di alzare a MINOR (non patch) dato che si trattava di una nuova capacità, non un fix — rinominati `Atelier_260.py`/`Vogue_220.py`. **Prima di consegnare qualunque file bot modificato, verificare che il nome del file consegnato rifletta la VERSION appena scritta al suo interno — mai lo stesso nome del file caricato da Walter.**
21. **Ogni volta che un file bot viene rinominato (regola #20), il Run command del servizio Koyeb corrispondente deve essere aggiornato ALLO STESSO nome file, altrimenti il deploy fallisce silenziosamente al prossimo redeploy — l'istanza vecchia continua a girare finché Walter non ricarica, poi crash immediato.** Scoperto 05/08: dopo tre rename consecutivi di Atelier (261 → 262 → 270) nella stessa sessione, Walter ha caricato `Atelier_270.py` su Koyeb ma il Run command del servizio era rimasto `python Atelier_261.py` — deploy fallito con `can't open file '/workspace/Atelier_261.py': No such file or directory`. Il codice consegnato era corretto (verificato: sintassi pulita, import ok) — causa puramente di configurazione Koyeb, non di codice. **Quando si consegna un file bot rinominato, ricordare esplicitamente a Walter di aggiornare anche il Run command su Koyeb allo stesso nome — non darlo per scontato.**
22. **Il comando "Update" deve aggiornare TUTTO il README, non solo la sezione narrativa "Fix robustezza"/TODO — in particolare l'intestazione "Ultimo aggiornamento", la tabella "Bot attivi" (nomi file, versioni) E il blocco "Struttura file" in cima al file, facili da dimenticare perché non fanno parte del changelog.** Scoperto 05/08: per tre Update consecutivi (01/08, 02/08, 05/08) Claude ha aggiornato correttamente la sezione "Fix robustezza" e i TODO, ma ha lasciato intestazione e tabella Bot attivi ferme al 25/07 — Walter se n'è accorto da uno screenshot di GitHub. **Recidiva il 10/08:** anche dopo aver corretto intestazione e tabella, il blocco di codice "Struttura file" (elenco file con commenti) era rimasto anch'esso fermo ai vecchi nomi (`Vogue_210.py`, `Architect_302.py`, `Atelier_254.py`) — un terzo punto mai controllato nonostante questa stessa lezione. **Ad ogni Update, controllare e aggiornare SEMPRE tutti e quattro questi punti: intestazione "Ultimo aggiornamento", tabella "Bot attivi", blocco "Struttura file", riga "Shared" (versione shared) — non solo "Fix robustezza"/TODO.**
...
25. **RISOLTO il 25/07 — segnale positivo parziale, non conclusivo.** shared 2.4.1: `VALERIA_FACE`, `review_and_fix()` (regole 2 e 4) e `sanitize_user_input()` non specificano più forma/lunghezza di occhiali e barba — rimandano alla foto di riferimento reale allegata da Walter in Flow, confermata da lui come sempre autorevole su questi due punti (era: "thin octagonal Vogue Havana dark tortoiseshell", "beard 6-7cm", in contraddizione diretta con la foto reale). Corretto anche un disallineamento indipendente trovato per caso: `VERSION` era rimasta a `2.3.18` mentre `SHARED_VERSION` (quella esportata) era già a `2.4.0` dal 17/07 — allineate entrambe a `2.4.1`. Vedi sezione 2duodevicies. Walter ha mostrato un mosaico generato dopo il fix: identità stabile sui 4 scatti, nessuna deriva su occhiali/barba. Un solo campione — non tratta come "risolto per sempre" (lezione #16), serve conferma su più generazioni nel tempo.

26. **RISOLTO il 25/07 — stesso segnale positivo parziale del punto 25.** Atelier 2.5.1: stessa correzione del punto 25 estesa ai 3 blocchi locali `⚠️ IDENTITY LOCK` (`build_full_prompt`, `build_shooting_prompt` singolo e mosaico), che avevano "tortoiseshell glasses" hardcoded indipendentemente da shared — stesso motivo, causa diversa (duplicazione locale, non shared). Vedi sezione 2duodevicies.

27. **Fuori scope su richiesta esplicita di Walter (25/07), non aperto come TODO:** `WALTER_DNA` in Surprise (feature Pride) e il testo "octagonal glasses printed as dark frames" nel comando LEGO minifig di Filtro hanno la stessa specifica vecchia (occhiali/barba hardcoded) dei punti 25-26, mai toccata. Walter non usa questi due bot al momento — nessuna azione richiesta finché non cambia idea.

28. **RISOLTO il 25/07, non ancora testato in produzione.** shared 2.4.2 + 2.4.3, Atelier 2.5.2: sfondi generati molto più poveri di dettaglio/densità rispetto alla foto originale (caso di test: tea-party surrealista pieno di orologi/lanterne/porcellane). Causa: campo BACKGROUND di `_ANALYZE_PROMPT` era l'unico senza richiesta di enumerazione oggetto-per-oggetto, a differenza di OUTFIT/ACCESSORIES — riscritto con lo stesso trattamento esaustivo (2.4.2), poi reso domain-agnostic con un secondo esempio per scene organiche dopo che Walter ha chiesto se il fix generalizzasse a scene non fatte di oggetti discreti come una giungla (2.4.3). Aggiunta anche clausola di densità in `BACKGROUND LOCK` lato Atelier (2.5.2). Vedi sezione 2duodevicies. Walter deve verificare su Flow, con la foto originale a confronto, se la densità dello sfondo generato è genuinely più vicina all'originale — sia su scene "da interno con oggetti" sia su scene organiche/naturali, dato che quest'ultimo caso non è ancora stato testato affatto (nessuna foto di giungla o simile mostrata finora).
...
34. **TODO aperto (30/07), priorità alta, non ancora testato in produzione.** shared 2.4.8: errori "Servizio Gemini non disponibile" ricorrenti da due settimane — diagnosticati con un log reale come 429/quota (20 richieste/giorno/chiave/modello esaurite su tutte le chiavi), non solo 503/overload come ipotizzato inizialmente. Fallback reattivo su `MODEL_LITE` (quota separata) esteso a qualunque errore transitorio. Walter deve verificare nei prossimi giorni se gli errori "Servizio Gemini non disponibile" si sono ridotti/eliminati nella fascia oraria segnalata (12/13-sera, Lisbona). Vedi sezione 2unvicies.

35. **RISOLTO e CONFERMATO in produzione il 02/08.** Supporto multi-soggetto (2+ persone nella foto di riferimento) — shared 2.4.9/2.4.10, Atelier 2.6.0/2.6.1, Vogue 2.2.0, Architect 3.1.0. Testato da Walter su foto reale con due donne: Architect e Vogue ok al primo colpo, Atelier singola ok, Atelier mosaico ha fallito al primo giro (figura secondaria con volto reale) — corretto con `VALERIA_MULTI_SUBJECT_LOCK` reso esplicito per-figura invece di un rimando indiretto. Su 12 immagini di verifica successive, il mosaico (caso peggiore) tiene il volto di Valeria su entrambe le figure. Vedi sezione 2duovicies.

36. **RISOLTO, non ancora testato in produzione.** shared 2.4.11 + Atelier 2.6.2: riga "Subject identity" del checklist mosaico resa condizionale tramite nuova `has_multiple_figures()` — prima restava testo statico/morto anche con una sola figura in foto. Vedi sezione 2trevicies.

37. **TODO — non ancora testato in produzione.** shared 2.4.12 + Atelier 2.7.0: header del prompt shooting editorial ora mostra il codice errore (503/429/transient) quando scatta il fallback su `MODEL_LITE`, con indicazione se conviene riprovare a breve (503) o se resta su lite fino alle 08:00 (429). Walter deve verificare che la nota compaia correttamente al prossimo fallback reale. Vedi sezione 2trevicies.

38. **Incidente di deploy 05/08, causa esterna non di codice — verificare risolto alla prossima sessione.** Dopo il rename di Atelier a `Atelier_270.py`, il Run command del servizio Koyeb `flexible-denna/atelier` era rimasto a `Atelier_261.py` (due rename indietro) — deploy fallito. Walter deve aver corretto il Run command su Koyeb a `python Atelier_270.py` e ridistribuito — confermare che il bot sia di nuovo online con la versione giusta. Vedi lezione #21 in sezione 11 e sezione 2trevicies.

39. **RISOLTO e CONFERMATO in produzione il 05-10/08.** Supporto video su Vogue — fork indipendente `Vogue_300.py` (poi `Vogue_310.py`), menu esplicito Immagine/Video su `/start`, `_ANALYZE_PROMPT_VIDEO`/`analyze_video()` isolati (shared non toccato). Testato da Walter con un video reale (mermaid tail garment): risultato "funziona egregiamente per essere una beta" — `PROPS & ACTIONS` ha catturato bene il momento dinamico della clip. Vogue 2.2 ora considerata archiviata, Vogue 3.1.0 è la versione di riferimento (Run command Koyeb aggiornato a `Vogue_310.py` — verificare che Walter l'abbia fatto, stessa lezione #21). Vedi sezione 2quattuorvicies.

40. **RISOLTO il 12/08 — non più applicabile.** L'input testuale di Vogue è stato rimosso del tutto (Vogue 4.2.0) su richiesta di Walter — restano solo I2V e V2V, quindi la domanda su come vincolarlo alla scelta Immagine/Video non si pone più.

41. **TODO — non ancora testato in produzione.** Vogue 4.3.0: `VALERIA_FACE_VIDEO`, prima descrizione testuale completa del volto (niente più foto reale allegata per il video, causa isolata di ~1/20 di successo). Walter deve verificare sia il tasso di accettazione da Flow sia — separatamente — se il volto generato risulta effettivamente riconoscibile come Valeria, non solo genericamente simile. Vedi sezione 2quinvicies.

42. **Osservazione permanente, non azionabile per ora.** Rilasciato Gemini 3.7 Flash (13/08) — miglioramenti dichiarati concentrati su coding/sviluppo web/lavoro agentico, nessun beneficio evidente per il caso d'uso reale (analisi visiva strutturata). Deciso di non passare a questo modello per ora: un modello appena lanciato tende ad avere più 503/overload nelle prime settimane per la domanda concentrata — lo stesso problema già affrontato con `gemini-3-flash-preview` (sezione 4). Nessuna modifica al pin `MODEL` in shared.

43. **Chiarito — nessuna azione necessaria.** Walter ha segnalato "due nuove versioni" trovate in `requirements.txt` da aggiornare; verificato su PyPI pacchetto per pacchetto (pyTelegramBotAPI, flask, Pillow, google-genai, openpyxl) — tutte già alla versione più recente, nessuna discrepanza. Chiarito da Walter: il file che aveva allegato era già la sua versione corretta con le due modifiche già fatte da lui — nessun disallineamento reale, nessuna modifica necessaria.
