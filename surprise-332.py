...
last_prompt    = {}
manual_state   = {}
processing_locks = {}  # uid → threading.Lock() per prevenire click duplicati
sent_steps = {}         # uid → set di (step_idx, page) già inviati — anti-duplicato
...
def send_manual_step(cid, uid, step_idx, page=0):
    # --- Anti-duplicato: evita di inviare lo stesso passo+pagina due volte ---
    key = (step_idx, page)
    if uid not in sent_steps:
        sent_steps[uid] = set()
    if key in sent_steps[uid]:
        return  # già inviato, ignora
    sent_steps[uid].add(key)
...
def cmd_start(message):
    ...
    sent_steps.pop(uid, None)  # reset anti-duplicato
...
def handle_callback(call):
    ...
    # --- Deduplicazione: ignora click multipli simultanei sullo stesso uid ---
    if uid not in processing_locks:
        processing_locks[uid] = threading.Lock()
    lock = processing_locks[uid]
    if not lock.acquire(blocking=False):
        return  # già in elaborazione, scarta il click duplicato
    ...
    try:
        ...
        # --- Paginazione ---
            ...
            if uid in sent_steps:
                sent_steps[uid] = {k for k in sent_steps[uid] if k[0] != step}
        ...
        # --- Selezione manuale ---
            ...
            if next_step < len(MANUAL_STEPS):
                ...
                if uid in sent_steps:
                    sent_steps[uid] = {k for k in sent_steps[uid] if k[0] != next_step}
