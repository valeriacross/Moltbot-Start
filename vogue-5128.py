    ...
    def run_task(idx):
        ...
        if res:
            ...
            try:
                ...
                if caption:
                    bot.send_message(call.message.chat.id, caption)
                # Faceswap: mostra prompt completo copiabile dopo la generazione
                if data.get('_is_faceswap') and idx == qty - 1:
                    prompt_display = data['full_p']
                    # Tronca a 3800 chars per sicurezza Telegram (limite 4096)
                    if len(prompt_display) > 3800:
                        prompt_display = prompt_display[:3800] + "\n[...]"
                    bot.send_message(call.message.chat.id,
                        f"📋 <b>Prompt faceswap:</b>\n<code>{html.escape(prompt_display)}</code>")
            except Exception as e:
