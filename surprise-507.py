...
def generate_caption_from_scenario(scenario):
    """Genera caption social con Gemini — 4/5 emoji + frase breve. Stile Atelier."""
    try:
        loc    = scenario.get('location', '')
        mood   = scenario.get('mood', '')
        sky    = scenario.get('sky', '')
        ...
        instr = (
            ...
            "Example: '🩰🍷🕊️🎞️🌑 dramatic white layers against a deep burgundy backdrop'\n"
            "Example: '🌊✨💎🎤🌃 shimmering turquoise beads under the stage lights'\n"
            "Example: '🌸🪨🩰🕯️✨ delicate pink lace within ancient stone alcove'"
        )
        if not client:
