...
# ─── EMOJI ART ────────────────────────────────────────────────────────────────
EMOJI_PALETTE = [
    ("😀",255,214,0),("😂",255,214,0),("😍",255,214,0),("😎",255,214,0),
    ("🤩",255,214,0),("😭",255,214,0),("😡",220,50,32),("🤢",90,180,80),
    ("😰",130,210,230),("😈",140,60,180),
    ("👦🏿",70,40,20),("👩🏾",120,70,35),("👧🏽",160,100,55),
    ("👴🏼",210,160,100),("👶🏻",245,210,175),
    ("🌿",50,140,50),("🍃",70,160,70),("🌱",80,170,60),
    ("🌲",30,100,30),("🌵",60,130,50),("🍀",60,150,60),
    ("🥬",80,170,80),("🫛",110,180,80),
    ("🍋",255,230,30),("🌽",240,200,30),("🍊",240,130,20),
    ("🍑",250,160,100),("🥭",250,140,20),("🍯",210,140,30),("🧀",250,210,80),
    ("🍎",200,30,30),("🍓",210,40,40),("🌹",190,20,50),
    ("🍒",160,20,40),("🍅",200,50,30),("🌶️",210,40,20),
    ("🍇",110,50,150),("🫐",80,80,180),("🍆",90,30,110),
    ("🍫",100,50,20),("🍪",180,120,60),("🥐",200,150,80),
    ("🍞",210,165,90),("🥜",180,130,60),("🍄",140,80,40),
    ("🌊",30,120,200),("💧",60,150,220),("🫧",180,220,240),
    ("🌀",50,100,200),("🔵",30,100,220),("💙",50,130,220),
    ("🔮",130,60,180),("💜",140,60,200),
    ("⬜",240,240,240),("☁️",220,230,240),("🌫️",190,200,210),
    ("🏔️",200,210,220),("🤍",245,245,245),("🌸",250,180,200),
    ("🐘",150,150,165),("🌑",80,80,90),("⬛",30,30,35),
    ("🖤",20,20,25),("🐾",100,80,70),
    ("🎱",20,20,20),("🐈‍⬛",25,25,30),
    ("🌺",240,100,140),("🩷",240,120,160),("🌷",230,100,130),("🐷",240,160,170),
    ("❤️",210,30,50),("🔴",200,30,30),("🎈",230,50,50),
    ("🟠",240,120,20),("🦊",230,110,30),("🎃",230,110,20),("🔶",250,150,40),
    ("🌟",255,230,50),("⭐",255,220,30),("🌻",250,210,20),("🟡",240,210,30),
    ("🟢",50,170,60),("🐸",80,160,60),("🥑",80,130,60),
    ("🩵",100,200,230),("🌐",70,160,210),("🧊",160,220,240),
    ("🪵",130,80,40),("🌰",140,80,30),("🐻",150,100,50),("🦫",140,90,50),
    ("🏆",220,170,50),("👑",220,180,40),("🥇",220,180,60),
    ("🥈",180,185,195),("⚙️",160,165,170),
    ("🔥",220,80,20),("💥",230,90,30),
]
_EMOJI_PALETTE_BUILT = [(r, g, b, e) for e, r, g, b in EMOJI_PALETTE]

def _closest_emoji(r, g, b):
    best, best_dist = "⬛", float("inf")
    for pr, pg, pb, em in _EMOJI_PALETTE_BUILT:
        d = (2*(r-pr)**2 + 4*(g-pg)**2 + 3*(b-pb)**2) ** 0.5
        if d < best_dist:
            best_dist, best = d, em
    return best

def _run_emoji_art(chat_id, uid, username, img_bytes):
    """Converte l'immagine in emoji art usando pilmoji e la invia come foto PNG."""
    try:
        from PIL import Image as PILImage
        from pilmoji import Pilmoji

        wait = bot.send_message(chat_id, "🎨 <b>Genero la Emoji Art...</b>\n⏳ Qualche secondo.")
        ...
        w, h = src.size
        cols = min(w // BLOCK, MAX_COLS)
        rows = int(cols * h / w)
        src = src.resize((cols * BLOCK, rows * BLOCK), PILImage.LANCZOS)

        # Calcola griglia emoji
        grid = []
        for row in range(rows):
            line = []
            for col in range(cols):
                box = (col*BLOCK, row*BLOCK, (col+1)*BLOCK, (row+1)*BLOCK)
                avg = src.crop(box).resize((1,1), PILImage.LANCZOS).getpixel((0,0))
                line.append(_closest_emoji(avg[0], avg[1], avg[2]))
            grid.append(line)

        # Rendering con pilmoji
        out = PILImage.new("RGB", (cols * BLOCK, rows * BLOCK), (17, 17, 17))
        with Pilmoji(out) as pilmoji_draw:
            for r_idx, row in enumerate(grid):
                for c_idx, em in enumerate(row):
                    pilmoji_draw.text((c_idx * BLOCK, r_idx * BLOCK), em, font_size=BLOCK)
        ...
        bot.send_photo(chat_id, buf,
            caption=f"🎨 <b>Emoji Art</b> — {cols}×{rows} emoji",
            parse_mode="HTML")
        logger.info(f"✅ emoji_art inviata a {username} ({cols}×{rows})")

    except Exception as e:
        logger.error(f"❌ _run_emoji_art: {e}", exc_info=True)
        try: bot.delete_message(chat_id, wait.message_id)
        except Exception: pass
        bot.send_message(chat_id,
            f"❌ <b>Emoji Art fallita.</b>\n<code>{html.escape(str(e))}</code>",
            parse_mode="HTML")
