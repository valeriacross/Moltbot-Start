...
## Fix robustezza (20/06/2026 → 02/08/2026)

**Il 01-02/08 — supporto foto di riferimento con 2+ soggetti distinti (m+f, m+m, f+f...).** Su richiesta di Walter, rilevamento automatico (non un comando): nuovo campo `FIGURES` in `_ANALYZE_PROMPT` (conta le persone, posa/posizione/interazione, cieco sull'identità), OUTFIT/ACCESSORIES/BODY ART strutturabili per figura, nuova `VALERIA_MULTI_SUBJECT_LOCK`/`multi_subject_clause()` (stesso pattern di `body_art_clause()`) — shared 2.4.9. Applicato a Vogue (2.2.0) e Atelier (2.6.0); Architect (3.1.0) invece rende `SUBJECT` multi-figura consapevole senza nessun lock, perché non inietta DNA — descrive fedelmente le persone reali. Bump di MINOR su tutti e tre (nuova capacità, non fix) — file rinominati `Vogue_220.py`/`Atelier_260.py`/`Architect_310.py` (prima consegna aveva erroneamente mantenuto i vecchi nomi file, corretto da Walter). Test in produzione: Architect, Vogue e Atelier modalità singola ok al primo colpo; Atelier mosaico ha fallito (figura secondaria con volto reale invece di Valeria) — diagnosticato in due cause (lock troppo indiretto + riga "Subject identity" del checklist mosaico al singolare) e corretto in shared 2.4.10 + Atelier 2.6.1 (patch). **Confermato risolto in produzione il 02/08** su 12 immagini di verifica, mosaico incluso (caso peggiore, comunque accettabile).

**Il 30/07 — errori "Servizio Gemini non disponibile" ricorrenti da due settimane (12/13-sera, Lisbona), tutti i bot.** Diagnosi iniziale (ricerca web + codice): 503 "model overloaded", correlati al carico globale — il retry esistente ruota le chiavi ma non aiuta contro un overload lato server condiviso da tutte. Un log di produzione allegato da Walter ha però mostrato che il sintomo reale era un **429 RESOURCE_EXHAUSTED** (quota free tier, 20 richieste/giorno/chiave/modello, esaurita su tutte e 5 le chiavi di Atelier nello stesso giro) — non (solo) overload. Aggiunto un fallback reattivo su `gemini-3.1-flash-lite` (quota separata, non toccata): su qualunque errore transitorio (503, 429, timeout, connessione) ritenta subito con il modello lite sulla stessa chiave, prima di ruotare (shared 2.4.8, dopo due iterazioni intermedie 2.4.6/2.4.7 — dettagli in HANDOFF, sezione 2unvicies). Non ancora testato in produzione.

**Il 28/07 — torso reso maschile su scene a pelle scoperta, tutto l'ecosistema.** Walter ha mostrato un mosaico poolside: volto/barba/occhiali corretti, ma torso completamente maschile (peloso, senza seno) nonostante FULL D-CUP BUST e COEXISTENCE già presenti, e nonostante i PROPS del prompt menzionassero esplicitamente "breasts" — a differenza del caso mosaico (corpo coperto da tuta), qui il torso era nudo/scoperto, nessun tessuto a mediare la forma. Confermato da Walter come pattern ricorrente su scene simili (spiaggia, doccia), non isolato. Segnalata anche una correlazione con la variante Gemini scelta in Flow ("nano pro" molto più spesso soggetto al problema di "nano 2"/"nano light") — verificato che è una scelta manuale di Walter in Flow, non un parametro di codice: nessuna leva qui. Su richiesta esplicita di Walter di non legare il fix a scene specifiche: aggiunta una riga incondizionata dopo COEXISTENCE sia in `VALERIA_BODY_STRONG` ("BUST VOLUME — ALWAYS VISIBLE") sia in `VALERIA_BODY_SAFE` ("SILHOUETTE — ALWAYS VISIBLE") — shared 2.4.5. Non ancora testato in produzione.

**Il 27/07 — pattern/texture semplificati rispetto all'originale, tutto l'ecosistema. Confermato risolto in produzione.** Walter ha allegato foto originale + mosaico generato (bodysuit a mosaico fine tipo "specchio infranto", reso invece come pochi grandi rombi piatti — sfondo corretto, solo il pattern del capo era il problema). Prima ipotesi (negative prompt) scartata su richiesta di Walter — Gemini non li considera in generazione. Aggiunta nuova costante generica e in positivo `VALERIA_TEXTURE_LOCK` (shared 2.4.4), che richiede granularità fine per qualunque pattern/texture descritta, a prescindere dalla scena. Verificato che Atelier importa `VALERIA_DNA` ma non lo usa mai — solo `build_valeria_identity()` — quindi la costante è stata inserita in entrambi i punti per arrivare davvero a tutti i bot. Walter ha confermato su un mosaico successivo: pattern a frammenti fini, coerente con l'originale.
