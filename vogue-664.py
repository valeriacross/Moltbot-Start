...
def handle_confirmation(call):
    ...
    # img_caption è presente nel faceswap (img=None), altrimenti usa img
    img_for_caption = data.get('img_caption') or data.get('img')
...
def ask_confirmation(m):
    ...
    if m.content_type == 'photo':
        ...
        if not user_text or len(user_text.strip()) < 30:
            ...
            # img=None → generazione da zero (no faceswap meccanico)
            # img_caption=img_data → usata solo per caption post-generazione
            pending_prompts[uid] = {
                'full_p': faceswap_prompt,
                'qty': qty,
                'img': None,
                'img_caption': img_data,
            }
...
def handle_choice(call):
    ...
    if call.data == "choice_faceswap":
        ...
        def do_faceswap():
            ...
            # img=None → generazione da zero (no faceswap meccanico)
            # img_caption=img_data → usata solo per caption post-generazione
            pending_prompts[uid] = {'full_p': faceswap_prompt, 'qty': qty, 'img': None, 'img_caption': img_data}
            user_last_prompt[uid] = {'full_p': faceswap_prompt, 'img': None}
