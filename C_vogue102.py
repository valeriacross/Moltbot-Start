...
def generate_caption(img_bytes):
    """Analizza foto e genera caption: 5 emoji + 5/10 parole in inglese."""
    if not gemini.available:
        return None, "⚠️ API key non configurata."
    try:
        img_part = genai_types.Part.from_bytes(data=img_bytes, mime_type="image/jpeg")
        prompt = (
            ...
            "Format: exactly 5 relevant emojis followed by 5 to 10 words.\n"
            "The emojis come first, all together, then the words.\n"
            "Example: ✨🖤👗🌙📸 Dark elegance in every carefully chosen detail.\n"
            "Output ONLY the caption — no explanations, no quotes, no extra text."
        )
        result = gemini.generate(prompt, contents=[img_part])
        if result:
        ...
        logger.error(f"❌ Caption fallita: {e}")
        return None, f"❌ Errore caption: {html.escape(str(e))}"
...
def handle_photo(m):
    ...
    if pending_caption.pop(uid, False):
        ...
        caption, err = generate_caption(img_bytes)
        try: bot.delete_message(m.chat.id, wait.message_id)
        except Exception: pass
        if not caption:
            bot.send_message(m.chat.id, err or "❌ Caption fallita. Riprova.")
        else:
