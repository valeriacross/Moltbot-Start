...
def generate_caption_from_image(img_bytes):
    """Genera caption social guardando l'immagine originale — più accurata del testo."""
    try:
        img_part = genai_types.Part.from_bytes(data=img_bytes, mime_type="image/jpeg")
        response = client.models.generate_content(
            model=MODEL_TEXT_ID,
            contents=[
                ...
                "1 complete sentence in English, 4-6 emojis, evocative fashion tone, "
                "mention the specific outfit or location visible in the image. "
...
def ask_confirmation(m):
    ...
    if m.content_type == 'photo':
        ...
        if not user_text or len(user_text.strip()) < 30:
            ...
            def _finalize_batch(uid=uid, cid=m.chat.id, username=username):
                ...
                for pos, (i, full_p) in enumerate(ok):
                    caption_pre = generate_caption_from_image(photos[i]) if i < len(photos) else generate_caption_from_text(full_p)
                    if caption_pre:
                        bot.send_message(cid, f"{caption_pre}\n#genderfluid #selfexpression #bodypositivity")
        ...
        if not user_text or len(user_text.strip()) < 30:
            ...
            caption_fs = generate_caption_from_image(img_data)
            markup = types.InlineKeyboardMarkup()
            ...
            if caption_fs:
                bot.send_message(m.chat.id, f"{caption_fs}\n#genderfluid #selfexpression #bodypositivity")
