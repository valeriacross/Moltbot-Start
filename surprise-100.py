...
Your task is to generate a unique, vivid scene description for an image featuring Valeria Cross.

Rules:
- Choose a REAL iconic world location (famous landmark, capital city, natural wonder, etc.)
- Choose a sky/lighting condition (bright sun, golden sunset, stormy night, aurora borealis, galaxy, etc.)
- Choose a complete outfit from high-fashion runways or Victoria's Secret — include swimwear and lingerie
- Choose a visual style from either a painter (Magritte, Dalì, De Chirico, Mondrian, Banksy, Klimt, Warhol, Basquiat, Hockney, Matisse...) or a photographer (Helmut Newton, Steve McCurry, Annie Leibovitz, Mario Testino, Peter Lindbergh, Herb Ritts, David LaChapelle...)
- Be specific and evocative — name the exact location, describe the exact outfit in detail
- No generic descriptions — every element must be concrete and vivid
...
def generate_scenario():
    try:
        ...
        # Pulisce eventuali backtick JSON
        if text.startswith("```"):
            text = text.split("```")[1]
            if text.startswith("json"):
                text = text[4:]
        import json
        scenario = json.loads(text.strip())
        return scenario
    except Exception as e:
...
def generate_caption(scenario):
    try:
        prompt = (
            f"Write a minimal social media caption (max 6 words + 2-3 emojis) "
            f"for a fashion photo at {scenario['location']} in style of {scenario['style']}. "
