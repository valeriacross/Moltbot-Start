...
def ask_confirmation(m):
    ...
    # Ottimizzazione in thread separato con timeout di 30 secondi
    ...
    # Fallback al build_master_prompt originale se ottimizzazione fallisce
    if not optimized_prompt:
        logger.warning(f"⚠️ Ottimizzazione fallita per {username}, uso build_master_prompt diretto. Errore: {opt_err}")
        optimized_prompt = build_master_prompt(user_text, ar_snapshot)
        bot.send_message(m.chat.id, "⚠️ <b>Ottimizzazione non disponibile, uso prompt standard.</b>")

    # Sanitizza il prompt: rimuove caratteri che possono causare MALFORMED_FUNCTION_CALL
    sanitized_prompt = optimized_prompt.replace("{", "(").replace("}", ")").replace("[", "(").replace("]", ")")

    pending_prompts[uid] = {
        'full_p': sanitized_prompt,
        'qty': user_qty[uid],
        'img': img_data
    }
    ...
    preview_prompt = sanitized_prompt
    max_len = 3500
