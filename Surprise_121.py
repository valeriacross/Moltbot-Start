...
# Pool outfit per colore — 3 varianti per pannello, pescate random a ogni lancio
_FLAG_OUTFITS = {
    "P": [  # Rosso
        "strapless scarlet red satin bubble-skirt mini dress, dramatic tiered ruffle hem",
        "flowing deep red chiffon sundress with asymmetric hemline caught in wind, spaghetti straps",
        "cherry red off-shoulder mini dress with full skirt, lightweight organza layers billowing",
    ],
    "R": [  # Arancione
        "off-shoulder tangerine organza mini dress, full billowing skirt in lightweight silk",
        "orange strapless dress with exaggerated puffball skirt and wide flutter sleeves",
        "burnt orange wrap dress with tiered ruffle skirt, fabric spinning in movement",
    ],
    "I": [  # Giallo
        "bright lemon-yellow halter sundress with full layered tulle skirt, air-filled volume",
        "sunshine yellow strapless gown with tiered ruffles from waist to hem, fabric billowing",
        "canary yellow one-shoulder mini dress, asymmetric hem, light chiffon skirt floating",
    ],
    "D": [  # Verde
        "emerald green backless satin midi-length dress with dramatic fabric movement",
        "forest green deep-V wrap dress with chiffon overlay panels billowing outward",
        "jade green strapless mini dress, full tulle underskirt, lightweight overlay flying",
    ],
    "E": [  # Blu
        "cobalt blue one-shoulder silk charmeuse dress with cascade waterfall drape",
        "royal blue strapless structured bodice with massive full skirt, fabric exploding outward",
        "sapphire blue flutter-sleeve mini dress, layered chiffon skirt catching the breeze",
    ],
    "!": [  # Viola
        "deep purple organza dress with dramatic full sleeves and billowing skirt",
        "violet silk-satin mini with structured bustier bodice and full skirt exploding outward",
        "lavender-purple strapless dress, giant ruffled skirt layers, ultra-light fabric in motion",
    ],
}

# Pose fisse per ogni pannello (non variano — definiscono il ritmo visivo del mosaico)
_FLAG_POSES = {
    "P": "standing confident, one hand on hip, slight body lean forward, direct gaze to camera, chin slightly raised",
    "R": "caught mid-twirl, dress spinning dramatically outward, arms slightly extended, hair lifting, laughing with joy",
    "I": "both arms raised above head in celebration, standing on tiptoes, wide open smile, playful energy",
    "D": "stepping dynamically forward, one leg extended in confident stride, one hand gesturing outward, powerful stance",
    "E": "leaning slightly back, head gently tilted, eyes half-closed in pure joy, one hand softly touching hair lifted by breeze",
    "!": "full celebratory jump — both feet off ground, arms wide open, maximum joy, dress and hair in mid-air movement",
}
...
def _build_flag_prompt() -> str:
    """Costruisce il prompt PRIDE! mosaic — zero Gemini, outfit random per ogni pannello."""
    import random
    ...
    for letter in _FLAG_PANEL_ORDER:
        ...
        pose = _FLAG_POSES[letter]
        panels_desc += (
            f"\n**Panel {letter} — {color_name.upper()}**\n"
            f"Background: #{bg} · Letter: #{fg} (pastel {color_name.lower()})\n"
            f"Dress: {outfit}\n"
            f"Pose: {pose}\n"
        )
    ...
    prompt = (
        ...
        "Generate a single image composed of exactly 6 vertical panels arranged in a 3×2 grid.\n"
        "Row 1 (top): panels P · R · I\n"
        "Row 2 (bottom): panels D · E · !\n"
        "Each panel has identical dimensions (1:1.4 portrait ratio). "
        "No borders, no gaps, no frames between panels — seamless tiling.\n\n"
        "---\n\n"
        "**FACE (PROVIDED AS REFERENCE)**\n"
        "Use the face from the attached reference image across all 6 panels with ZERO identity drift. "
        "Same face, same features, same expression style in every panel.\n\n"
        "---\n\n"
        "**BODY — DO NOT OVERRIDE:**\n"
        "Feminine hourglass silhouette. Height: 180 cm. Full bust (D-cup), defined waist, full rounded hips. "
        "Smooth skin throughout — NO body hair, NO chest hair, NO arm hair, NO leg hair. "
        "Long slim legs. Natural skin texture.\n\n"
        "---\n\n"
        "**PANEL TEMPLATE (apply to all 6):**\n"
        "- Background: solid flat matte color (see palette per panel)\n"
        "- Giant letter: ultra-bold rounded sans-serif font, fills approximately 65% of panel height, "
        "positioned center-right, slightly lighter/pastel version of background color, "
        "semi-transparent layer behind subject\n"
        "- Subject: full-body portrait, overlapping the letter (subject in front, letter partially behind), "
        "head to feet visible, editorial fashion photography\n"
        "- Dress: summer flowy lightweight mini dress matching panel color — "
        "billowing as if caught by a gentle warm breeze, movement visible in fabric\n"
        "- Shoes: strappy heeled sandals, metallic silver or matching panel color\n"
        "- Expression: joyful, celebratory, Pride energy — each panel unique\n\n"
        "---\n\n"
        f"**PANELS — COLOR · LETTER · OUTFIT · POSE:**{panels_desc}\n"
        "---\n\n"
        "**TYPOGRAPHY RULES (apply to all panels):**\n"
        "- Font style: ultra-bold geometric rounded sans-serif (Nunito Black, Futura Heavy or similar)\n"
        "- Letter occupies top-to-bottom ~65% of panel height, sits center-to-right\n"
        "- Letter color: pastel version of background (see per panel above)\n"
        "- Clean flat graphic look — no drop shadow, no outline\n"
        "- The ! panel: exclamation mark same style and size as the letters\n\n"
        "---\n\n"
        "**CONSISTENCY ACROSS ALL 6 PANELS:**\n"
        "- Identical aspect ratio per panel, seamlessly tiled — no seams visible\n"
        "- Same bright clean high-key fashion editorial lighting across all panels\n"
        "- Same body proportions and face identity in every panel\n"
        "- Background: flat solid color, no gradients, no texture\n\n"
        "---\n\n"
        "**TECHNICAL:**\n"
        "Format: 3:2.8 landscape (3 panels wide × 2 rows)\n"
        "Quality: 8K, sharp, fashion editorial\n"
        "Lighting: bright soft studio-style, even across subject, no harsh shadows\n"
        "NEGATIVE: face drift between panels, inconsistent body proportions, "
        "visible borders or seams between panels, dark moody lighting, "
        "casual non-fashion clothing, body hair of any kind, flat lifeless fabric\n\n"
...
def _flag_send_prompt(cid, uid):
    """Costruisce e invia il prompt PRIDE! flag — zero Gemini."""
    prompt = _build_flag_prompt()
    flag_last_prompt[uid] = prompt

    # Mini prompt da parser locale (zero API)
    mini, _ = generate_mini_prompt(prompt)
    if mini:
        bot.send_message(cid,
            f"🏳️\u200d🌈 <b>PRIDE! Flag</b>\n{mini}",
            parse_mode="HTML")
    ...
    bot.send_message(cid, "🏳️\u200d🌈 <b>PRIDE! Mosaic Prompt</b>\n\nCopia e incolla su Flow:")
    CHUNK = 3800
    chunks = [prompt[i:i+CHUNK] for i in range(0, len(prompt), CHUNK)]
    for idx, chunk in enumerate(chunks):
        header = "" if len(chunks) == 1 else f"<i>({idx+1}/{len(chunks)})</i>\n"
    ...
    logger.info(f"🏳️‍🌈 /flag prompt inviato a uid={uid} ({len(prompt)} chars)")
