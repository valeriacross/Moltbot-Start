import os, io, threading, logging, flask, telebot, html, time, random
...
def _run_stereo(chat_id, uid, username, img_bytes):
    t_start = time.time()
    logger.info(f"   🎥 Inizio generazione stereo | {username}")
    try:
        img_part = genai_types.Part.from_bytes(data=img_bytes, mime_type="image/jpeg")
        contents = [STEREO_PROMPT, img_part]
        ...
        response = client.models.generate_content(
            ...
            config=genai_types.GenerateContentConfig(
                ...
                image_config=genai_types.ImageConfig(image_size="2K", aspect_ratio="21:9"),
        ...
        err_markup.add(types.InlineKeyboardButton("🔁 Riprova", callback_data="stereo_retry"))
        err_markup.add(types.InlineKeyboardButton("🏠 Menu", callback_data="stereo_menu"))

        if not response.candidates:
            bot.send_message(chat_id, "❌ Nessun risultato dall'API. Riprova.", reply_markup=err_markup)
            return
        ...
        for part in candidate.content.parts:
            if part.inline_data:
                ...
                stereo_last_img[uid] = img_bytes  # salva per retry
                result_markup = types.InlineKeyboardMarkup()
                result_markup.add(types.InlineKeyboardButton("🔁 Riprova stessa foto", callback_data="stereo_retry"))
                ...
                bot.send_document(chat_id, io.BytesIO(img_out),
                    visible_file_name="stereo_3d.jpg",
                    caption=f"🎥 3D Stereo — {elapsed}s\n👁️ Incrocia gli occhi per vedere la profondità",
                    reply_markup=result_markup)
                logger.info(f"   ✅ Stereo inviato a {username} in {elapsed}s")
                return

        bot.send_message(chat_id, "❌ Nessuna immagine nella risposta. Riprova.", reply_markup=err_markup)
...
# --- MIRROR OUTFITS ---
# Pool outfit copiato da sorpresa-210.py — aggiornare entrambi in sync
MIRROR_OUTFIT_TOP = [
    ...
    "tiered ruffle organza top, dramatic volume",
    "silver chainmail mesh top, cowl neckline, draped loose fit",
    "black fitted tank bodysuit, deep scoop neckline, sleeveless",
    "cropped black ribbed turtleneck long sleeve, midriff exposed",
    "white fitted ribbed short sleeve t-shirt, crew neck",
    "denim corset bustier top, sweetheart neckline, ruffle cap sleeves, button front",
    "oversized black blazer worn open over bare chest",
    "metallic fuchsia blazer cape, worn off shoulders",
]
MIRROR_OUTFIT_BOTTOM = [
    ...
    "fitted wrap skirt with deep side slit, above knee",
    ...
    "white high-waisted pleated micro mini skirt, knife pleats",
    "blush pink ribbed pleated micro mini skirt",
    "floral ruffle tiered mini skirt, ditsy print, lace trim",
]
MIRROR_OUTFIT_INTERO = [
    "one-shoulder white polka dot mini dress with neck scarf tie, ruched silhouette",
    "silver metallic chainmail mini dress with pearl embellishments",
    "white blue floral zip-front halter mini dress, gold zip detail",
    "black floral print bodycon swimsuit dress, thick straps, sweetheart neckline",
    "strapless ivory polka dot mini dress with black lace trim and flared skirt",
    "white long-sleeve fitted one-piece swimsuit, high cut leg",
    "gold metallic bandage midi dress, sleeveless, backless with rear zipper",
    "lime green latex strapless bandeau mini dress",
    "champagne silk slip mini dress, spaghetti straps, gathered bust",
    "ivory structured strapless mini dress with bow detail at bust",
    "red pleated mini sundress, deep V plunge neckline, thin spaghetti straps",
    "hot pink strapless bandeau tube mini dress, ruched stretch fabric",
    "sage green lace spaghetti strap mini dress, backless with tie detail",
    "metallic fuchsia bustier mini dress with matching blazer cape",
    "black ribbed tank leotard with sheer black tights",
    "white halter neck zip-front mini dress with ditsy blue floral print",
]

def _mirror_random_outfit():
    """Estrae un outfit casuale dal pool — intero o top+bottom."""
    import random
    # 40% probabilità di outfit intero
    if random.random() < 0.4:
        return random.choice(MIRROR_OUTFIT_INTERO)
    else:
        top = random.choice(MIRROR_OUTFIT_TOP)
        bottom = random.choice(MIRROR_OUTFIT_BOTTOM)
        return f"{top} with {bottom}"

def _build_mirror_prompt(img_bytes):
    """Genera il prompt Mirror con 4 outfit casuali diversi."""
    import random
    outfits = []
    pool_intero = MIRROR_OUTFIT_INTERO.copy()
    pool_top = MIRROR_OUTFIT_TOP.copy()
    pool_bottom = MIRROR_OUTFIT_BOTTOM.copy()
    random.shuffle(pool_intero)
    random.shuffle(pool_top)
    random.shuffle(pool_bottom)

    for i in range(4):
        if i < 2 and pool_intero:
            outfits.append(pool_intero.pop())
        else:
            if pool_top and pool_bottom:
                outfits.append(f"{pool_top.pop()} with {pool_bottom.pop()}")
            elif pool_intero:
                outfits.append(pool_intero.pop())
            else:
                outfits.append(random.choice(MIRROR_OUTFIT_INTERO))

    return (
        "Analyze the provided image carefully and extract the subject's exact facial identity, "
        "hair, skin tone, age, and body proportions. Preserve all facial features with absolute fidelity.\n\n"
        "Generate a single photorealistic image showing a luxury vanity dressing room. "
        ...
        "Each of the 4 mirror panels reflects a different version of the subject wearing a completely different outfit.\n\n"
        "The 4 mirror panels must show:\n"
        f"PANEL 1 (leftmost, angled left): {outfits[0]}\n"
        f"PANEL 2 (center-left, slightly angled left): {outfits[1]}\n"
        f"PANEL 3 (center-right, slightly angled right): {outfits[2]}\n"
        f"PANEL 4 (rightmost, angled right): {outfits[3]}\n\n"
        "Each panel shows the same face but a different outfit, different expression, "
        "slightly different angle — as if showing 4 parallel versions of the same person.\n\n"
        ...
        "- Each panel reflection is sharp and clearly shows the outfit\n"
...
def handle_mirror_start(call):
    ...
    bot.send_message(call.message.chat.id,
        ...
        "Genererò un'immagine con 4 riflessi allo specchio, ognuno con un outfit diverso.")

def _run_mirror(chat_id, uid, username, img_bytes):
    t_start = time.time()
    logger.info(f"   🪞 Mirror Outfits | {username}")
    try:
        ...
        response = client.models.generate_content(
            ...
            config=genai_types.GenerateContentConfig(
                response_modalities=["IMAGE"],
                image_config=genai_types.ImageConfig(image_size="2K", aspect_ratio="4:3"),
                safety_settings=[{"category": c, "threshold": "BLOCK_NONE"} for c in
                                  ["HARM_CATEGORY_SEXUALLY_EXPLICIT", "HARM_CATEGORY_HATE_SPEECH",
                                   "HARM_CATEGORY_HARASSMENT", "HARM_CATEGORY_DANGEROUS_CONTENT"]]
            )
        ...
        elapsed = round(time.time() - t_start, 1)
        err_markup = types.InlineKeyboardMarkup()
        err_markup.add(types.InlineKeyboardButton("🔁 Riprova", callback_data="mirror_retry"))
        err_markup.add(types.InlineKeyboardButton("🏠 Menu", callback_data="mirror_menu"))
        ...
        for part in candidate.content.parts:
            if part.inline_data:
                img_out = part.inline_data.data
                mirror_last_img[uid] = img_bytes
                result_markup = types.InlineKeyboardMarkup()
                result_markup.add(types.InlineKeyboardButton("🔁 Riprova stessa foto", callback_data="mirror_retry"))
                ...
                bot.send_document(chat_id, io.BytesIO(img_out),
                    visible_file_name="mirror_outfits.jpg",
                    caption=f"🪞 Mirror Outfits — {elapsed}s",
                    reply_markup=result_markup)
                logger.info(f"   ✅ Mirror inviato a {username} in {elapsed}s")
                return
        bot.send_message(chat_id, "❌ Nessuna immagine nella risposta. Riprova.", reply_markup=err_markup)
    except Exception as e:
        elapsed = round(time.time() - t_start, 1)
...
def cmd_mosaic(m):
    ...
    logger.info(f"🖼️ /mosaic da {m.from_user.username or m.from_user.first_name} (id={uid})")
    bot.send_message(m.chat.id,
...
@app.route('/')
def health():
    return f"Filtro v{VERSION} Online"

if __name__ == "__main__":
    logger.info(f"🟢 Avvio Filtro Bot v{VERSION}")
    threading.Thread(
