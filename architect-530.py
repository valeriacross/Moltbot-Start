...
# --- CONFIGURAZIONE ---
VERSION = "5.3 (Synthetix - Signature Edition)"
...
# --- FIRMA MANDATORIA ---
WATERMARK_SPECS = """
Include a subtle watermark signature reading:
"feat. Valeria Cross 👠"
Style: elegant handwritten cursive, thin/medium stroke, fashion-editorial vibe
Color: warm off-white / champagne, opacity 88–92%
Placement: bottom center or bottom left, discreet, very small font size, non-intrusive.
"""
...
def generate_monolith_prompt(user_input, engine):
    try:
        instr = (
            ...
            "FUSE the user idea with Valeria Cross's identity into a single technical document. "
            "MANDATORY: You must replace any other watermark, signature, or logo with the following exclusive signature:\n"
            f"{WATERMARK_SPECS}\n"
            "The signature must be the final element of the prompt. "
            "Structure: IDENTITY -> FACE/HAIR -> SCENE CONCEPT -> TECHNICAL PARAMS -> WATERMARK -> NEGATIVES.\n"
            f"DNA: {VALERIA_DNA}\nIDEA: {user_input}\n"
            "Style: Prolix, Technical, Vogue Editorial. NO bullet points."
        )
...
@app.route('/')
def h(): return f"ARCHITECT_V5.3_ONLINE"
