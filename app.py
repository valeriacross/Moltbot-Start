...
def avvia_bot():
    try:
        ...
        def genera_valeria(m, prompt_text, images=None):
            wait = bot.reply_to(m, "📸 Moltbot in azione con Nano Banana...")
            try:
                # Costruzione contenuti multimodali
                contents = [SYSTEM_PROMPT, f"Genera questa scena: {prompt_text}"]
                if images:
                    contents.extend(images)

                # Chiamata al modello con i nomi delle categorie corretti
                ...
                if image_sent:
                    ...
                    bot.edit_message_text("⚠️ Il modello ha filtrato l'immagine (troppo esplicita o non generata).", m.chat.id, wait.message_id)
        ...
        def handle_photo(m):
            try:
                ...
                # Creazione corretta della parte immagine per Gemini
                ...
                bot.reply_to(m, f"❌ Errore nel processare la foto: {e}")

        @bot.message_handler(func=lambda m: True)
        def handle_text(m):
            genera_valeria(m, m.text)

        print("Moltbot Online - Fix Safety Settings applicato.")
        bot.infinity_polling(skip_pending=True)
    
    except Exception as e:
        print(f"CRASH FATALE: {e}")
