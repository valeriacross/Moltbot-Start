...
B2 = "BLOCK 2 (Subject & Face): Nameless Italian transmasculine avatar. Body: soft feminine, harmonious hourglass, prosperous full breasts (cup D), 180cm, 85kg. Body hairless. FACE: Male Italian face, ~60 years old, ultra-detailed skin (pores, wrinkles, bags). Expression: calm, half-smile, NO teeth. Beard: light grey/silver, groomed, 6–7 cm. Glasses MANDATORY: thin octagonal Vogue, Havana dark."
B3 = "BLOCK 3 (Hair & Technique): HAIR: Light grey/silver. Short elegant Italian style, volume. Nape exposed. Top <15 cm. IMAGE CONCEPT: High-fashion magazine photoshoot, 8K, cinematic realism. CAMERA: 85mm, f/2.8, ISO 200, 1/160s. Focus on face/torso. Shallow depth of field, natural bokeh."
B4 = "BLOCK 4 (Rendering & Output): RENDERING: Subsurface Scattering, Global Illumination, Fresnel, Frequency separation on skin. Watermark: 'feat. Valeria Cross 👠' (elegant cursive, champagne, bottom center/left, very small size, opacity 90%)."
...
def start(m):
    ...
    bot.send_message(m.chat.id, f"<b>🏛️ Architect v{VERSION} Online</b>", reply_markup=get_kb())
...
def set_engine(m):
    ...
    else: bot.send_message(cid, f"🎯 Target: {user_session[cid]['e']}\nInvia idea:")
...
def collect_idea(m):
    ...
    if cid not in user_session or not user_session[cid]['e']:
        ...
        bot.send_message(cid, "⚠️ Scegli motore:", reply_markup=get_kb())
        return
...
def generate_final_prompt(m):
    ...
    wait = bot.send_message(cid, "🏗️ <b>Generazione...</b>")
    
    if context:
        instruction = f"Post-prod expert. Context: '{context}'. Changes: '{idea}'. Rewrite SCENE in English. No face desc."
    else:
        instruction = f"Expand into SCENE: '{idea}'. Setting, lighting, clothes. No face desc. English only."
...
@app.route('/')
def health(): return "ARCHITECT_OK"
