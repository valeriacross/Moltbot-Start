...
def build_full_prompt(filter_key, outfit_description, scene_override=None, mode="single"):
    ...
    return (
        ...
        f"**⚠️ HAIR LOCK — ABSOLUTE PRIORITY:** The subject has SHORT silver-grey hair — an Italian crop "
        f"cut, short on the sides and back with the nape and ears clearly visible, slightly voluminous "
        f"only on top, ending well above the shoulders. Hair must remain fully present, this exact short "
        f"length, color and style — neither bald/shaved NOR long/flowing. Do NOT generate a bald head, "
        f"shaved head, buzzcut, receding hairline, OR long hair, shoulder-length hair, hair past the "
        f"shoulders, flowing hair, wavy long hair. Any hair shorter or longer than this exact short crop "
        f"is a generation FAILURE.\n\n"
        f"**⚠️ NEGATIVE PROMPT — IDENTITY & OUTFIT:** shaved face, clean-shaven, no beard, missing beard, beard removed, "
        f"stubble instead of beard, bald, shaved head, buzzcut, receding hairline, missing hair, hair loss, "
        f"long hair, shoulder-length hair, hair past the shoulders, flowing hair, wavy long hair, hair extensions, "
        f"female face, young face, missing glasses, face drift, body hair, chest hair, "
...
def build_shooting_prompt(outfit_desc, mode="mosaic"):
    ...
    if mode == "single":
        prompt = (
            ...
            f"**⚠️ HAIR LOCK — ABSOLUTE PRIORITY:** The subject has SHORT silver-grey hair — an Italian "
            f"crop cut, short on the sides and back with the nape and ears clearly visible, slightly "
            f"voluminous only on top, ending well above the shoulders. Hair must remain fully present, "
            f"this exact short length, color and style — neither bald/shaved NOR long/flowing. Do NOT "
            f"generate a bald head, shaved head, buzzcut, receding hairline, OR long hair, shoulder-length "
            f"hair, hair past the shoulders, flowing hair, wavy long hair. Any hair shorter or longer than "
            f"this exact short crop is a generation FAILURE.\n\n"
            ...
            f"bald, shaved head, buzzcut, receding hairline, missing hair, hair loss, "
            f"long hair, shoulder-length hair, hair past the shoulders, flowing hair, wavy long hair, hair extensions, "
        ...
        prompt = (
            ...
            f"**⚠️ HAIR LOCK — ABSOLUTE PRIORITY:** The subject has SHORT silver-grey hair — an Italian "
            f"crop cut, short on the sides and back with the nape and ears clearly visible, slightly "
            f"voluminous only on top, ending well above the shoulders — in EVERY shot. Hair must remain "
            f"fully present, this exact short length, color and style, identically across all 4 shots — "
            f"neither bald/shaved NOR long/flowing. Do NOT generate a bald head, shaved head, buzzcut, "
            f"receding hairline, OR long hair, shoulder-length hair, hair past the shoulders, flowing "
            f"hair, wavy long hair, in any shot. Any hair shorter or longer than this exact short crop, "
            f"in even one of the 4 shots, is a generation FAILURE.\n\n"
            f"**⚠️ CRITICAL — BACKGROUND LOCK:** The exact location and background described above MUST appear "
            ...
            f"beard removed, stubble instead of beard, bald, shaved head, buzzcut, receding hairline, missing hair, "
            f"hair loss, long hair, shoulder-length hair, hair past the shoulders, flowing hair, wavy long hair, "
            f"hair extensions, female face, face drift, desaturated colors, color shift, "
            f"clothing added where none exists, garments invented from color palette, "
            f"simplified outfit, missing embellishments, missing embroidery, missing gemstones, plain fabric "
            f"replacing detailed pattern, flattened texture, generic garment reinterpretation.\n"
        )
