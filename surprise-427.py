...
def analyze_photo_location(photo_bytes, mime_type="image/jpeg"):
    ...
    Strategia: chiede JSON strutturato con 7 campi, poi concatena tutto.
    Usa MODEL_ID (pro) per garantire risposte complete."""
    import json, re
    json_prompt = (
        "You are a fashion editorial location analyst. Analyze this photo carefully.\n"
        "Return ONLY a valid JSON object with exactly these 7 keys. No other text, no markdown.\n"
        "{\n"
        '  "subject": "main subject or phenomenon dominating the image (e.g.: monumental statue on tall pedestal, bare dead tree)",\n'
        '  "ground": "ground surface material, color, texture (e.g.: white marble tiles with black geometric inlay pattern)",\n'
        '  "sky": "sky condition, colors, clouds, phenomena (e.g.: blue hour sky, pink and violet clouds, dusk gradient)",\n'
        '  "colors": "list ALL dominant colors explicitly (e.g.: warm golden yellow, terracotta brown, deep blue, white)",\n'
        '  "lighting": "lighting quality, direction, source (e.g.: artificial warm uplighting on statue, natural blue hour ambient)",\n'
        '  "depth": "foreground, midground, background elements (e.g.: ornate plaza foreground, flanking trees and brick pillars, tall monument background)",\n'
        '  "mood": "emotional atmosphere (e.g.: monumental, solemn, dramatic, majestic)"\n'
        "}"
    )
    try:
        photo_p = genai_types.Part.from_bytes(data=photo_bytes, mime_type=mime_type)
        response = client.models.generate_content(
            model=MODEL_ID,  # usa pro per analisi completa
            contents=[photo_p, json_prompt],
            config=genai_types.GenerateContentConfig(
                temperature=0.4,
        ...
        if response.text:
            ...
            try:
                ...
                # Concatena tutti i campi in una descrizione ricca
                parts = [
                    data.get("subject", ""),
                    data.get("ground", ""),
                    data.get("sky", ""),
                ...
                desc = ", ".join(p.strip().rstrip(",") for p in parts if p.strip())
                logger.info(f"📍 Location da foto (JSON): {desc[:120]}...")
                return desc, None
            except json.JSONDecodeError:
                # Fallback: se sembra JSON ma non parsato, estrai valori con regex
                import re as _re
                values = _re.findall(r'"(?:subject|ground|sky|colors|lighting|depth|mood)"\s*:\s*"([^"]+)"', text)
                if values:
                    desc = ", ".join(v.strip().rstrip(",") for v in values if v.strip())
                    logger.warning(f"📍 Location fallback regex: {desc[:80]}")
                else:
                    desc = text.strip().strip(".")
                    logger.warning(f"📍 Location fallback raw: {desc[:80]}")
                return desc, None
...
def build_flow_prompt(scenario, uid=None):
    """Genera un prompt dettagliato stile Architect/Atelier per uso su Flow."""
    # Se c'è una foto, usa la descrizione estesa di Gemini Vision — non quella sintetica dello scenario
    ...
    photo_note = (
        "\n⚠️ REFERENCE PHOTO PROVIDED: Use the attached reference photo as the background "
        "environment. Reproduce faithfully: colors, materials, architectural/natural elements, "
        "atmosphere and lighting from the reference. The subject (Valeria Cross) must be "
        "composited INTO this environment — do not replace or ignore the background."
        if has_photo else ""
    )
...
def format_scenario(s):
    ...
    # Sanitizza: se la location contiene JSON grezzo, mostra solo le prime 80 chars
    if '{' in loc_display and '"subject"' in loc_display:
        ...
        loc_display = ", ".join(v.strip() for v in values if v.strip()) if values else loc_display[:80]
    return (
