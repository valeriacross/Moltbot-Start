...
def generate_caption_from_scenario(scenario):
    """Caption locale senza API — almeno 10 parole, emoji descrittive."""
    loc    = scenario.get('location', '')
    mood   = scenario.get('mood', '')
    sky    = scenario.get('sky', '')
    outfit = scenario.get('outfit', '')
    ...
    sky_text    = sky.split(',')[0].lower()
    outfit_text = outfit.split('·')[0].strip() if '·' in outfit else outfit.split(',')[0].strip()
    pose_text   = pose.split('·')[0].strip() if '·' in pose else pose.split(',')[0].strip()

    caption = (
        f"{emojis} {mood_text} mood · {sky_text} · "
        f"{outfit_text.lower()} · {pose_text.lower()} "
        f"at {loc_short}"
    )
    return caption[:200], None
