...
from C_shared100 import GeminiClient, CaptionGenerator, HealthServer, is_allowed, analyze_scene
...
server  = HealthServer("NOSURPRISE", VERSION)
...
def build_flow_prompt(scenario, uid=None):
    ...
    mode = user_shooting_mode.get(uid, 'single') if uid else 'single'
...
user_shooting_mode    = {}  # uid → 'single' | 'mosaic'
...
def format_scenario(s, uid=None):
    ...
    mode = user_shooting_mode.get(uid, 'single') if uid else 'single'
    mode_label = "🗂️ <b>Mosaico 4 foto</b>" if mode == 'mosaic' else "📷 <b>Scatto singolo</b>"
    return (
        f"{mode_label}\n"
...
def handle_location_photo(m):
    ...
    scene_desc, err = analyze_scene(img_bytes, gemini)
    try: bot.delete_message(cid, wait.message_id)
    except Exception: pass

    if not scene_desc:
        bot.send_message(cid,
            f"❌ Impossibile analizzare la foto dopo 5 tentativi.\n{err or ''}\n\n"
            f"Procedo senza foto — scegli il formato:",
            reply_markup=get_shooting_keyboard())
        return

    # Estrae solo la sezione BACKGROUND come location
    location_text = None
    for line in scene_desc.split('\n'):
        clean = line.strip().lstrip('*').strip()
        if clean.upper().startswith('BACKGROUND:'):
            location_text = clean[len('BACKGROUND:'):].strip().lstrip('*').strip()
            if location_text:
                break
    if not location_text:
        # Fallback: prende le prime 2 righe non vuote
        lines = [l.strip() for l in scene_desc.split('\n') if l.strip()]
        location_text = ' '.join(lines[:2])[:300]
...
def handle_callback(call):
    ...
    if data == "has_photo_no":
        ...
        user_photo_location.pop(uid, None)
        bot.send_message(cid, "Scegli il formato di output:", reply_markup=get_shooting_keyboard())
        return

    if data in ("shooting_single", "shooting_mosaic"):
        mode = "single" if data == "shooting_single" else "mosaic"
        user_shooting_mode[uid] = mode
        mode_label = "📷 Scatto singolo" if mode == "single" else "🗂️ Mosaico 4 foto"
        logger.info(f"🎬 {username} (id={uid}) → shooting mode: {mode}")
        try:
            bot.edit_message_text(
                f"✅ Formato: <b>{mode_label}</b>\n\nCome vuoi generare la scena?",
    ...
    if data == "mode_auto":
        ...
        def pick_auto():
            ...
            bot.send_message(cid, format_scenario(scenario, uid=uid), parse_mode="HTML")
...
if __name__ == '__main__':
    import time
    logger.info(f"📍 NOSURPRISE v{VERSION} — avvio")
    server.start()
