...
# --- FLOW PROMPT (ricco, da copiare su Flow) ---
EDITORIAL_WRAPPER = (
...
def build_flow_prompt(scenario, uid=None):
    """Prompt ricco per Flow — struttura identica ad Atelier/Vogue/Architect."""
    scenario = sanitize_scenario(scenario)
    fmt = random.choice(["2:3", "16:9"])

    loc_note = ""
    if uid and photo_location.get(uid):
        loc_note = "⚠️ LOCATION FROM REFERENCE PHOTO — recreate this exact environment as background. "

    prompt = (
        f"{EDITORIAL_WRAPPER}\n\n"
        f"{VALERIA_IDENTITY}\n"
        f"**TASK: HIGH-FASHION EDITORIAL PHOTOGRAPH**\n\n"
        f"**LOCATION & SCENE:**\n"
        f"{loc_note}{scenario['location']}.\n"
        f"Sky and lighting: {scenario['sky']}.\n\n"
        f"**⚠️ BACKGROUND LOCK — ABSOLUTE:** The exact location described above MUST appear in the image. "
        f"Never replace it with a studio, neutral backdrop or different environment. "
        f"The background IS the location. It must be fully rendered, recognisable and dominant.\n\n"
        f"**OUTFIT:**\n{scenario['outfit']}\n\n"
        f"**POSE & EXPRESSION:**\n"
        f"{scenario['pose']}. "
        f"Body must follow realistic human anatomy and physics — weight supported, no floating positions. "
        f"Expression: calm, self-aware, characteristic half-smile.\n\n"
        f"**MOOD & ATMOSPHERE:**\n{scenario['mood']}\n\n"
        f"**VISUAL STYLE:** {scenario['style']}\n\n"
        f"**FORMAT:** {fmt}\n\n"
        f"**TECHNICAL:** Ultra-detailed 8K cinematic photography. "
        f"85mm lens, f/2.8, ISO 200, 1/160s. Natural bokeh. "
        f"Subsurface Scattering, Global Illumination, Ambient Occlusion.\n\n"
        f"**⚠️ NEGATIVE PROMPT:** studio background replacing real location, wrong setting, "
        f"shaved face, clean-shaven, missing beard, beard removed, stubble instead of beard, "
        f"female face, young face, missing glasses, face drift, "
        f"flat chest, male physique, masculine body, body hair, chest hair, "
        f"extra fingers, blurry hands, mismatched skin tone, "
        f"text overlay, watermark (except mandatory one), JSON output, metadata.\n"
    )
    return prompt, fmt, None
...
LOCATION_ANALYZE_PROMPT = (
    "You are cataloguing a filming location for a high-fashion editorial shoot. "
    "Your job is identical to cataloguing an outfit: list EVERY visible element with maximum detail.\n\n"
    "Catalogue ALL of the following — miss nothing:\n"
    "- ARCHITECTURE: every structure, building, monument, wall, column, arch, tower, bridge visible\n"
    "- MATERIALS: stone type, tile pattern, metal, wood, concrete, brick — name the exact material and color\n"
    "- COLORS: list every dominant color explicitly — warm golden, terracotta, deep blue, chalk white, ochre, etc.\n"
    "- GROUND: exact surface — cobblestones, marble tiles, sand, grass, water, cracked clay, pavement pattern\n"
    "- SKY: cloud type, sky color, gradients, phenomena — star trails, fog, storm, golden haze, blue hour, aurora\n"
    "- LIGHTING: source, direction, quality — artificial uplighting, backlit sunset, diffused overcast, neon glow\n"
    "- VEGETATION: trees, flowers, vines, grass — species if identifiable, colors, density\n"
    "- WATER: sea, river, lake, pool, waves, reflections — color, movement, distance\n"
    "- DEPTH: what is in foreground / midground / background\n"
    "- ATMOSPHERE: emotional quality — desolate, cosmic, romantic, dramatic, monumental, intimate, surreal\n\n"
    "Format: comma-separated noun phrases, no verbs, no sentences. "
    "Same level of detail as a fashion stylist cataloguing every thread of an outfit. "
    "Write at least 80 words. Every element you see must appear in the output.\n"
    "Return ONLY the catalogue. Nothing else."
)

def analyze_photo_location(photo_bytes, mime_type="image/jpeg"):
    """Analisi location con approccio Atelier: struttura a campi obbligatori, 3000 token."""
    try:
        ...
        response = client.models.generate_content(
            ...
            contents=[
                photo_p,
                "This is a BACKGROUND REFERENCE image for a fashion editorial shoot. "
                "Describe ONLY the empty environment — the set, the backdrop, the location itself. "
                "Ignore completely any people, models, faces or bodies that may appear in the image.\n\n"
                "Return EXACTLY this structure — fill every field with maximum detail:\n"
                "SETTING: [Location type and main architectural or natural element — name it specifically]\n"
                "GROUND: [Exact surface material, color, texture, pattern visible underfoot or in foreground]\n"
                "SKY: [Sky condition, colors, clouds, time of day, any special phenomena — star trails, aurora, fog, storm, golden haze]\n"
                "COLORS: [Every dominant color present — be specific: warm golden yellow, terracotta orange, cobalt blue, chalk white, ochre]\n"
                "LIGHTING: [Light source, direction, quality — artificial uplighting, backlit sunset, long exposure night, overcast diffused, neon]\n"
                "SURROUNDINGS: [Everything visible around — trees, water, distant buildings, vegetation, horizon, background elements]\n"
                "ATMOSPHERE: [Emotional quality of this empty place — monumental, desolate, cosmic, romantic, dramatic, surreal, intimate]\n\n"
                "IMPORTANT: describe ONLY the location. Do NOT mention any person, model, human figure or body."
            ],
            config=genai_types.GenerateContentConfig(
                temperature=0.2,
        ...
        if response.text:
            ...
            logger.info(f"📍 Location analizzata ({len(desc)} chars): {desc[:100]}")
            return desc, None
