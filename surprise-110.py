...
# --- HELPER: singola chiamata Gemini image ---
...
# --- PIPELINE DUE STEP: T2I scena + I2I faceswap ---
def execute_generation(full_prompt, formato="2:3"):
    try:
        # STEP 1: genera scena T2I pura (senza masterface)
        logger.info("🎨 Step 1: generazione scena T2I...")
        scene_img, err = _gemini_generate([full_prompt])
        if not scene_img:
            return None, f"❌ Step 1 fallito: {err}"

        # STEP 2: faceswap I2I — scena + masterface
        if MASTER_PART:
            logger.info("👤 Step 2: faceswap I2I con masterface...")
            faceswap_prompt = (
                "You have two images: the first is a fashion scene, the second is a face reference. "
                "Replace the face in the scene with the face from the reference. "
                "Preserve ALL scene elements (background, outfit, pose, lighting, body) exactly. "
                "The face must be naturally integrated with matching lighting, skin tone and perspective. "
                "ZERO face drift. Return the complete image with only the face replaced."
            )
            scene_part = genai_types.Part.from_bytes(data=scene_img, mime_type="image/jpeg")
            img, err2 = _gemini_generate([faceswap_prompt, scene_part, MASTER_PART])
            if img:
                logger.info("✅ Step 2 completato.")
                return img, None
            logger.warning(f"⚠️ Step 2 fallito ({err2}) — fallback su Step 1")
            return scene_img, None
        else:
            logger.warning("⚠️ MASTER_PART non disponibile — skip faceswap")
            return scene_img, None
...
def generate_caption(scenario):
    try:
        prompt = (
            ...
            f"Rules: write exactly 1 complete sentence (never cut it short), rich with emojis (4-6 emojis), poetic and fashion-forward tone. "
            f"The sentence MUST be complete with proper punctuation. Return ONLY the caption, nothing else."
        )
        response = client.models.generate_content(
            model=MODEL_TEXT_ID,
            contents=[prompt],
