...
def handle_photo(m):
    ...
    if uid in mosaic_collecting:
        ...
        return
    # ─────────────────────────────────────────────────────────────────────────
    # Cleanup sessione mosaic zombie — può esistere se l'utente ha abbandonato
    # una sessione mosaic senza passare da /start o /mosaic (es. timeout timer
    # scattato dopo che handle_photo era già uscito dal ramo mosaic_collecting)
    if uid in mosaic_collecting:
        session = mosaic_collecting.pop(uid)
        if session.get('timer'):
            try: session['timer'].cancel()
            except Exception: pass
        logger.info(f"   🧹 Sessione mosaic zombie rimossa in handle_photo per uid={uid}")
...
def _run_generation(chat_id, uid, username, data):
    ...
    try:
        ...
        cap_result = CaptionGenerator.local(full_prompt, filter_label=fname)
        if cap_result:
