...
def generate_scenario(uid=None):
    ...
    # Esclude location già usate nella sessione (fallback: tutto il pool se esaurite)
    used_loc = used_locations.get(_uid_for_gen, set())
    available_loc = [l for l in LOCATION_POOL if l[0] not in used_loc] or LOCATION_POOL
...
LOCATION_ANALYZE_PROMPT = (
    "You are a fashion editorial location analyst. "
    "Analyze this photo and write a detailed description of the location for an AI image generator. "
    "Include ALL of these elements if visible: "
    "1) environment type (beach, cave, forest, rooftop, cliff, etc.), "
    "2) dominant colors and materials, "
    "3) key architectural or natural features (arches, rocks, walls, water, vegetation), "
    "4) lighting quality and time of day, "
    "5) atmosphere and mood of the place. "
    "Write 40-60 words maximum. Use comma-separated descriptive phrases. No sentences with verbs. "
    "Be specific — avoid vague words like 'secluded' or 'beautiful'. Describe what you SEE.\n"
    "Example: dramatic sea cave with ochre limestone arch, turquoise emerald water inside, small sandy beach, aerial perspective, warm golden rock tones, enclosed dramatic atmosphere, boat on water, natural light filtered through arch opening\n"
    "Example: sun-drenched Amalfi terrace, terracotta floor tiles, cascading bougainvillea, city panorama below, turquoise sea horizon, warm golden afternoon light, Mediterranean atmosphere\n"
    "IMPORTANT: Your response MUST be at least 40 words. A response shorter than 40 words is WRONG. Describe every visible element specifically.\n""Return ONLY the description, nothing else."
)
...
def get_retry_keyboard(has_flow=False):
    markup = InlineKeyboardMarkup()
    markup.row(
...
def send_manual_step(cid, uid, step_index):
    ...
    if step_index == 0:  # location
        ...
        filtered = [o for o in pool if o[0] not in used] or pool
        if len(filtered) < len(pool):
            note = f" <i>({len(pool)-len(filtered)} già usati nascosti)</i>"
    ...
    if data == "mode_auto":
        ...
        def run():
            try:
                ...
                if img:
                    # 4.1 — genera e salva prompt Flow-ready
                    flow_p = build_flow_prompt(scenario, uid=uid)
                    last_flow_prompt[uid] = flow_p
                    has_photo = bool(photo_part.get(uid))
                    # Invia riepilogo standard
                    bot.send_message(cid, format_scenario(scenario), parse_mode="HTML")
                    bot.send_document(cid, io.BytesIO(img),
                        visible_file_name="surprise.jpg",
                        caption=f"✅ {elapsed}s | {fmt}{loc_note}",
                        reply_markup=get_retry_keyboard(has_flow=True))
...
def handle_photo_location(message):
    ...
    def process():
        try:
            ...
            # Salva location nello state e procedi al passo 1 (outfit)
            if uid not in manual_state:
                manual_state[uid] = {}
            manual_state[uid]['location'] = desc
            send_manual_step(cid, uid, 1)  # step 1 = outfit
